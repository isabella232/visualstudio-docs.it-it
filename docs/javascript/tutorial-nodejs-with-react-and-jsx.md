...
1. Creare un nuovo progetto.
    ...
    ::: moniker range=">=vs-2019"
    Premere **ESC** per chiudere la finestra iniziale. Digitare **CTRL + Q** per aprire la casella di ricerca, digitare **Node.js**, quindi scegliere **vuota Node.js applicazione Web-JavaScript**. Sebbene in questa esercitazione venga utilizzato il compilatore TypeScript, i passaggi necessari per iniziare con il modello **JavaScript** .
...
1. Anziché utilizzare l'interfaccia utente per cercare e aggiungere il resto dei pacchetti uno alla volta, incollare il codice seguente in *package.js*. A tale scopo, aggiungere una sezione `dependencies` con il codice seguente:
    ...
    Se la versione del modello vuoto in uso contiene già una sezione `dependencies`, sostituirla con il codice JSON precedente. Per altre informazioni sull'uso di questo file, vedere [package.jsin Configuration](../javascript/configure-packages-with-package-json.md).
...
4. Scegliere **debug**  >  **Connetti a processo**.
