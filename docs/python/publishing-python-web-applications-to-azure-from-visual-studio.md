---
title: Pubblicazione di un'app Python nel servizio app di Azure
...
ms.locfileid: "39468670"
---
# <a name="publish-to-azure-app-service"></a>Eseguire la pubblicazione nel servizio app di Azure

> [!Important]
> La distribuzione delle app Python in Servizio app di Azure per Linux non è al momento supportata da Visual Studio. Microsoft intende anche deprecare Python nel servizio app in Windows. Gli aggiornamenti verranno pubblicati in questo articolo quando disponibili. Nel frattempo è possibile eseguire la distribuzione nel servizio app in Linux usando i contenitori. Per altre informazioni, vedere [Distribuire un'app Web Python in app Web per contenitori](/azure/app-service/containers/quickstart-python).
...
Il processo di pubblicazione è diverso in Visual Studio 2017 e in Visual Studio 2015. In particolare, Visual Studio 2015 automatizza alcuni passaggi, inclusa la creazione del file *web.config*, ma questa automazione limita la flessibilità e il controllo a lungo termine. Visual Studio 2017 richiede più passaggi manuali ma offre un controllo più preciso sull'ambiente Python. Entrambe le opzioni sono descritte di seguito.
...
### <a name="use-a-subscription"></a>Usare una sottoscrizione
...
1. Selezionare **+Nuovo**, quindi **Web e dispositivi mobili** e infine **App Web**.
...
### <a name="use-a-temporary-app-service"></a>Usare un servizio app temporaneo
...
1. In Visual Studio, in **Esplora soluzioni** fare clic con il pulsante destro del mouse sul progetto e scegliere **Aggiungi** > **Nuovo elemento**. Nella finestra di dialogo visualizzata selezionare il modello **Azure web.config (Fast CGI)** e scegliere **OK**. Verrà creato un file *web.config* nella radice del progetto.
...
1. Impostare la voce `WSGI_HANDLER` in *web.config* in base al framework in uso:
    ...
    - **Bottle**: aggiungere le parentesi dopo `app.wsgi_app` come illustrato di seguito. Questa operazione è necessaria perché l'oggetto è una funzione (vedere *app.py*) anziché una variabile:
...
1. **Solo app Django**: nel file *settings.py* del progetto Django aprire e aggiungere il dominio dell'URL del sito in `ALLOWED_HOSTS` come illustrato di seguito, sostituendo 'vspython-test-02.azurewebsites.net' con il proprio URL:
    ...
    Si noti che quando la matrice è vuota, Django autorizza automaticamente 'localhost', ma l'aggiunta dell'URL di produzione rimuove tale funzionalità. Per questo motivo è consigliabile mantenere copie separate di *settings.py* per sviluppo e produzione, oppure usare le variabili di ambiente per controllare i valori della fase di esecuzione.

1. In **Esplora soluzioni** espandere la cartella con lo stesso nome del progetto, fare clic con il pulsante destro del mouse sulla cartella **static**, scegliere **Aggiungi** > **Nuovo elemento**, selezionare il modello **web.config di file statici di Azure** e scegliere **OK**. Questa azione crea un altro file *web.config* nella cartella *static* che disabilita l'elaborazione Python per la cartella. Questa configurazione invia le richieste per i file statici al server Web predefinito anziché usare l'applicazione Python.
...
1. All'apertura del browser è possibile che venga visualizzato il messaggio **Impossibile visualizzare la pagina a causa di un errore interno del server.** Questo messaggio indica che l'ambiente Python nel server non è configurato completamente. In tal caso, seguire questa procedura:
    ...
    1. Fare di nuovo riferimento a [Gestire Python nel servizio app di Azure](managing-python-on-azure-app-service.md) e assicurarsi di aver installato un'estensione del sito Python appropriata.
    ...
    4. Per l'output dell'errore dettagliato, aggiungere la riga seguente a *web.config* all'interno del nodo `<system.webServer>` che visualizza un output dell'errore più dettagliato:

    ```xml
    <httpErrors errorMode="Detailed"></httpErrors>
    ```

    5. Provare a riavviare il servizio app dopo l'installazione dei nuovi pacchetti. Quando si modifica *web.config* non è necessario riavviare poiché il servizio app esegue un riavvio automatico ogni volta che viene modificato il file *web.config*.
...
## <a name="publish-to-app-service---visual-studio-2015"></a>Pubblicare nel servizio app - Visual Studio 2015
...
1. Se necessario, scegliere **Avanti** per esaminare le impostazioni aggiuntive. Se si intende di [eseguire il debug del codice Python in Azure in modalità remota](debugging-remote-python-code-on-azure.md), è necessario impostare **Configurazione** su **Debug**.
...
È anche possibile gestire il proprio file *web.config* e usare il file *requirements.txt* per gestire i pacchetti direttamente nel server. L'uso del file *requirements.txt*, in particolare, garantisce che l'ambiente di sviluppo e l'ambiente del server corrispondano sempre.
...
Con Visual Studio 2017, questi componenti vengono aggiunti direttamente al progetto. Fare clic con il pulsante destro del mouse sul progetto in **Esplora soluzioni**, scegliere **Aggiungi** > **Nuovo elemento** e selezionare il modello **web.config di debug remoto di Azure**. Nel progetto verranno visualizzati un file *web.debug.config* di debug e la cartella degli strumenti *ptvsd*.
