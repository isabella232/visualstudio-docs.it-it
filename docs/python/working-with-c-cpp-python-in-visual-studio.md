...
1. Creare un nuovo progetto Python in Visual Studio selezionando **File** New Project  >  **(File nuovo**  >  **progetto).** Cercare "Python", selezionare il modello **Applicazione Python**, assegnare a questo un nome e un percorso appropriati e selezionare **OK**.
...
> [!TIP]
> Quando si eseguono benchmark, usare sempre Avvia debug senza eseguire debug per evitare l'overhead generato durante l'esecuzione di codice all'interno  >   Visual Studio debugger.
...
1. Impostare le proprietà specifiche, come descritto nella tabella seguente e quindi selezionare **OK**.
    ...
    | **C/C++**  >  **Generazione di codice** | **Libreria di runtime** | **DLL multithread (/MD)** (vedere l'avviso più avanti) |
...
1. Aggiungere una struttura che definisca il modulo come si vuole farvi riferimento nel codice Python, in particolare quando si usa l'istruzione `from...import`. Fare in modo che corrisponda al valore nelle proprietà del progetto in **Proprietà di configurazione**  >  **Generale**  >  **Nome destinazione**.) Nell'esempio seguente il nome del modulo "superfastcode" indica che è possibile usare in Python, perché `from superfastcode import fast_tanh` è definito all'interno di `fast_tanh` `superfastcode_methods` . I nomi di file interni al progetto C++, ad esempio *module.cpp,* non sono sequenziali.
...
1. Creare un file denominato *setup.py* nel progetto C++ facendo clic con il pulsante destro del mouse sul progetto e scegliendo **Aggiungi** > **Nuovo elemento**. Selezionare quindi **File di C++ (.cpp)**, assegnare il nome `setup.py` al file e selezionare **OK**. L'assegnazione di un nome al file con estensione *py* consente di fare in modo che Visual Studio lo riconosca come Python nonostante l'uso del modello di file C++. Quando il file viene visualizzato nell'editor, incollare nel file il codice seguente in base al metodo di estensione:
    ...
    **Estensioni CPython (progetto superfastcode):**
    ...
    Vedere [Compilazione di estensioni C e C++](https://docs.python.org/3/extending/building.html) (python.org) per la documentazione su questo script.
...
1. Provare a incrementare la variabile `COUNT` in modo che le differenze siano più evidenti. Anche **una** build di debug del modulo C++ viene  eseguita più lentamente rispetto **a** una build di versione perché la build di debug è meno ottimizzata e contiene vari controlli degli errori. È possibile alternare liberamente queste configurazioni ai fini del confronto.
...
1. Impostare la configurazione di compilazione su **Debug** nella barra Visual Studio di lavoro.
...
| Cython (consigliato per C) | 2007 | [gevent](https://www.gevent.org/), [kivy](https://kivy.org/) |
