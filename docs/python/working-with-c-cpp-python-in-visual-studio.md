...
ms.locfileid: "109973440"
---
# <a name="create-a-c-extension-for-python"></a>Creare un'estensione C++ per Python

I moduli scritti in C++ (o C) vengono comunemente usati per estendere le funzionalità di un interprete Python. Vengono usati anche per consentire l'accesso alle funzionalità del sistema operativo di basso livello. 

I moduli sono disponibili in tre tipi principali:

- **Moduli acceleratore:** poiché Python è un linguaggio interpretato, è possibile scrivere moduli acceleratore in C++ per ottenere prestazioni migliori.
...
- **Moduli di accesso al sistema di basso livello:** è possibile creare questi moduli per accedere alle funzionalità di livello inferiore del runtime, del sistema operativo `CPython` o dell'hardware sottostante.

Questo articolo illustra come creare un modulo di estensione C++ per che calcola una `CPython` tangente iperbolica e la chiama dal codice Python. La routine viene implementata prima in Python per illustrare il miglioramento relativo delle prestazioni dell'implementazione della routine stessa in C++.
...
- Usare [PyBind11,](https://github.com/pybind/pybind11)consigliato per C++11 per motivi di semplicità.

L'esempio completo di questa procedura dettagliata è riportato in GitHub all'indirizzo [python-samples-vs-cpp-extension.](https://github.com/Microsoft/python-sample-vs-cpp-extension)
...
- Visual Studio 2017 o versione successiva, con il carico di lavoro Sviluppo Python installato. Il carico di lavoro include gli strumenti di sviluppo nativi Python, che includono il carico di lavoro C++ e i set di strumenti necessari per le estensioni native.
    ...
    ![Screenshot di un elenco di opzioni di sviluppo Python, con l'opzione Strumenti di sviluppo nativi Python evidenziata.](media/cpp-install-native.png)

    > [!NOTE]
    > Quando si installa il **carico di lavoro Applicazioni analitiche** e di analisi scientifica dei dati, Python e l'opzione Strumenti di sviluppo nativi **Python** vengono installati per impostazione predefinita.
...
1. Creare un nuovo progetto Python in Visual Studio selezionando **File**  >  **nuovo**  >  **progetto**. Cercare **Python,** selezionare il modello **Applicazione Python,** immettere un nome e un percorso e quindi selezionare **OK.**
...
1. Per visualizzare i risultati, eseguire il programma selezionando **Avvia** debug senza eseguire debug  >   o premendo CTRL+F5. 
   ...
   > [!TIP]
   > Quando si eseguono benchmark, usare sempre **Avvia debug**  >  **senza eseguire debug**. In questo modo è possibile evitare il sovraccarico che si verifica quando si esegue il codice all'interno del debugger Visual Studio debugger.
...
1. In **Esplora soluzioni** fare clic con il pulsante destro del mouse sulla soluzione e quindi scegliere **Aggiungi**  >  **nuovo progetto**. Una Visual Studio può contenere progetti Python e C++, uno dei vantaggi dell'uso di Visual Studio per Python.
...
1. Selezionare **File C++,** assegnare al file il *nome module.cpp* e quindi selezionare **OK.**

    > [!Important]
    > Un file con estensione *cpp* è necessario per attivare le pagine delle proprietà C++ nei passaggi che seguono.

1. Nella barra degli strumenti principale usare il menu a discesa per eseguire una delle operazioni seguenti:
...
1. Impostare le proprietà come descritto nella tabella seguente:
    ...
    | **C/C++** > **Generazione codice** | **Libreria di runtime** | **DLL multi-thread (/MD) in** modo che corrisponda alla versione non di debug di CPython. Quando si usa *python_d.exe*, lasciare questo valore come DLL di **debug multi-thread (/MDd).** |
    ...
    | Scheda | Proprietà | Valore |
    | --- | --- | --- |
    | **Generale** | **Generale** > **Nome di destinazione** | Specificare il nome del modulo a cui fare riferimento da Python nelle `from...import` istruzioni . Lo stesso nome viene utilizzato nel codice C++ quando si definisce il modulo per Python. Per usare il nome del progetto come nome del modulo, lasciare il valore predefinito **$\<ProjectName>** . Per `python_d.exe` , aggiungere alla fine del `_d` nome. |
...
1. Aggiungere una struttura che definisce il modulo a cui si vuole fare riferimento nel codice Python, in particolare quando si usa `from...import` l'istruzione . 
   ...
   Nell'esempio seguente il nome del modulo indica che è possibile usare in Python, perché `"superfastcode"` è definito `from superfastcode import fast_tanh` `fast_tanh` all'interno di `superfastcode_methods` . I nomi di file interni al progetto C++, ad esempio *module.cpp,* non sono sequenziali.
...
1. Aggiungere un metodo che Python chiama quando carica il modulo, che deve essere denominato , dove corrisponde esattamente alla proprietà General Target Name del progetto `PyInit_<module-name>` *\<module-name>*   >   C++. Ciò significa che corrisponde al nome file del file *con estensione pyd* compilato dal progetto.
...
1. Compilare di nuovo il progetto C++ per verificare il codice. Se si verificano errori, vedere la [sezione "Risoluzione dei problemi".](#troubleshoot-compiling-failures)

### <a name="use-pybind11"></a>Usare PyBind11

Se è stata completata la procedura della sezione precedente, si sarà notato l'uso di una grande quantità di codice boilerplate per creare le strutture di modulo necessarie per il codice C++. PyBind11 semplifica il processo tramite macro in un file di intestazione C++ che hanno lo stesso risultato, ma con molto meno codice. 

Per altre informazioni sul codice in questa sezione, vedere Informazioni di base su [PyBind11.](https://github.com/pybind/pybind11/blob/master/docs/basics.rst)
...
1. Nella parte superiore di un *file module.cpp* nuovo che non include le modifiche della sezione precedente, includere *pybind11.h:*

    ```cpp
    #include <pybind11/pybind11.h>
    ```

1. Nella parte inferiore di *module.cpp* usare la macro per definire il `PYBIND11_MODULE` punto di ingresso alla funzione C++:
...
* Un metodo alternativo installa il modulo nell'ambiente Python, che rende il modulo disponibile anche per altri progetti Python. Per altre informazioni, vedere la documentazione [ **del progetto setuptools.**](https://setuptools.readthedocs.io/) Eseguire le operazioni seguenti:
    ...
    1. Creare un secondo file denominato *pyproject.toml* nel progetto C++ e incollarlo nel codice seguente:
    ...
    1. Per compilare l'estensione, fare clic con il pulsante destro del mouse sulla scheda *apri pyproject.toml* e quindi scegliere **Copia percorso completo**. Il nome *pyproject.toml* verrà eliminato dal percorso prima di usarlo.
...
1. Aggiungere le righe seguenti nel file *py* per chiamare i metodi esportati dalle DLL e visualizzarne gli output:
...
1. Eseguire il programma Python selezionando **Avvia debug**  >  **senza eseguire** debug o premendo CTRL+F5.
    ...
    > [!NOTE]
    > Se il **comando Avvia** senza eseguire debug è disabilitato, in Esplora soluzioni fare clic con il pulsante destro del mouse sul progetto Python e quindi scegliere Imposta come progetto di **avvio**.   
...
Nell'output si potrebbe vedere che l'estensione PyBind11 non è veloce come l'estensione CPython, anche se dovrebbe essere notevolmente più veloce rispetto all'implementazione Python pura. Questa differenza è in gran parte perché è stata usata la chiamata , che non supporta più parametri, nomi di `METH_O` parametro o argomenti di parole chiave. PyBind11 genera codice leggermente più complesso per fornire un'interfaccia più simile a Python ai chiamanti. Tuttavia, poiché il codice di test chiama la funzione 500.000 volte, i risultati potrebbero aumentare notevolmente questo sovraccarico.

È possibile ridurre ulteriormente l'overhead spostando `for` il ciclo nel codice nativo. Questo approccio comporterebbe l'uso del protocollo [iteratore](https://docs.python.org/c-api/iter.html) (o del tipo PyBind11 per il parametro della funzione ) per `py::iterable` elaborare ogni elemento. [](https://pybind11.readthedocs.io/en/stable/advanced/functions.html#python-objects-as-args) La rimozione delle transizioni ripetute tra Python e C++ è un modo efficace per ridurre il tempo necessario per elaborare la sequenza.

### <a name="troubleshoot-importing-errors"></a>Risolvere gli errori di importazione
...
| [PyBind11](https://github.com/pybind/pybind11) (consigliato per C++) | 2015 |  |
