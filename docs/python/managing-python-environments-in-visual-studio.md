...
> [!Note]
> Per ottenere risultati ottimali con gli ambienti Conda, usare Conda 4.4.8 o versione successiva (le versioni Conda sono diverse rispetto alle versioni Anaconda). Installazione di Anaconda 5.1 dal programma di installazione di Visual Studio 2017
...
La selezione di un ambiente non ne determina l'attivazione in alcun modo. L'ambiente predefinito, visualizzato in grassetto nell'elenco, è l'ambiente attualmente attivato che Visual Studio usa per i nuovi progetti. Per attivare un ambiente diverso, usare il comando **Imposta come ambiente predefinito per i nuovi progetti**. Nel contesto di un progetto è sempre possibile attivare un ambiente diverso. Per altre informazioni, vedere [Selezione di un ambiente per un progetto](selecting-a-python-environment-for-a-project.md).
...
1. Passare a `HKEY_LOCAL_MACHINE\SOFTWARE\Python` per interpreti a 32 bit o a `HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Python` per interpreti a 64 bit. Per IronPython, cercare invece `IronPython`.
...
1. Controllare i valori nel nodo `InstallPath`:
    ...
    - Se l'ambiente è ancora presente nel computer, modificare il valore di `ExecutablePath` nella posizione corretta. Correggere anche i valori `(Default)` e `WindowedExecutablePath` se necessario.
...
1. All'interno di Visual Studio è possibile attivare un ambiente Conda per un progetto come per qualsiasi altro ambiente come descritto in [Selezione di un ambiente per un progetto](selecting-a-python-environment-for-a-project.md).
