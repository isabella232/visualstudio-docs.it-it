...
| Il meccanismo consente di specificare che cosa non deve essere copiato. | Y | A seconda del fatto che i file vengano eliminati nella cartella *Redist* o *\designtime* , è possibile controllare quali file copiare nel pacchetto dell'applicazione consumer. | N | I file da copiare vengono dichiarati dall'utente nel manifesto del pacchetto. |
...
| Il meccanismo distribuisce file non di riferimento, ad esempio un framework di test con cui eseguire i test delle app di [!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)]. | Y | Se si eliminano i file nella cartella *Redist* , i file vengono distribuiti automaticamente. | Y | |
...
| Il meccanismo contiene un file con *estensione exe* autonomo per la creazione e l'utilizzo di pacchetti. | Y | L'SDK contiene *MSBuild.exe*. | Y | |
...
| È possibile usare un pacchetto simboli per il supporto del debug. | Y | Se si trascinano i file con *estensione PDB* nell'SDK, i file vengono prelevati automaticamente. | Y | |
