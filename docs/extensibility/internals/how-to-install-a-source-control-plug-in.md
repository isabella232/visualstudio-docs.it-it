...
1. Aggiungere due voci sotto la chiave **HKEY_LOCAL_MACHINE** della sottochiave **software** che specifica la sottochiave del nome della società seguita dalla sottochiave del nome del prodotto. Il modello è **HKEY_LOCAL_MACHINE \\ \<company name> \\ \<product name> \\ \<entry> **  =  *valore*\SOFTWARE. Le due voci sono sempre denominate **SCCServerName** e **SCCServerPath**. Ogni è una stringa normale.
