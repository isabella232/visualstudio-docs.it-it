...
- Una pagina Web intelligente può essere associata a un linguaggio di programmazione lato server usando la direttiva di pagina o il tag di > \<script runat = "Server".
...
- Un sistema di progetto di sito Web viene implementato aggiungendo modelli di progetti e di elementi e la registrazione al progetto [!INCLUDE[vwprvw](../../extensibility/internals/includes/vwprvw_md.md)].

- Un sistema WAP viene implementato come sottotipo di progetto, detto anche versione del progetto. Il progetto [!INCLUDE[vwprvw](../../extensibility/internals/includes/vwprvw_md.md)] è caratterizzato dal sottotipo WAP per creare il sistema WAP. Per altre informazioni sui sottotipi di progetto, vedere [sottotipi di progetto](../../extensibility/internals/project-subtypes.md).

- Una pagina Web intelligente combina il codice HTML con un linguaggio di programmazione lato server. Il linguaggio del lato server è denominato linguaggio contenuto. Per supportare un linguaggio contenuto, il sistema del progetto Web deve implementare la famiglia di interfacce <xref:Microsoft.VisualStudio.TextManager.Interop.IVsContainedLanguage>.
