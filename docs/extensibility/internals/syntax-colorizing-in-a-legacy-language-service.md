...
> [!NOTE]
> Se si modifica il nome di uno dei colori durante lo sviluppo, è necessario reimpostare la cache creata Visual Studio al primo accesso ai colori. A tale scopo, eseguire il comando Reset the Experimental Hive (Reimposta **hive sperimentale)** dal menu Visual Studio programma SDK.

 Si noti che non viene mai fatto riferimento al primo elemento nell'elenco di elementi colorabili. Visual Studio fornisce sempre i colori del testo e gli attributi predefiniti per l'elemento. Il modo più semplice per gestire questo problema è fornire un elemento colorabile segnaposto come primo elemento.
