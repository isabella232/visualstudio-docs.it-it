...
## <a name="parsing-for-colorization"></a>Analisi per la colorazione
 ...
 La <xref:Microsoft.VisualStudio.Package.Colorizer> classe viene usata anche per identificare i trigger di token. Questi trigger indicano all'MPF che un token specifico può avviare un'operazione più complessa, ad esempio il completamento delle parole o le parentesi graffe corrispondenti. Poiché l'identificazione di questi trigger deve essere veloce e deve verificarsi in qualsiasi posizione, lo scanner è più adatto per questa attività.
