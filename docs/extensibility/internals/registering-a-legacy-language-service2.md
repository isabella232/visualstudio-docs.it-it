...
Le sezioni seguenti forniscono elenchi di voci del registro di sistema per le varie opzioni del servizio di linguaggio disponibili in [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)].
 ...
 Nel seguente elenco di voci del registro di sistema, *vs reg root* è uguale a HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\VisualStudio \\*X. y*, dove *x. y* è il numero di versione [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)].
...
## <a name="registry-entries-for-language-service-options"></a>Voci del registro di sistema per le opzioni del servizio di linguaggio
 I servizi \Languages\Language *radice di vs Reg*\\ chiave*nome lingua* possono contenere i valori seguenti.

|Name|Digitare|Intervallo|Descrizione|
|----------|----------|-----------|-----------------|
...
|Supporto di CF_HTML|REG_DWORD|0-1|Specifica se l'editor consente di copiare e incollare i dati HTML.|
...
## <a name="registry-entries-for-debugger-languages-options"></a>Voci del registro di sistema per le opzioni dei linguaggi del debugger
 I servizi \Languages\Language della *radice di vs Reg*\\*nome lingua*\Debugger lingue \\*GUID*\ chiave possono includere i valori seguenti.

|Name|Digitare|Intervallo|Descrizione|
|----------|----------|-----------|-----------------|
|(Predefinito)|REG_SZ|ASCII|Il valore predefinito può essere utilizzato per documentare il nome della lingua. Il nome di questa chiave è un GUID di un analizzatore di espressioni con una voce corrispondente nella *\<VS radice Reg >* analizzatore \AD7Metrics\Expression.|
...
|Name|Digitare|Intervallo|Descrizione|
|----------|----------|-----------|-----------------|
|(Predefinito)|REG_SZ|Da un Resid|Nome visualizzato localizzato della pagina di opzioni. Il nome può essere un testo letterale o # `nnn`, dove `nnn` è un ID di risorsa stringa nella DLL satellite del pacchetto VSPackage specificato.|
|Pacchetto|REG_SZ|*GUID*|GUID del pacchetto VSPackage che implementa questa pagina di opzioni.|
|Pagina|REG_SZ|*GUID*|GUID della pagina delle proprietà da richiedere dal pacchetto VSPackage chiamando il metodo <xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage.GetPropertyPage%2A>. Se questa voce del registro di sistema non è presente, la chiave del registro di sistema descrive un nodo, non una pagina.|
...
|Name|Digitare|Intervallo|Descrizione|
|----------|----------|-----------|-----------------|
|(Predefinito)|REG_SZ|*GUID*|GUID del servizio per il servizio di linguaggio predefinito per questo tipo di estensione del nome di file.|
...
|Name|Digitare|Intervallo|Descrizione|
|----------|----------|-----------|-----------------|
|(Predefinito)|REG_SZ|""|Inutilizzati qui è possibile inserire il nome per la documentazione.|
...
## <a name="registry-entries-for-logical-view-options"></a>Voci del registro di sistema per le opzioni di visualizzazione logica
 L'*interfaccia utente*di \Editors \\ editor *radice di vs reg*> chiave \LogicalViews può contenere i valori seguenti.
...
## <a name="registry-entries-for-editor-extension-options"></a>Voci del registro di sistema per le opzioni di estensione dell'editor
 La chiave \Editors del GUID dell'*Editor*\\ di *vs reg radice*\Extensions può contenere i valori seguenti. L'estensione del nome file non include il punto iniziali.
...
|*\<ext >*|REG_DWORD|0-0xFFFFFFFF|Priorità relativa delle estensioni. Se due o più lingue condividono la stessa estensione, viene scelta la lingua con priorità più alta.|
 ...
 Inoltre, la selezione predefinita dell'utente corrente per un editor è archiviata in HKEY_Current_User\Software\Microsoft\VisualStudio \\*X. Y*\Default editor \\*ext*. Il GUID del servizio di linguaggio selezionato si trova nella voce personalizzata. Questa operazione ha la precedenza sull'utente corrente.
...
## <a name="registry-entries-for-managed-package-framework-language-service-options"></a>Voci del registro di sistema per le opzioni del servizio di linguaggio Managed Package Framework
 ...
 È possibile accedere a queste voci del registro di sistema tramite la classe <xref:Microsoft.VisualStudio.Package.LanguagePreferences>.
