...
L'uso delle classi del linguaggio MPF (Managed Package Framework) per implementare un servizio di linguaggio in [!INCLUDE[csprcs](../../data-tools/includes/csprcs_md.md)] è semplice. È necessario un VSPackage per ospitare il servizio di linguaggio, il servizio di linguaggio stesso e un parser per il linguaggio.
...
## <a name="locations-for-the-visual-studio-package-project-template"></a>Posizioni del modello di progetto di pacchetto di Visual Studio
 Il Visual Studio pacchetto Project modello è disponibile in tre percorsi di modello diversi nella finestra di **dialogo Project** nuovo modello:
...
1. Creare un nuovo VSPackage con il modello di Visual Studio pacchetto.
...
2. Immettere MyLanguagePackage come nome del progetto e fare clic su **OK.**
    ...
    È possibile usare il nome desiderato. Queste procedure descritte in dettaglio presuppongono MyLanguagePackage come nome.
...
6. Immettere **Inserisci frammento** di codice **come Nome comando** e per ID `cmdidInsertSnippet` **comando**. Fare clic su **Fine**.
...
1. In **Esplora soluzioni** fare clic con il pulsante destro del mouse sul progetto MyLanguagePackage, scegliere **Aggiungi** **,** Riferimento e quindi scegliere il pulsante **Aggiungi nuovo** riferimento.
...
2. Nella finestra **di dialogo** Aggiungi riferimento selezionare **Microsoft.VisualStudio.Package.LanguageService** nella scheda **.NET** e fare clic su **OK.**
     ...
     Questa operazione deve essere eseguita una sola volta per il progetto di pacchetto del linguaggio.
...
5. Immettere **MyLanguageService.cs** come nome del file di classe e fare clic su **Aggiungi**.
     ...
     È possibile usare il nome desiderato. Queste procedure descritte in dettaglio `MyLanguageService` presuppongono come nome.
...
## <a name="language-service-features"></a>Funzionalità del servizio di linguaggio
 Per implementare ogni funzionalità nel servizio di linguaggio, in genere si deriva una classe dalla classe del servizio di linguaggio MPF appropriata, si implementano i metodi astratti in base alle esigenze ed eseguire l'override dei metodi appropriati. Le classi create e/o derivate da dipendono dalle funzionalità che si intende supportare. Queste funzionalità sono descritte in dettaglio in [Funzionalità del servizio di linguaggio legacy](../../extensibility/internals/legacy-language-service-features1.md). La procedura seguente è l'approccio generale alla derivazione di una classe dalle classi MPF.
...
6. Scegliere **Implementa** classe astratta dal  menu Modifica , **IntelliSense** se la classe di base include metodi astratti che devono essere implementati.

7. In caso contrario, posizionare il cursore all'interno della classe e immettere il metodo da sottoporre a override.

     Ad esempio, digitare per visualizzare un elenco di tutti i metodi di cui è `public override` possibile eseguire l'override in tale classe.
