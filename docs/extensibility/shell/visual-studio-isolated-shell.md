...
> [!NOTE]
>  Per ulteriori informazioni su come distribuire un'applicazione basata su shell isolata, vedere [procedura dettagliata: creazione di un'applicazione Shell isolata di base](walkthrough-creating-a-basic-isolated-shell-application.md).  
...
-   È possibile utilizzare componenti VSPackage e Managed Extensibility Framework (MEF) per estendere un'applicazione shell isolata, esattamente come si farebbe in un'altra estensione di Visual Studio. Per ulteriori informazioni, vedere [estensione Shell isolata](extending-the-isolated-shell.md).  
...
-   Alcuni aspetti della shell possono anche essere specificati quando l'applicazione viene avviata. A tale scopo, aggiornare i parametri nella chiamata al punto di ingresso di avvio del appenvstub.dll.  
 ...
 Per ulteriori informazioni sui diversi elementi che è possibile personalizzare, vedere [gli elementi della Shell isolata](elements-of-the-isolated-shell.md).  
