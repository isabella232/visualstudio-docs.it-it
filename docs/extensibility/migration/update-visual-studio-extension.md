...
1. Nel nuovo progetto VSIX aggiungere un riferimento al progetto condiviso allo stesso progetto condiviso a cui fa riferimento il Visual Studio VSIX di destinazione 2019.
...
Sono state [apportate modifiche di rilievo all'API](breaking-api-list.md) in Visual Studio 2022 che potrebbero richiedere modifiche al codice da quando è stato eseguito nelle versioni precedenti. Esaminare il documento per suggerimenti su come aggiornare il codice per ognuno di essi.

Quando si adatta il codice, [](#use-conditional-compilation-symbols) è consigliabile usare la compilazione condizionale in modo che il codice possa continuare a supportare le Visual Studio 2022 durante l'aggiunta del supporto per Visual Studio 2022.
