...
Se si MSBuild attività, tenere presente che in Visual Studio 2022 è molto più probabile che verranno caricate in un processo di MSBuild.exe a 64 bit. Se l'attività richiede un processo a 32 bit per l'esecuzione, fare riferimento a questa documentazione di [MSBuild](../../msbuild/how-to-configure-targets-and-tasks.md#usingtask-attributes-and-task-parameters) per assicurarsi che MSBuild sappia caricare l'attività in un processo a 32 bit.

## <a name="modernize-your-vsix-project"></a>Modernizzare il progetto VSIX

Prima di Visual Studio supporto di 2022 all'estensione, è consigliabile prendere questo tempo per pulire e modernizzare il progetto esistente prima di procedere ulteriormente, tra cui:
...
1. Sostituire tutti i riferimenti all'assembly diretto di VS SDK con elementi PackageReference.
   ...
   Quando si passa dai riferimenti di assembly diretti NuGet ai riferimenti al pacchetto, è possibile che si prelevino altri riferimenti ad assembly e pacchetti analizzatori NuGet l'installazione automatica della chiusura transitiva delle dipendenze. Questo è in genere OK, ma potrebbe causare la segnalazione di altri avvisi durante la compilazione. Esaminare questi avvisi e risolvere il maggior numero possibile di avvisi e valutare la possibilità di eliminare gli avvisi che non è possibile risolvere usando aree nel `#pragma warning disable <id>` codice.
...
1. Nel nuovo progetto VSIX aggiungere un riferimento al progetto condiviso allo stesso progetto condiviso a cui fa riferimento l'Visual Studio VSIX con destinazione 2019.
...
1. Per le estensioni vs gestite, aggiornare i riferimenti al pacchetto dalla versione 16.x (o precedente) alle versioni del pacchetto 17.x nel file di progetto di destinazione di Visual Studio 2022 usando il NuGet Gestione pacchetti o modificando direttamente il file di progetto:
...
1. Per i progetti C++, devono essere compilati per amd64. Per le estensioni gestite, è consigliabile modificare il progetto dalla compilazione per Qualsiasi CPU alla destinazione , per riflettere che in Visual Studio 2022 l'estensione viene sempre caricata in un processo `x64` a 64 bit. `Any CPU` è anche valido, ma può generare avvisi se si fa riferimento a file binari nativi solo x64.
...
1. Modificare il `source.extension.vsixmanifest` file in modo da riflettere la destinazione Visual Studio 2022. Impostare il `<InstallationTarget>` tag in modo che rifletta Visual Studio 2022 e indicare un payload amd64:
...
Quando si adatta il codice, [](#use-conditional-compilation-symbols) è consigliabile usare la compilazione condizionale in modo che il codice possa continuare a supportare la pre-Visual Studio 2022 aggiungendo il supporto per Visual Studio 2022.
...
Non sarà possibile eseguire estensioni a 64 bit nelle versioni di Visual Studio precedenti a Visual Studio 2022 Preview.
...
La pubblicazione dell'estensione [nel Visual Studio Marketplace](https://marketplace.visualstudio.com/) è un ottimo modo per consentire ai nuovi utenti di trovare e installare l'estensione. Sia che l'estensione Visual Studio 2022 sia destinata esclusivamente alle versioni precedenti di Visual Studio, Marketplace è disponibile per supportare l'utente.

In futuro, Marketplace consentirà di caricare più VSIX in un'unica inserzione del Marketplace, consentendo di caricare il pacchetto VSIX di Visual Studio 2022 e un vsix di Visual Studio 2022. Gli utenti otterrà automaticamente il vsix giusto per la versione di Visual Studio installata, quando usano Gestione estensioni di Visual Studio.
