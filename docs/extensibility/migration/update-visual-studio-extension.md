...
Le *estensioni* devono essere compilate con la piattaforma "Qualsiasi CPU" o "x64". La piattaforma "x86" non è compatibile Visual Studio processo a 64 bit di 2022.
...
Poiché Visual Studio 2022 richiede un set distinto di assembly di riferimento da tutte le versioni precedenti di Visual Studio, le linee guida sono l'uso di progetti condivisi per usare facilmente più destinazione dell'estensione a pre-Visual Studio 2022 e Visual Studio 2022 (e versioni successive), offrendo la condivisione del codice, ma riferimenti distinti.
...
Continuare ad aggiungere Visual Studio 2022 all'estensione con questi passaggi, che possono essere completati Visual Studio 2019:
...
1. Nel nuovo progetto VSIX aggiungere un riferimento al progetto condiviso allo stesso progetto condiviso a cui fa riferimento il Visual Studio 2019 con destinazione VSIX.
...
1. Per le estensioni di Visual Studio gestite, aggiornare i riferimenti ai pacchetti dalla versione 16.x (o precedenti) alle versioni del pacchetto 17.x nel file di progetto di destinazione di Visual Studio 2022 usando il NuGet Gestione pacchetti o modificando direttamente il file di progetto:
...
1. Apportare eventuali altre modifiche necessarie in source.extension.vsixmanifest in modo che corrisponda a quella destinata a Visual Studio 2019 (se presente). È fondamentale che l'ID di VSIX, nell'elemento `Identity` del manifesto, sia identico per entrambe le estensioni.

A questo punto, è presente un'estensione Visual Studio 2022 di destinazione VSIX. È consigliabile compilare Visual Studio progetto VSIX di destinazione 2022 e usare tutte le [interruzioni di compilazione visualizzate.](#handle-breaking-api-changes) Se non sono presenti interruzioni di compilazione nel progetto VSIX Visual Studio 2022 di destinazione, si è pronti per il test.

## <a name="handle-breaking-api-changes"></a>Gestire le modifiche dell'API di rilievo

Sono presenti [modifiche dell'API](breaking-api-list.md) che Visual Studio 2022 che potrebbero richiedere modifiche al codice da quando è stata eseguita nelle versioni precedenti. Esaminare questo documento per suggerimenti su come aggiornare il codice per ognuno di essi.
...
Quando si ottiene l'Visual Studio di estensione di destinazione 2022, procedere al [test di](#test-your-extension).

## <a name="use-conditional-compilation-symbols"></a>Usare simboli di compilazione condizionale

Se si vuole usare lo stesso codice sorgente, anche lo stesso file, per Visual Studio 2022 e versioni precedenti, potrebbe essere necessario usare la compilazione condizionale in modo da poter creare un fork del codice per adattarsi alle modifiche di rilievo. La compilazione condizionale è una funzionalità dei linguaggi C#, Visual Basic e C++ che possono essere usati per condividere la maggior parte del codice, mantenendo al tempo stesso le API divergenti in posizioni specifiche.

Altre informazioni sull'uso delle direttive del preprocessore e dei simboli di compilazione condizionale sono disponibili nella documentazione Microsoft #if [direttiva del preprocessore](/dotnet/csharp/language-reference/preprocessor-directives#conditional-compilation).
...
Tuttavia, in alcuni casi, un riferimento a un tipo presente in assembly distinti tra Visual Studio 2022 e versioni precedenti potrebbe essere ancora rappresentabile in un file XAML rimuovendo lo spazio dei nomi che fa riferimento all'assembly:
...
La pubblicazione dell'estensione [Visual Studio Marketplace](https://marketplace.visualstudio.com/) è un ottimo modo per consentire ai nuovi utenti di trovare e installare l'estensione. Sia che l'estensione Visual Studio 2022 sia destinata esclusivamente a versioni precedenti di Visual Studio, Marketplace è disponibile per supportare l'utente.

In futuro, Marketplace consentirà di caricare più VSIX in un'unica inserzione del Marketplace, consentendo di caricare il pacchetto VSIX di destinazione Visual Studio 2022 e un pacchetto VSIX pre-Visual Studio 2022. Gli utenti otterrà automaticamente il vsix giusto per la versione di Visual Studio installata, quando usano Gestione estensioni Di Visual Studio.

Per le versioni di anteprima Visual Studio 2022, marketplace supporterà solo un singolo file VSIX per ogni inserzione del Marketplace. Mentre Visual Studio 2022 è in anteprima, è necessario avere una presentazione separata Visual Studio Marketplace 2022 per l'estensione. In questo modo è possibile scorrere l'estensione Visual Studio 2022 in base alle esigenze senza influire sui clienti sulle versioni precedenti di Visual Studio. È anche possibile contrassegnare l'estensione come "anteprima" per impostare l'aspettativa che sarà probabilmente meno affidabile, anche se l'origine di tale inaffiabilità è Visual Studio 2022, rispetto all'estensione mainstream.
...
È possibile condividere l'estensione su una LAN o in qualsiasi altro modo. Se si ha come destinazione Visual Studio 2022 e pre-Visual Studio 2022, è necessario condividere più VSIX singolarmente e assegnare loro nomi file (o posizionarli in cartelle univoche) che consentono agli utenti di sapere quale VSIX installare in base alla versione di Visual Studio installata.

### <a name="other-considerations"></a>Altre considerazioni

#### <a name="dependencies"></a>Dependencies

Se VSIX specifica un altro VSIX come dipendenza tramite l'elemento , ogni vsix a cui si fa riferimento deve essere installato nelle stesse destinazioni e nelle stesse architetture di prodotto di `<dependency>` VSIX. Se un vsix dipendente non supporta l'installazione di destinazione di Visual Studio, vsix avrà esito negativo. È possibile che vsix dipendente supporti più destinazioni e architetture rispetto alla propria, ma non meno. Questa restrizione significa che l'approccio di distribuzione e distribuzione di un VSIX con dipendenze deve rispecchiare quello dei dipendenti.
...
**A**: Sì!  Per [altre informazioni, vedere Estensioni senza](#extensions-without-running-code) eseguire codice.
...
**D:** I comandi e i tasti di scelta rapida non funzionano Visual Studio dopo il passaggio dei file di origine a un progetto condiviso.
...
Seguire un esempio dettagliato, [ImageOptimizer](samples.md), con collegamenti al progetto e alle modifiche al codice per ogni passaggio.
