...
description: Informazioni sui problemi relativi alla programmazione per schermate ad alta risoluzione, ad esempio l'aumento del contenuto, i problemi di layout e l'uso delle API di ridimensionamento DPI.
...
ms.locfileid: "126709825"
---
# <a name="address-dpi-issues"></a>Risolvere i problemi di DPI
Un numero crescente di dispositivi viene spedito con schermate "ad alta risoluzione". Queste schermate hanno in genere più di 200 pixel per pollice (ppi). L'uso di un'applicazione in questi computer richiederà un ridimensionamento del contenuto per soddisfare le esigenze di visualizzazione del contenuto a una distanza di visualizzazione normale per il dispositivo. A inizio 2014, l'obiettivo principale per gli schermi ad alta densità sono i dispositivi di mobile computing (tablet, pc portatili a conchiglia e telefoni).

Windows 8.1 e versioni successive contengono diverse funzionalità che consentono a questi computer di lavorare con schermi e ambienti in cui la macchina è collegata contemporaneamente a schermi ad alta densità e a densità standard.
...
- Windows 8.1 e versioni successive ridimensionano automaticamente il contenuto per la maggior parte delle applicazioni in modo che sia coerente quando vengono spostate tra le diverse densità di pixel. Quando la visualizzazione primaria è ad alta densità (ridimensionamento del 200%) e la visualizzazione secondaria è a densità standard (100%), Windows ridimensiona automaticamente il contenuto della finestra dell'applicazione verso il basso sullo schermo secondario (1 pixel visualizzato ogni 4 pixel sottoposto a rendering dall'applicazione).

- Windows ridimensionamento predefinito per la densità dei pixel e la distanza di visualizzazione per lo schermo (Windows 7 e versioni successive, configurabile dall'OEM).

- Windows ridimensionare automaticamente il contenuto fino al 250% nei nuovi dispositivi che superano i 280 ppi (a Windows 8.1 S14).

  Windows un modo per gestire la scalabilità verticale dell'interfaccia utente per sfruttare il numero di pixel aumentato. Un'applicazione acconsente esplicitamente a questo sistema dichiarando se stessa "in grado di riconoscere DPI di sistema". Le applicazioni che non esitono questa operazione vengono ridimensionate dal sistema. Ciò può comportare un'esperienza utente "fuzzy" in cui l'intera applicazione viene estesa in modo uniforme in pixel. Ad esempio:

  ![Problemi DPI - sfocatura](../extensibility/media/dpi-issues-fuzzy.png "Problemi DPI - sfocatura")

  Visual Studio acconsente esplicitamente al ridimensionamento DPI e pertanto non è "virtualizzato".

  Windows (e Visual Studio) sfruttano diverse tecnologie dell'interfaccia utente, che hanno modi diversi di gestire i fattori di scalabilità impostati dal sistema. Ad esempio:

- WPF misura i controlli in modo indipendente dal dispositivo (unità, non pixel). L'interfaccia utente WPF viene automaticamente ridimensionata per il valore DPI corrente.

- Tutte le dimensioni del testo indipendentemente dal framework dell'interfaccia utente sono espresse in punti e pertanto vengono considerate dal sistema indipendenti da DPI. Il testo in Win32, WinForms e WPF è già ridimensionato correttamente quando viene disegnato sul dispositivo di visualizzazione.

- Le finestre e le finestre di dialogo Win32/WinForms consentono di abilitare il layout che viene ridimensionato con il testo, ad esempio tramite pannelli di layout di griglia, di flusso e di tabella. Questi consentono di evitare posizioni di pixel hard-coded che non vengono ridimensionate quando le dimensioni del carattere vengono aumentate.
...
Sebbene WPF sia già a elevato livello di riconoscimento DPI, gran parte del codice basato su Win32/GDI non è stato originariamente scritto tenendo presente la consapevolezza DPI. Windows ha fornito API di ridimensionamento DPI. Le correzioni ai problemi Win32 devono essere usate in modo coerente in tutto il prodotto. Visual Studio ha fornito una libreria di classi helper per evitare la duplicazione delle funzionalità e garantire la coerenza nel prodotto.

## <a name="high-resolution-images"></a>Immagini ad alta risoluzione
Questa sezione è destinata principalmente agli sviluppatori che estendono Visual Studio 2013. Per Visual Studio 2015, usare il servizio immagini integrato in Visual Studio. È anche possibile che sia necessario supportare o impostare come destinazione molte versioni di Visual Studio e pertanto l'uso del servizio immagini nel 2015 non è un'opzione perché non esiste nelle versioni precedenti. Questa sezione è anche per l'utente.

## <a name="scaling-up-images-that-are-too-small"></a>Aumento delle dimensioni delle immagini troppo piccole
Le immagini troppo piccole possono essere ridimensionate e sottoposte a rendering in GDI e WPF usando alcuni metodi comuni. Le classi helper DPI gestite sono disponibili per gli integratori di Visual Studio interni ed esterni per gestire icone di ridimensionamento, bitmap, imagestrip ed elenchi di immagini. Gli helper C/C++ nativi basati su Win32 sono disponibili per il ridimensionamento di HICON, HBITMAP, HIMAGELIST e VsUI::GdiplusImage. Il ridimensionamento di una bitmap richiede in genere solo una modifica di una riga dopo aver incluso un riferimento alla libreria helper. Ad esempio:
...
Il ridimensionamento di un elenco immagini dipende dal fatto che l'elenco immagini sia completo in fase di caricamento o che sia aggiunto in fase di esecuzione. Se completato in fase di caricamento, chiamare `LogicalToDeviceUnits()` con l'elenco immagini come si farebbe con una bitmap. Quando il codice deve caricare una singola bitmap prima di comporre l'elenco immagini, assicurarsi di ridimensionare le dimensioni dell'immagine dell'elenco immagini:
...
Le funzioni nella libreria consentono di specificare l'algoritmo di ridimensionamento. Quando si ridimensionano le immagini da inserire negli elenchi immagini, assicurarsi di specificare il colore di sfondo usato per la trasparenza oppure usare il ridimensionamento NearestNeighbor (che causerà distorsioni al 125% e al 150%).

Consultare la <xref:Microsoft.VisualStudio.PlatformUI.DpiHelper> documentazione su MSDN.

La tabella seguente mostra esempi di come ridimensionare le immagini in base ai fattori di ridimensionamento DPI corrispondenti. Le immagini descritte in arancione indicano la procedura consigliata Visual Studio 2013 (ridimensionamento DPI dal 100%-200%):
...
I problemi di layout comuni possono essere evitati principalmente mantenendo i punti nell'interfaccia utente ridimensionati e relativi l'uno rispetto all'altro anziché usando posizioni assolute (in particolare, in unità pixel). Ad esempio:

- Le posizioni del layout o del testo devono essere modificate per prendere in considerazione le immagini con scalabilità verticale.

- Le colonne nelle griglie devono avere larghezze regolate per il testo con scalabilità verticale.

- Sarà anche necessario aumentare le dimensioni hard-coded o lo spazio tra gli elementi. Le dimensioni basate solo sulle dimensioni del testo sono in genere erre, perché i tipi di carattere vengono ridimensionati automaticamente.
...
La Visual Studio helper DPI è disponibile in moduli nativi e gestiti e può essere usata all'esterno della shell Visual Studio da altre applicazioni.
...
> [!NOTE]
> Non usare le funzioni helper nelle variabili statiche a livello di modulo o di classe. La libreria usa anche statici per la sincronizzazione dei thread ed è possibile che si verificano problemi di inizializzazione dell'ordine. Convertire tali variabili statiche in variabili membro non statiche o eseguire il wrapping in una funzione (in modo che siano costruite al primo accesso).
...
- Assicurarsi che il progetto abbia riferimenti **a System.Windows. Form,** **PresentationCore** e **PresentationUI**.
...
In WPF le bitmap vengono ridimensionate automaticamente da WPF per il livello di zoom DPI corrente usando un algoritmo bicubico di alta qualità (impostazione predefinita), che funziona bene per immagini o screenshot di grandi dimensioni, ma non è appropriato per le icone delle voci di menu perché introduce un'incoerenza percepita.

Consigli:

- Per la grafica dell'immagine del logo e dei banner, è possibile usare la modalità <xref:System.Windows.Media.BitmapScalingMode> di ridimensionamento predefinita.
...
- Per i livelli di zoom di grandi dimensioni non multipli del 100% (ad esempio, 250% o 350%), ridimensionare le immagini iconografiche con risultati bicubici in un'interfaccia utente fuzzy e lavata. Un risultato migliore viene ottenuto ridimensionando prima l'immagine con NearestNeighbor al multiplo più grande del 100% (ad esempio, 200% o 300%) e ridimensionando con bicubico da qui. Per altre informazioni, vedere Caso speciale: prescalamento delle immagini WPF per livelli DPI di grandi dimensioni.

  La classe DpiHelper nello spazio dei nomi Microsoft.VisualStudio.PlatformUI fornisce un membro che <xref:System.Windows.Media.BitmapScalingMode> può essere usato per l'associazione. Consentirà alla shell Visual Studio di controllare in modo uniforme la modalità di ridimensionamento delle bitmap nel prodotto, a seconda del fattore di scala DPI.
...
Eseguire il ridimensionamento preliminare dell'immagine usando un convertitore applicato a un'associazione o con un'estensione di markup XAML. Ad esempio:
...
- Se le dimensioni dell'immagine originale non sono note, è possibile usare LayoutTransform per ridimensionare l'oggetto Image finale. Ad esempio:
