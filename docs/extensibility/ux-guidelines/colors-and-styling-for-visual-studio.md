...
Agli utenti viene richiesto di selezionare un tema durante il primo uso di Visual Studio e possono cambiare i temi in un secondo momento passando a **strumenti &gt; opzioni &gt; ambiente &gt; generale** e scegliendo un nuovo tema dal menu a discesa "tema colori".
...
Il servizio colori ambiente legge VSColors definito nel pkgdef per il componente dell'interfaccia utente. A questi VSColors viene quindi fatto riferimento nel markup o nel codice XAML e vengono caricati tramite il `IVsUIShell5.GetThemedColor` o un mapping di `DynamicResource`.
...
La shell fornisce un servizio che consente di accedere ai `COLORREF` dei colori. Il servizio/interfaccia è:

```
IVsUIShell2::GetVSSysColorEx(VSSYSCOLOR dwSysColIndex, DWORD *pdwRGBval)
```

Nel file VSShell80. idl, l'enumerazione `__VSSYSCOLOREX` contiene costanti colore della shell. Per usarlo, passare come valore di indice uno dei valori del `enum __VSSYSCOLOREX` documentato in MSDN o un numero di indice regolare accettato dall'API di sistema di Windows, `GetSysColor`. Questa operazione ottiene il valore RGB del colore da usare nel secondo parametro.
...
È possibile eseguire il binding ai colori di Visual Studio tramite i valori esportati nel `ResourceDictionary`dell'applicazione. Di seguito è riportato un esempio dell'uso delle risorse dalla tabella dei colori, nonché dell'associazione ai dati del tipo di carattere dell'ambiente in XAML.
...
I metodi helper della classe `Microsoft.VisualStudio.Shell.VsColors` in MPF includono `GetThemedGDIColor()` e `GetThemedWPFColor()`. Questi metodi helper restituiscono il valore del colore di una voce di tema come `System.Drawing.Color` o `System.Windows.Media.Color`, da utilizzare nell'interfaccia utente di Windows Form o WPF.
...
![Confronto tra Editor Contrasto elevato](../../extensibility/ux-guidelines/media/030303-b_hceditorcomparison.png "030303-b_HCEditorComparison")<br />Confronto di editor a contrasto elevato
...
Costruire un tipo speciale di voce del registro di sistema Category in `[HKLM\SOFTWARE\Microsoft \Visual Studio\\<Visual Studio version\>\FontAndColors\\<Category\>]` in cui `<Category>` è il nome non localizzato della categoria.
...
Costruire un tipo speciale di voce del registro di sistema Category in `[HKLM\SOFTWARE\Microsoft \Visual Studio\\<Visual Studio version\>\FontAndColors\\<group\>]` in cui `<group>` è il nome non localizzato del gruppo.
...
L'IDE utilizza l'interfaccia <xref:Microsoft.VisualStudio.Shell.Interop.IVsFontAndColorGroup> per definire un'Unione di diverse categorie.
...
- `"NameID"` = ID di risorsa del nome della categoria localizzata nel pacchetto
