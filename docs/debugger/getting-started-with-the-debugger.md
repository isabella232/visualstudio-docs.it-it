...
Sebbene l'app demo sia un'app C# e C++, le funzionalità sono applicabili a Visual Basic, JavaScript e ad altri linguaggi supportati da Visual Studio (se non diversamente specificato). Gli screenshot sono in linguaggio C#.
...
* È necessario aver installato Visual Studio 2017 e avere il carico di lavoro **Sviluppo per desktop .NET** o **Sviluppo di applicazioni desktop con C++**.
...
2. In **Visual C#** o **Visual C++** scegliere **Windows Desktop**, quindi nel riquadro centrale scegliere **App console** (**Applicazione console di Windows** in C++).
...
3. Digitare un nome come **get-started-debugging** e fare clic su **OK**.
    ...
    > [!NOTE]
    > Per passare dal codice di esempio C# al codice di esempio C++ e viceversa in questo articolo, usare il filtro del linguaggio nella parte superiore destra della pagina.

4. In *Program.cs* (C#) o *get-started-debugging.cpp* (C++) sostituire il codice seguente
...
1. Nel ciclo `foreach` della funzione `Main` (ciclo `for` nella funzione `main` di C++) impostare un punto di interruzione facendo clic sul margine sinistro della riga di codice seguente:

    `shape.Draw()` (oppure `shape->Draw()` in C++)
...
1. Mentre l'esecuzione è in pausa nella chiamata al metodo `shape.Draw` nel metodo `Main` (`shape->Draw` in C++), premere **F11** (oppure scegliere **Debug > Esegui istruzione**) per avanzare nel codice per la classe `Rectangle`.

     ![Usare F11 per eseguire l'istruzione nel codice](../debugger/media/get-started-f11.png "F11 Esegui istruzione")
...
2. Premere **F10** (oppure scegliere **Debug > Esegui istruzione/routine**) alcune volte fino a quando il debugger non si arresta nella chiamata al metodo `base.Draw` (`Shape::Draw` in C++) e quindi premere **F10** ancora una volta.

     ![Usare F10 per eseguire l'istruzione/routine nel codice](../debugger/media/get-started-step-over.png "F10 Esegui istruzione/routine")

     Si noti che questa volta il debugger non esegue l'istruzione nel metodo `Draw` della classe di base (`Shape`). **F10** fa avanzare il debugger senza eseguire le istruzioni nelle funzioni o nei metodi del codice dell'app (il codice rimane in esecuzione). Premendo F10 nella chiamata al metodo `base.Draw` (o `Shape::Draw`) anziché **F11**, è stato ignorato il codice di implementazione per `base.Draw` (non d'interesse ai fini dell'esercitazione) .
...
6. Fare clic sul pulsante per l'**esecuzione fino alla riga selezionata dall'utente** ![Esecuzione fino alla riga selezionata dall'utente](../debugger/media/dbg-tour-run-to-click.png "RunToClick").
    ...
    Il debugger avanza fino all'implementazione del metodo `Console.WriteLine` per la classe `Triangle` (`std::cout` in C++).
...
1. Premere **MAIUSC** + **F11** (oppure **Debug > Esci da istruzione/routine**).
     ...
     L'esecuzione dovrebbe tornare al ciclo `foreach` nel metodo `Main` (ciclo `for` in C++).
...
Il debugger si arresta nuovamente nel punto di interruzione impostato, nel metodo `shape.Draw()` (`shape->Draw()` in C++).
...
1. Mentre l'esecuzione è in pausa nel metodo `shape.Draw()` (`shape->Draw()` in C++), passare il mouse sull'oggetto `shapes` per visualizzare il valore della proprietà predefinita, la proprietà `Count`.

1. Espandere l'oggetto `shapes` per visualizzarne tutte le proprietà, ad esempio il primo indice della matrice `[0]` che ha il valore `Rectangle` (C#) o un indirizzo di memoria (C++).

     ![Visualizzare un suggerimento dati](../debugger/media/get-started-data-tip.gif "Visualizzare un suggerimento dati")
...
1. Osservare la finestra **Auto** nella parte inferiore dell'editor di codice.
    ...
    Nella finestra **Auto** vengono visualizzate le variabili e i relativi valori correnti. La finestra **Auto** mostra tutte le variabili usate nella riga corrente o nella riga precedente (in C++ la finestra mostra le variabili nelle tre righe di codice precedenti. Vedere la documentazione per il comportamento specifico del linguaggio).
...
2. Osservare quindi la finestra **Variabili locali** in una scheda accanto alla finestra **Auto**.
    ...
    La finestra **Variabili locali** mostra le variabili presenti nell'[ambito](https://www.wikipedia.org/wiki/Scope_(computer_science)) corrente.
...
1. Mentre l'esecuzione è in pausa nel ciclo `foreach` (ciclo `for` in C++) fare clic sulla finestra **Stack di chiamate**, visualizzata per impostazione predefinita nel riquadro inferiore destro.

2. Fare clic su **F11** alcune volte fino a quando il debugger non viene messo in pausa nel metodo `Circle.Draw` nell'editor del codice. Osservare la finestra **Stack di chiamate**.

    ![Esaminare lo stack di chiamate](../debugger/media/get-started-call-stack.png "ExamineCallStack")

    La finestra **Stack di chiamate** mostra l'ordine in cui vengono chiamati i metodi e le funzioni. La prima riga visualizza la funzione corrente (il metodo `Circle.Draw` o `Circle::Draw` in questa app). La seconda riga indica che `Circle.Draw` è stata chiamata dal metodo `Main` (`main` in C++) e così via.
...
1. Mentre il debugger è in pausa nella chiamata al metodo `Circle.Draw`, premere **F11** alcune volte fino a quando il debugger non viene messo in pausa nella chiamata al metodo `base.Draw` (`Shape::Draw` in C++).

1. Usare il mouse per selezionare la freccia gialla (il puntatore di esecuzione) a sinistra e spostare la freccia gialla in alto di una riga alla chiamata al metodo `Console.WriteLine` (`std::cout` in C++).

1. Premere **F11** ancora una volta.

    Il debugger esegue nuovamente il metodo `Console.WriteLine` (`std::cout` in C++).
