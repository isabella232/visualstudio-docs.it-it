...
> [!IMPORTANT]
> Se si ritiene di aver ricevuto questo messaggio a causa di un bug del prodotto, [segnalare il problema](../ide/how-to-report-a-problem-with-visual-studio.md) a Visual Studio. Per ottenere ulteriore assistenza, vedere [Talk to Us](../ide/feedback-options.md) per le modalità di contatto di Microsoft.

## <a name="what-is-the-detailed-error-message"></a><a name="specificerrors"></a>Qual è il messaggio di errore dettagliato?

Il `Unable to Connect to the Microsoft Visual Studio Remote Debugging Monitor` messaggio è generico. Di solito, un messaggio più specifico è incluso nella stringa di errore e che può aiutare a identificare la causa del problema o cercare una correzione più esatta. Ecco alcuni dei messaggi di errore più comuni che vengono aggiunti al messaggio di errore principale:
...
- [Il debugger remoto di Visual Studio nel computer di destinazione non è in grado di connettersi nuovamente a questo computer](#cant_connect_back)
...
1. Assicurarsi di immettere un nome computer e un numero di porta validi nella finestra di dialogo **Connetti a processo** o nelle proprietà del progetto (Per impostare le proprietà, vedere questi [passaggi](#server_incorrect)). Il nome del computer deve essere il seguente formato:
...
3. Assicurarsi che la versione del debugger remoto in esecuzione nel computer di destinazione corrisponda alla versione di Visual Studio. Per ottenere la versione corretta del debugger remoto, vedere [Debug remoto](../debugger/remote-debugging.md).
    ...
    > [!TIP]
    > Se ci si connette al processo e ci si connette correttamente ma il processo desiderato non è visualizzato, selezionare la casella di controllo **Mostra processi di tutti gli utenti**. Questo mostrerà i processi se si è connessi con un account utente diverso.

4. Se questa procedura non consente di risolvere l'errore, vedere [Il computer remoto non è raggiungibile.](#dns)
...
## <a name="there-is-no-server-by-the-specified-name-running-on-the-remote-computer"></a><a name="no_server"></a>Non esiste alcun server con il nome specificato in esecuzione nel computer remoto

Impossibile connettersi al debugger remoto. Questo messaggio può verificarsi per diversi motivi:

- Il debugger remoto potrebbe essere in esecuzione con un account utente diverso. Vedere [questi passaggi](#user_accounts)

- La porta è bloccata sul firewall. Assicurati che il firewall [non blocchi la tua richiesta,](#firewall)soprattutto se utilizzi un firewall di terze parti.

- La versione del debugger remoto non corrisponde a Visual Studio.The remote debugger version does not match Visual Studio. Per ottenere la versione corretta del debugger remoto, vedere [Debug remoto](../debugger/remote-debugging.md).
...
- Il debugger remoto potrebbe essere in esecuzione con un account utente diverso. Attenersi [alla seguente procedura](#user_accounts).

- La porta è bloccata sul firewall. Assicurati che il firewall [non blocchi la tua richiesta,](#firewall)soprattutto se utilizzi un firewall di terze parti.

- La versione del debugger remoto non corrisponde a Visual Studio.The remote debugger version does not match Visual Studio. Per ottenere la versione corretta del debugger remoto, vedere [Debug remoto](../debugger/remote-debugging.md).

## <a name="the-visual-studio-remote-debugger-on-the-target-computer-cannot-connect-back-to-this-computer"></a><a name="cant_connect_back"></a>Il debugger remoto di Visual Studio nel computer di destinazione non è in grado di connettersi nuovamente a questo computer

Il debugger remoto potrebbe essere in esecuzione con un account utente diverso. Nel debugger remoto aprire **Strumenti > autorizzazioni** per aggiungere l'utente alle autorizzazioni del debugger remoto. Per ulteriori informazioni, vedere [Il debugger remoto è in esecuzione con un account utente diverso.](#user_accounts)
...
Questo potrebbe essere un problema legacy specifico di Windows XP e Windows 7. Vedere queste [informazioni](https://stackoverflow.com/questions/4786016/unable-to-connect-to-the-microsoft-remote-debugging-monitor-a-security-package).
...
Se non è possibile connettersi utilizzando il nome del computer remoto, provare a utilizzare l'indirizzo IP. È possibile `ipconfig` utilizzare in una riga di comando sul computer remoto per ottenere l'indirizzo IPv4. Se si utilizza un file HOSTS, verificare che sia configurato correttamente.

Se si verifica un errore, verificare che il computer remoto sia accessibile in rete[(eseguire](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/ee624059(v=ws.10)) il ping del computer remoto). Il debug remoto su Internet non è supportato, tranne in alcuni scenari di Microsoft Azure.Remote debugging over the Internet is not supported, except in some Microsoft Azure scenarios.
...
In Visual Studio esaminare le proprietà del progetto e assicurarsi che il nome del server sia corretto. Vedere gli argomenti relativi [a C' e Visual Basic](../debugger/remote-debugging-csharp.md#remote_csharp) e C [.](../debugger/remote-debugging-cpp.md#remote_cplusplus) Ad ASP.NET, aprire **Proprietà / Web / Server** o Proprietà / **Debug** a seconda del tipo di progetto.

> [!NOTE]
> Se si esegue la connessione al processo, le impostazioni remote nelle proprietà del progetto non vengono utilizzate.

Se il nome del server è corretto, è possibile che il software antivirus o un firewall di terze parti blocchi il debugger remoto. Quando si esegue il debug in locale, ciò può verificarsi perché Visual Studio è un'applicazione a 32 bit, pertanto utilizza la versione a 64 bit del debugger remoto per eseguire il debug di applicazioni a 64 bit. I processi a 32 bit e a 64 bit comunicano utilizzando la rete locale all'interno del computer locale. Il traffico di rete resta all'interno del computer, ma è possibile che un software per la sicurezza di terze parti blocchi la comunicazione.
...
Per impostazione predefinita, il debugger remoto accetta solo le connessioni dall'utente che ha avviato il debugger remoto e dai membri del gruppo Administrators. Agli utenti aggiuntivi devono essere concesse esplicitamente le autorizzazioni.
...
- Nel computer remoto riavviare il debugger remoto con lo stesso account utente e la stessa password in uso nel computer Visual Studio.

    > [!NOTE]
    > Se si esegue il debugger remoto su un server remoto, fare clic con il pulsante destro del mouse sull'app Debugger remoto e scegliere **Esegui come amministratore** (oppure è possibile eseguire il debugger remoto come servizio). Se non si esegue su un server remoto, è sufficiente avviarlo normalmente.

- Avviare il debugger remoto dalla riga di comando con il parametro **/allow \<nomeutente>**: `msvsmon /allow <username@computer>`.

- In alternativa, è possibile consentire a qualsiasi utente di eseguire il debug remoto. Nella finestra del debugger remoto passare alla finestra di dialogo **Strumenti > Opzioni**. Se si seleziona   **Nessuna autenticazione**, è possibile selezionare **Consenti debug da parte di qualsiasi utente**. Tuttavia, è consigliabile provare questa opzione solo se le altre opzioni hanno esito negativo o se si utilizza una rete privata.

### <a name="the-firewall-on-the-remote-machine-doesnt-allow-incoming-connections-to-the-remote-debugger"></a><a name="firewall"></a>Il firewall sul computer remoto non consente connessioni in ingresso al debugger remoto
...
### <a name="network-security-policy-is-blocking-communication-between-the-remote-machine-and-visual-studio"></a>I criteri di sicurezza di rete bloccano la comunicazione tra il computer remoto e Visual Studio
 Esaminare la sicurezza della rete per assicurarsi che non blocchi la comunicazione. Per ulteriori informazioni sui criteri di sicurezza di rete di Windows, vedere [Impostazioni dei criteri](/windows/device-security/security-policy-settings/security-policy-settings)di protezione .
...
## <a name="more-help"></a>Altre informazioni
 Per ottenere altre informazioni sul debugger remoto, aprire la pagina della Guida del debugger remoto **(Guida > utilizzo** nel debugger remoto).
