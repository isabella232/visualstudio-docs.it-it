...
## <a name="inspect-strings-in-a-visualizer"></a><a name="string_visualizer"></a>Controllare le stringhe in un visualizzatore
...
Il debugger interrompe il codice in corrispondenza di eccezioni non gestite. Tuttavia, le eccezioni gestite, ad esempio le eccezioni che si verificano all'interno di un `try/catch` blocco, possono anche essere un'origine di bug e potrebbe essere necessario analizzare quando si verificano. È possibile configurare il debugger per l'interruzione nel codice anche per le eccezioni gestite configurando le opzioni nella finestra di dialogo **Impostazioni eccezioni** . Aprire questa finestra di dialogo scegliendo **Debug > Windows > impostazioni eccezioni**.

La finestra di dialogo **Impostazioni eccezioni** consente di indicare al debugger di suddividere il codice in base a eccezioni specifiche. Nell'illustrazione seguente il debugger si interrompe nel codice ogni volta che `System.NullReferenceException` si verifica un errore. Per ulteriori informazioni, vedere [gestione delle eccezioni](../debugger/managing-exceptions-with-the-debugger.md).
...
## <a name="get-more-familiar-with-how-the-debugger-attaches-to-your-app-c-c-visual-basic-f"></a><a name="modules_window"></a> Acquisire familiarità con le modalità di connessione del debugger all'app (C#, C++, Visual Basic, F #)
