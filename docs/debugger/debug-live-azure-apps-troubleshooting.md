...
* Assicurarsi che l'account Visual Studio di personalizzazione abbia le autorizzazioni per la sottoscrizione e la risorsa di Azure a cui si è collegati. Un modo rapido per determinare questo problema è controllare se la risorsa è disponibile nella finestra di dialogo da Debug  >  **Snapshot Debugger...**  >  **Risorsa di Azure**  >  **Selezionare Esistente** o in Cloud Explorer.
...
La prima route protegge in modo efficace il dominio dell'app **simile a Accedi con [IdentityProvider]**. La seconda route espone l'endpoint SnapshotDebugger AgentLaunch all'esterno dell'autenticazione, che esegue  l'azione predefinita di avvio dell'agente di diagnostica SnapshotDebugger solo se l'estensione del sito preinstallata SnapshotDebugger è abilitata per il servizio app. Per altri dettagli sulla configurazione di authorization.json, vedere Regole [di autorizzazione url](https://azure.microsoft.com/updates/url-authorization-rules-in-app-service/).
...
Si tratta di un problema noto che si verifica quando un utente tenta di collegare Snapshot Debugger a un servizio app che ha abilitato ApplicationInsights. ApplicationInsights imposta AppSettings con maiuscole e minuscole diverse da quelle Visual Studio, causando questo problema.
...
* Verificare che portale di Azure appSettings per SnapshotDebugger (SNAPSHOTDEBUGGER_EXTENSION_VERSION) e InstrumentationEngine (INSTRUMENTATIONENGINE_EXTENSION_VERSION) siano in maiuscolo. In caso contrario, aggiornare le impostazioni manualmente, forzando il riavvio del sito.
::: moniker-end
* Se l'errore persiste, usare uno dei canali di feedback descritti all'inizio di questo articolo.

### <a name="500-internal-server-error"></a>(500) Errore interno del server

Questo errore indica che il sito è completamente in attività o che il server non è in grado di gestire la richiesta. Snapshot Debugger funzioni solo nelle applicazioni in esecuzione. [L Insights Snapshot Debugger appliazione](/azure/azure-monitor/app/snapshot-debugger) fornisce snapshot sulle eccezioni e può essere lo strumento migliore per le proprie esigenze.
...
Se viene visualizzata un'icona di avviso ![Icona](../debugger/media/snapshot-troubleshooting-snappoint-warning-icon.png "Icona di avviso del punto di agganciamento") di avviso Punto di agganciamento con il punto di agganciamento anziché l'icona del punto di agganciamento normale, il punto di agganciamento non è attivato.

![Snappoint non si attiva](../debugger/media/snapshot-troubleshooting-dont-turn-on.png "Il punto di agganciato non si attiva")
...
![Punto di snap limitato](../debugger/media/snapshot-troubleshooting-throttled-snapshots.png "Punto di agganciato limitato")
...
Viceversa, se si usa Visual Studio 2017 per collegare il Snapshot Debugger a un Servizio app di Azure di cui è stato precedentemente fatto il debug dal Snapshot Debugger in Visual Studio 2019, verrà visualizzato l'errore seguente:
