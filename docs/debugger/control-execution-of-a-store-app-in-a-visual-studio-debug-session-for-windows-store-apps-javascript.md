...
3.  **Sostituire il contenuto della pagina con il codice di esempio.** Eliminare tutto il contenuto dal file `default.js` . Fare clic sul collegamento: [il codice di esempio di navigazione (JavaScript) Debugger](https://github.com/MicrosoftDocs/visualstudio-docs/raw/master/docs/debugger/samples/debugger-navigation-sample-code-javascript.js), quindi copiare il codice riportato nella sezione JavaScript negli Appunti. (Scegliere **nuovamente** nel browser o Visualizzatore della Guida per tornare a questa pagina introduttiva.) Nell'editor di Visual Studio incolla il codice nel file `default.js` vuoto. Scegliere **CTRL+S** per salvare il file.  
