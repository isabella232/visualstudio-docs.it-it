...
## <a name="BKMK_symbols_release"></a>Genera i file di simboli (con estensione pdb) perC#una C++compilazione (, F#, Visual Basic,)
...
> [!IMPORTANT]
> Il debugger caricherà solo un file con estensione pdb per un file eseguibile che corrisponde esattamente al file pdb creato alla compilazione del file eseguibile (il file pdb deve essere l'originale o una copia del file pdb originale). Per ulteriori informazioni, vedere [motivi per cui Visual Studio richiede che i file di simboli del debugger corrispondano esattamente ai file binari con cui sono stati](https://blogs.msdn.microsoft.com/jimgries/2007/07/06/why-does-visual-studio-require-debugger-symbol-files-to-exactly-match-the-binary-files-that-they-were-built-with/)compilati.

Per ogni tipo di progetto è possibile impostare queste opzioni in modo diverso.

### <a name="generate-symbol-files-for-a-c-aspnet-or-visual-basic-project"></a>Genera i file di simboli C#per un progetto, ASP.NET o Visual Basic

Per informazioni dettagliate sulle impostazioni di progetto per le configurazioni C# di debug in o Visual Basic, vedere [impostazioni C# di progetto per una configurazione di debug](../debugger/project-settings-for-csharp-debug-configurations.md) o [impostazioni di progetto per una configurazione di](../debugger/project-settings-for-a-visual-basic-debug-configuration.md)debug di Visual Basic.
...
6. Nell'elenco **informazioni di debug** (o nell'elenco **genera informazioni di debug** in Visual Basic) scegliere **completo**, **solo PDB**o portabile.
   ...
   Il formato portatile è il formato multipiattaforma più recente per .NET Core. Per ulteriori informazioni sulle opzioni, vedere [finestra di dialogo Impostazioni di compilazioneC#avanzate ()](../ide/reference/advanced-build-settings-dialog-box-csharp.md).

   ![Genera PDB per C# ] le compilazioni in (../debugger/media/dbg_project_properties_pdb_csharp.png "GeneratePDBsForCSharp")
...
### <a name="generate-symbol-files-for-a-c-project"></a>Genera i file di simboli C++ per un progetto
...
4. Nel riquadro laterale scegliere linker **> debug**, quindi selezionare le opzioni per **genera informazioni di debug**.
   ...
   Per informazioni dettagliate sulle impostazioni di progetto per le configurazioni C++di debug in, vedere [impostazioni C++ di progetto per una configurazione di debug](../debugger/project-settings-for-a-cpp-debug-configuration.md).

5. Configurare le opzioni per la **generazione di file di database di programma**.

   Nella maggior C++ parte dei progetti, il valore `$(OutDir)$(TargetName).pdb`predefinito è, che genera i file con estensione PDB nella cartella di output.

   ![Genera PDB per C++ ] le compilazioni in (../debugger/media/dbg_project_properties_pdb_cplusplus.png "GeneratePDBsforCPlusPlus")
