...
description: Informazioni su come abilitare il debug per ASP.NET e ASP.NET core in Visual Studio. È possibile eseguire il processo in un server IIS Express o in un server IIS locale.
...
1. Nel riquadro **Proprietà,** in **Server**,
   ...
   - Per IIS locale,
     ...
     1. Accanto al **campo PROJECT URL (URL** progetto) selezionare Create Virtual Directory (Crea **directory** virtuale), se l'app non è ancora stata impostata in IIS.
...
1. Per eseguire il debug dell'app, nel progetto impostare punti di interruzione su codice. Nella barra Visual Studio, assicurarsi che la configurazione sia impostata su **Debug** e che il browser desiderato venga visualizzato in **IIS Express ( ) \<Browser name> o** IIS locale **( \<Browser name> )** nel campo emulatore.
...
## <a name="debug-aspnet-core-apps"></a>Eseguire il debug ASP.NET app Core
...
1. In **Variabili di** ambiente verificare che ASPNETCORE_ENVIRONMENT sia presente con il valore **Sviluppo**.  In caso contrario, **selezionare Aggiungi** e aggiungerlo.

   ![ASP.NET del debugger core](../debugger/media/dbg-aspnet-enable-debugging3.png "ASP.NET delle impostazioni del debugger core")

1. Usare **File**  >  **Save Selected Items (Salva elementi** selezionati) o **CTRL** + **S** per salvare le modifiche.

1. Per eseguire il debug dell'app, nel progetto impostare punti di interruzione in parte del codice. Nella barra degli strumenti Visual Studio verificare che la configurazione sia impostata su **Debug** e che nel campo dell'emulatore sia visualizzato **IIS Express** o il nuovo nome del profilo IIS.
...
2. Da Visual Studio selezionare Debug > Connetti **a** processo o premere **CTRL** ALT P e connettersi al processo ASP.NET o +  + ASP.NET Core (in genere **w3wp.exe** **odotnet.exe**). Per altre informazioni, vedere [Connettersi al processo](attach-to-running-processes-with-the-visual-studio-debugger.md) e [Come trovare il nome del ASP.NET processo](how-to-find-the-name-of-the-aspnet-process.md).
...
> [!NOTE]
> ASP.NET core non hanno inizialmente fileweb.config, ma  usanoappsettings.jssu  elaunchSettings.jsfile per la configurazione dell'app e le informazioni di avvio.  La distribuzione dell'app crea *web.config* file o file nel progetto, ma in genere non contengono informazioni di debug.
...
3. Scegliere **IIS, FTP e così via e fare** clic su **Pubblica.**
    ...
    ![Screenshot della finestra di dialogo Selezionare una destinazione di pubblicazione Visual Studio. Viene selezionato un Distribuzione Web IIS, FTP e il pulsante Pubblica evidenziato.](media/dbg-aspnet-local-iis.png)
