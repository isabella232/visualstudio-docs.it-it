...
description: Avviare una Visual Studio di debug per un'app piattaforma UWP (Universal Windows Platform) (UWP). Configurare la sessione di debug e scegliere la modalità di avvio dell'app.
...
In Visual Studio 2017, la maggior parte dei comandi e delle opzioni illustrati in questo articolo si applica anche alle app UWP per HTML e JavaScript. Se i comandi sono diversi tra le app gestite e le app C++, le app JavaScript sono in genere uguali ai comandi per le app UWP C++.
::: moniker-end

## <a name="start-debugging-from-the-visual-studio-toolbar"></a><a name="BKMK_The_easy_way_to_start_debugging"></a>Avviare il debug dalla barra degli strumenti Visual Studio comandi
...
1. Nell'elenco a discesa accanto alla freccia verde selezionare la destinazione di debug. È possibile scegliere un computer locale, un dispositivo connesso diretto, un simulatore Visual Studio locale, un dispositivo remoto o un emulatore.

1. Per avviare il debug, selezionare la freccia **Start** verde sulla barra degli strumenti oppure selezionare **Debug**  >  **Avvia debug** oppure premere **F5.**

   Visual Studio compila e avvia l'app con il debugger collegato.

Il debug continua fino a quando non viene raggiunto un punto di interruzione, si sospende manualmente l'esecuzione, si verifica un'eccezione non gestita o l'app termina.
...
È possibile impostare la destinazione di debug nella barra Visual Studio o nella pagina delle proprietà debug del progetto. Selezionare una delle opzioni seguenti:
...
|**Emulatore di dispositivi mobili**|Avviare l'emulatore specificato nel nome dell'emulatore, distribuire l'app e avviare il debug. Gli emulatori sono disponibili solo nei computer abilitati per Hyper-V.|
...
1. Sul lato sinistro del **riquadro** Proprietà:
   ...
   - Per Le app C# e Visual Basic, selezionare **Debug**.
     ...
     ![Pagina delle proprietà di debug Visual Basic progetto C# e](../debugger/media/dbg_csvb_debugpropertypage.png)
...
Per le app C# e Visual Basic, Visual Studio debug del codice gestito per impostazione predefinita. È possibile scegliere di eseguire il debug di altri tipi di codice o aggiuntivi. È anche possibile impostare valori **di tipo Debugger** per tutte le attività in background che fanno parte del progetto.
...
- Per le app C++, selezionare uno dei debugger seguenti nell'elenco a discesa **Tipo di debugger** nella pagina delle **proprietà** Debug.
...
|**Nativo con script**|Eseguire il debug di codice C/C++ nativo e codice JavaScript nell'app. Il codice gestito viene ignorato. Disponibile solo nei progetti C++ o nelle attività in background.|
...
### <a name="disable-network-loopbacks-optional"></a><a name="BKMK__Optional__Disable_network_loopbacks"></a> Disabilitare i loopback di rete (facoltativo)
 ...
 Per motivi di sicurezza, un'app UWP installata nel modo standard non può effettuare chiamate di rete al dispositivo in cui è installata. Visual Studio le app distribuite da questa regola per impostazione predefinita, in modo da poter testare le procedure di comunicazione in un singolo computer. Prima di rilasciare l'app, è consigliabile testare l'app senza l'esenzione.
...
- Per le app C++, selezionare **No** nell'elenco a discesa Consenti **loopback di** rete locale nella **pagina delle proprietà** Debug.
...
- Per le app C# e Visual Basic, nella pagina delle proprietà **Debug** selezionare **Computer remoto** come dispositivo di **destinazione**. Selezionare quindi **Nessuno o** Universale **(protocollo non crittografato) per** Modalità di **autenticazione**.
...
> [!CAUTION]
> Non esiste alcuna sicurezza di rete quando si esegue il debugger remoto in modalità **None** **o Universal (Unencrypted Protocol).** Scegliere queste modalità solo su reti attendibili che non sono a rischio di codice dannoso o traffico dannoso.
...
Per impostazione predefinita, Visual Studio l'app immediatamente all'avvio del debug. È anche possibile impostare l'app per l'esecuzione in modalità di debug, ma avviare l'app all'esterno del debugger. Ad esempio, è possibile eseguire il debug dell'avvio dell'app dal menu **Start** di Windows o eseguire il debug di un processo in background nell'app. Se si sceglie questa opzione, l'app viene avviata nel debugger all'avvio.
...
Per altre informazioni sul debug delle attività in background, vedere [Attivare eventi di sospensione, ripresa ed background per le app UWP.](../debugger/how-to-trigger-suspend-resume-and-background-events-for-windows-store-apps-in-visual-studio.md)
...
Puoi usare **Debug Installed App Package (Esegui** debug pacchetto app installato) per eseguire il debug di un'app UWP già installata o in esecuzione in un dispositivo locale o remoto. L'app potrebbe essere stata installata dal Microsoft Store o potrebbe non essere un progetto Visual Studio app. Ad esempio, l'app potrebbe avere un sistema di compilazione personalizzato che non usa Visual Studio.

È possibile avviare immediatamente l'app installata oppure impostarla per l'esecuzione nel debugger quando viene avviata con un altro metodo. Per altre informazioni, vedere [Attivare eventi di sospensione, ripresa e in background per le app UWP.](../debugger/how-to-trigger-suspend-resume-and-background-events-for-windows-store-apps-in-visual-studio.md)
...
### <a name="attach-the-debugger-to-a-running-windows-8x-app"></a><a name="BKMK_Attach_the_debugger_to_a_running_app_"></a> Collegare il debugger a un'app Windows 8.x in esecuzione

Per collegare il debugger a un'app in [!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)] , devi utilizzare Debuggable Package Manager per impostare l'esecuzione dell'app in modalità debug. Il Debuggable Package Manager viene installato con il Remote Tools per Visual Studio.
...
1. Nella schermata **Start di** Windows cercare e avviare **Debuggable Package Manager**.
...
1. Nella finestra **di dialogo Connessione a** processo specificare il dispositivo remoto nella casella **Destinazione** connessione .
...
::: moniker range="vs-2017"
> [!NOTE]
> Le app JavaScript vengono eseguite in un'istanza *delwwahost.exe* processo. Se è in esecuzione più di un'app JavaScript, è necessario conoscere l'ID processo numerico (PID) del processo diwwahost.exedell'app per *connettersi.*
>
> Il modo più semplice per connettersi all'app JavaScript è chiudere tutte le altre app JavaScript. Oppure, è possibile prendere nota dei PIN *dell'esecuzionewwahost.exe* processi in Windows Gestione attività prima di avviare l'app. Quando si avvia l'app, *wwahost.exe* PID sarà quello diverso da quello indicato in precedenza.
::: moniker-end
