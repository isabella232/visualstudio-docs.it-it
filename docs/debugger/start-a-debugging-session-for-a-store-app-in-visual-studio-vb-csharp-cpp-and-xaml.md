---
title: Avviare una sessione di debug per un'app UWP in Visual Studio (VB, c#, C++ e XAML) | Documenti Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
![Si applica a Windows e Windows Phone](../debugger/media/windows_and_phone_content.png "windows_and_phone_content")  
 ...
 In questo argomento viene descritto come avviare una sessione di debug per App UWP scritte in XAML e Visual C++, Visual c# o Visual Basic. Il debug di un'app comporta sia la configurazione della la sessione di debug che la scelta della modalità di avvio dell'app.  
...
2.  Premi F5.  
 ...
 Visual Studio compila e avvia l'app con il debugger collegato. L'esecuzione continua fino a raggiungere un punto di interruzione. Sospendi manualmente l'esecuzione e si verifica un'eccezione non gestita o l'app termina. Per ulteriori informazioni, vedere [esplorare una sessione di debug (Xaml e c#)](../debugger/navigate-a-debugging-session-in-visual-studio-xaml-and-csharp.md) .  
...
2.  In questo modo si apre la pagina delle proprietà di debug per il progetto:  
    ...
    -   Per le app Visual C# e Visual Basic scegli **Debug**.  
         ...
         ![C &#35; &#47; Pagina delle proprietà debug progetto VB](../debugger/media/dbg_csvb_debugpropertypage.png "DBG_CsVb_DebugPropertyPage")  
  
    -   Per le app Visual C++ espandi il nodo **Proprietà di configurazione**  , quindi scegli **Debug**.  
  
         ![C &#43; &#43; Pagina delle proprietà di debug dell'app di piattaforma UWP](../debugger/media/dbg_cpp_debugpropertypage.png "DBG_CPP_DebugPropertyPage")  
...
###  <a name="BKMK_Choose_the_deployment_target"></a> Scegliere la destinazione di distribuzione  
 ...
 È possibile distribuire ed eseguire il debug di un'app UWP nel computer Visual Studio, nel simulatore di Visual Studio nel computer locale o in un dispositivo remoto.  
...
-   Scegli il computer remoto nella finestra di dialogo **Seleziona connessione debugger remoto** .  
    ...
    -   Per le app C++, scegliere la freccia rivolta verso il basso nel **nome macchina** e scegliere  **\<trova... >**.  
 ...
 È possibile distribuire ed eseguire il debug di un'app di Windows Phone in un dispositivo o in uno degli emulatori del telefono di Visual Studio. Seleziona il dispositivo o l'emulatore nell'elenco **Dispositivo di destinazione** .  
...
|**Solo GPU**|Esegue il debug del codice C++ nativo eseguito su un'unità di elaborazione grafica (GPU).|  
 ...
 Per le app di Windows Phone, è anche possibile scegliere il debugger da usare per i processi in background dal **processo attività in Background**.  
