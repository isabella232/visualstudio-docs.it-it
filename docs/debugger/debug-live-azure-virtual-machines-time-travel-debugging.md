...
* Il debug dello viaggio nel tempo è disponibile per le app Web delle macchine virtuali di Azure seguenti:
...
1. Aprire il progetto per il quale si vuole raccogliere una registrazione dei viaggi nel tempo.

    > [!IMPORTANT]
    > Per avviare TTD, è necessario aprire la *stessa versione del* codice sorgente pubblicata nel servizio vm di Azure.

1. Scegliere **Debug > Collega Snapshot Debugger...**. Selezionare la macchina virtuale di Azure in cui è distribuita l'app Web e un account di archiviazione di Azure. Selezionare **l'opzione Enable the Time Travel Debugging** preview (Abilita l'anteprima di Time Travel Debugging) e quindi fare clic su Attach **(Allega).**
   ...
   La **finestra** Moduli mostra quando vengono caricati tutti i moduli per la macchina virtuale di Azure (scegliere Debug > **moduli windows >** per aprire questa finestra).
...
1. Fare clic con il pulsante destro del mouse sull'icona del punto di agganciato (la palla vuota) e scegliere **Azioni**. Nella finestra **Impostazioni snapshot** fare clic sulla casella **di controllo** Azione . Fare quindi clic sulla casella di controllo Collect a time travel trace to the end of this method (Raccogli una traccia di viaggio temporale fino **alla fine di questo metodo).**
...
1. Fare clic sul collegamento Visualizza snapshot per aprire la registrazione dei viaggi nel tempo nell'editor di codice.
   ...
   È possibile eseguire ogni riga di codice registrata dal TTD usando i pulsanti **Continua** **e Inverti continuazione.** È anche possibile usare la barra degli strumenti   **Debug** per visualizzare l'istruzione **successiva,** Esegui **istruzione,** Esegui istruzione/istruzione/ Esegui istruzione/istruzione successiva.  
