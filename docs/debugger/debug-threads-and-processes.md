...
- Per il codice che usa <xref:System.Threading.Tasks.Task> nel [Task Parallel Library (TPL)](/dotnet/standard/parallel-programming/task-parallel-library-tpl), il [runtime di concorrenza](/cpp/parallel/concrt/concurrency-runtime/) (codice nativo), gli strumenti principali per il debug di applicazioni multithread sono la finestra **stack in parallelo** , la finestra espressione di **controllo** in parallelo e la finestra **attività** (la finestra **attività** supporta anche l'oggetto Promise JavaScript).
