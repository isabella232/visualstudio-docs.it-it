...
description: Eseguire il debug usando il debugger JUST-In-Time in Visual Studio. Il debug JIT può essere avviato automaticamente Visual Studio quando un'app si arresta in modo anomalo o si arresta in modo anomalo.
...
Il debug JIT può essere avviato automaticamente Visual Studio'esecuzione di un'app all'esterno Visual Studio o si arresta in modo anomalo. Con il debug JIT, è possibile testare le app al di fuori Visual Studio e aprire Visual Studio per avviare il debug quando si verifica un problema.

Il debug JIT funziona per Windows desktop. Non funziona per le app Windows universali o per il codice gestito ospitato in un'applicazione nativa, ad esempio i visualizzatori.

> [!TIP]
> Se si vuole solo arrestare la visualizzazione della finestra di dialogo Debugger JUST-In-Time, ma non è installato Visual Studio, vedere Disabilitare il debugger [JUST-In-Time](../debugger/just-in-time-debugging-in-visual-studio.md). Se è stato installato Visual Studio, potrebbe essere necessario disabilitare il debug JIT dal registro Windows [.](#disable-just-in-time-debugging-from-the-windows-registry)
...
>[!NOTE]
>Per abilitare o disabilitare il debug JIT, è necessario eseguire Visual Studio come amministratore. L'abilitazione o la disabilitazione del debug JIT imposta una chiave del Registro di sistema e potrebbe essere necessario disporre dei privilegi di amministratore per modificare tale chiave. Per aprire Visual Studio amministratore, fare clic con il pulsante destro del mouse sull'app Visual Studio e scegliere **Esegui come amministratore**.

È possibile configurare il debug JIT dalla finestra di dialogo Visual Studio **Tools**  >  **Options** (o **Opzioni**  >  **di** debug).
...
## <a name="disable-just-in-time-debugging-from-the-windows-registry"></a>Disabilitare il debug JIT dal Registro Windows registrazione
...
1. Dal menu Windows **Start** eseguire **l'editor** del Registro di sistema (*regedit.exe*).
...
## <a name="enable-just-in-time-debugging-of-a-windows-form"></a>Abilitare il debug JIT di un form Windows JIT

Per impostazione predefinita, Windows le app Form dispongono di un gestore eccezioni di primo livello che consente all'app di continuare a essere in esecuzione se è in grado di eseguire il ripristino. Se un'Windows Forms genera un'eccezione non gestita, viene visualizzata la finestra di dialogo seguente:

![Windows Eccezione non gestita del modulo](../debugger/media/windowsformsunhandledexception.png "Windows Form unhandled exception (Genera eccezione non gestita)")

Per abilitare il debug JIT anziché la gestione degli errori Windows modulo standard, aggiungere queste impostazioni:

- Nella sezione `system.windows.forms` del file *machine.config* *\<app name> o con estensione exe.config* impostare il valore `jitDebugging` su `true` :
...
- È necessario aver installato Visual Studio per seguire questa procedura. Se non si dispone di Visual Studio, è possibile scaricare la versione [Visual Studio Community Edition](https://visualstudio.microsoft.com/thank-you-downloading-visual-studio/?sku=Community&rel=15)gratuita.

- Assicurarsi che il debug JIT sia [abilitato](#BKMK_Enabling) **in** Opzioni degli strumenti  >    >  **Debug**  >  **JIT.**
...
1. Per compilare la soluzione, scegliere la **configurazione Debug** (impostazione predefinita) o **Versione** e quindi selezionare **Compila**  >  **ricompila soluzione**.
   ...
   > - Se si seleziona [Configurazione](../debugger/how-to-set-debug-and-release-configurations.md) versione, è necessario disattivare Just My Code [per](../debugger/just-my-code.md) il funzionamento di questa procedura. In **Strumenti**  >  **Opzioni**  >  **debug** deselezionare **Abilita Just My Code**.

   Per altre informazioni sulle configurazioni di compilazione, vedere [Informazioni sulle configurazioni di compilazione](../ide/understanding-build-configurations.md).
...
1. Verrà visualizzata la finestra di dialogo Scegli debugger **JUST-In-Time.**

   ![Screenshot della finestra di dialogo Scegli debugger JUST-In-Time, che viene visualizzata dopo l'eccezione nella finestra della console ThrowsNullException.exe finestra della console.](../debugger/media/justintimedialog.png)
...
- **Impossibile connettersi al processo di arresto anomalo del sistema. Il programma specificato non è un Windows o MS-DOS.**

    Il debugger ha tentato di connettersi a un processo in esecuzione con un altro utente.

    Per risolvere questo problema, in Visual Studio aprire Debug collega a processo (o premere  >   **CTRL**  +  **ALT**  +  **P)** e trovare il processo di cui si vuole eseguire il debug **nell'elenco Processi** disponibili. Se non si conosce il nome del processo, trovare l'ID processo nella finestra di dialogo Visual Studio debugger **JUST-In-Time.** Selezionare il processo **nell'elenco Processi** disponibili e selezionare **Collega**. Selezionare **No** per chiudere la finestra di dialogo debugger JUST-In-Time.
