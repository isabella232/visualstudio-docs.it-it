...
Gli oggetti TableAdapter estendono la funzionalità degli adattatori dati standard incapsulando una classe <xref:System.Data.Common.DataAdapter> configurata. Per impostazione predefinita, il TableAdapter eredita dalla classe <xref:System.ComponentModel.Component> e non è possibile eseguirne il cast alla classe <xref:System.Data.Common.DataAdapter>. Il cast di un TableAdapter alla classe <xref:System.Data.Common.DataAdapter> genera un errore <xref:System.InvalidCastException>. Per modificare la classe di base di un TableAdapter, è possibile specificare una classe che deriva da <xref:System.ComponentModel.Component> nella proprietà della **classe base** del TableAdapter nell' **Progettazione DataSet**.
