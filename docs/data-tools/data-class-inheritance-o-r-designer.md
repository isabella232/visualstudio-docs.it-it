...
Nell'ereditarietà a tabella singola è presente una singola tabella di database che contiene colonne per le classi base e derivate. Insieme ai dati relazionali, una colonna discriminante contiene il valore che determina la classe a cui appartiene uno specifico record. Si consideri, ad esempio, una tabella `Persons` che contiene tutti gli utenti usati da una società. alcune delle quali sono dipendenti mentre altre sono manager. La tabella `Persons` contiene una colonna denominata `Type` con un valore pari a 1 per i Manager e un valore pari a 2 per i dipendenti. La colonna `Type` è la colonna discriminatore. In questo scenario è possibile creare una sottoclasse di dipendenti e popolare la classe con solo i record che hanno un valore `Type` 2.
