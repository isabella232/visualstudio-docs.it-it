---
title: Progettazione flussi di lavoro - ActivityDesigner di transizione
description: Informazioni su come usare l'ActivityDesigner di transizione per configurare una transizione tra due stati.
...
|<xref:System.Activities.Statements.Transition.Action%2A>|Falso|Specifica l'attività eseguita quando l'attività del trigger è stata completata e , se <xref:System.Activities.Statements.Transition.Condition%2A> presente, restituisce **true.** Questa attività viene eseguita durante la transizione allo stato di destinazione, dopo l'esecuzione dell'attività di <xref:System.Activities.Statements.State.Exit%2A> per lo stato di origine, se presente. Quando la finestra di progettazione delle transizioni viene espansa, questo valore può essere impostato trascinando un'attività dalla Casella degli strumenti e rilasciarla nella sezione **Azione** della transizione.  Possono essere presenti più azioni per una sola transizione. Le singole azioni possono essere espanse e contratte e possono essere ordinate facendo clic sulla freccia verso l'alto o verso il basso visualizzato sull'azione quando sono presenti più azioni in una transizione.|
