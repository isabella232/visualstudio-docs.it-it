...
* Usare Visual Studio e i servizi di **[Azure:](#use-visual-studio-and-azure-services)** questa tabella include gli URL di dominio da aggiungere a un elenco di indirizzi consentiti e le porte e i protocolli da aprire in modo da avere accesso a tutte le funzionalità e i servizi desiderati.
...
Per assicurarsi di avere accesso a tutti gli elementi desiderati quando si usa Visual Studio o Servizi di Azure dietro un firewall o un server proxy, di seguito sono elencati gli URL da aggiungere a un elenco di indirizzi consentiti e le porte e i protocolli che è possibile aprire.
...
| Componenti aggiuntivi di <br>Componenti aggiuntivi per Office | sharepoint.com<br> microsoft.com/microsoft-365<br> microsoftonline.com <br> outlook.com | https/443 | Consente di pubblicare e testare i componenti aggiuntivi di SharePoint e Office in SharePoint Online e Microsoft 365 |
...
| Active Directory <br>Grafico | graph.windows.net | https/443 | Usato per eseguire il provisioning di nuove applicazioni di Azure Active Directory. Usato anche dal provider di Microsoft 365 connesso a MSGraph |
...
| Servizi cloud | 1. RDP<br><br>2. core.windows.net <br><br>3.  management.azure.com<br> management.core.windows.net <br><br>4. &#42;.blob.core.windows.net <br>&#42;.queue.core.windows.net<br>&#42;.table.core.windows.net <br><br>5. portal.azure.com <br><br>6. &#60;servizio cloud utente&#62;.cloudapp.net <br> &#60;macchina virtuale utente&#62;.&#60;regione&#62;.azure.com | 1. rdp/3389 <br><br> 2. https/443 <br><br> 3. https/443 <br><br> 4. https/443 <br><br> 5. https/443 <br><br>6. tcp <br>a) 30398 <br>b) 30400 <br>c) 31398 <br>d) 31400 <br>e) 32398 <br>f) 32400 | 1. Desktop remoto macchina virtuale di Servizi cloud <br><br> 2. Componente dell'account di archiviazione della configurazione di diagnostica privata <br><br> 3. portale di Azure <br><br> 4. Esplora server: Archiviazione di Azure &#42; account di archiviazione denominato del cliente  <br><br> 5. Collegamenti per aprire il portale e &#47; scaricare il certificato di sottoscrizione &#47; file di impostazioni di pubblicazione <br><br>6. a) Porta locale del connettore per il debug remoto per il servizio cloud e la macchina virtuale<br> 6. b) Porta pubblica del connettore per il debug remoto per il servizio cloud e la macchina virtuale <br> 6. c) Porta locale del server di inoltro per il debug remoto per il servizio cloud e la macchina virtuale <br> 6. d) Porta pubblica del server di inoltro per il debug remoto per il servizio cloud e la macchina virtuale  <br> 6. e) Porta locale del caricatore di file per il debug remoto per il servizio cloud e la macchina virtuale <br> 6. f) Porta pubblica del caricatore di file per il debug remoto per il servizio cloud e la macchina virtuale |
| Service Fabric | 1. <br>Documenti. Microsoft.com<br>aka.ms <br>go.microsoft.com <br><br>2. <br>vssftools.blob.core.windows.net <br>Vault.azure.com <br>Portal.azure.com <br><br> 3. &#42; vault.azure.net<br><br> 4. <br>app.vsaex.visualstudio.com<br>&#42; .vsspsext.visualstudio.com<br>clouds.vsrm.visualstudio.com <br>clouds.visualstudio.com<br>app.vssps.visualstudio.com <br>&#42; .visualstudio.com | https/443 | 1. Documentazione <br><br> 2. Creare la funzionalità cluster <br><br>3. Il &#42; è il nome dell'insieme di credenziali delle chiavi di Azure (ad esempio: - test11220180112110108.vault.azure.net  <br><br>  4. Il &#42; dinamico (esempio: vsspsextprodch1su1.vsspsext.visualstudio.com) |
| Snapshot <br>Debugger | 1. go.microsoft.com <br>2. management.azure.com <br> 3. &#42;.azurewebsites.net <br> 4. &#42;.scm.azurewebsites.net<br>5. api.nuget.org/v3/index.json <br>6. Indirizzo IP/FQDN del servizio remoto/server | 1. https/443 <br>2. https/443  <br>3. http/80 <br>4. https/443 <br>5. https/443 <br>6. Concord/<br> 4022 (dipendente Visual Studio versione) | 1. Eseguire query sul file JSON per le dimensioni dello SKU del servizio app <br>2. Varie chiamate RM di Azure <br>3. Chiamata di riscaldamento del sito tramite  <br>4. Endpoint Kudu del servizio app di destinazione del cliente <br>5. Eseguire query sulla versione dell'estensione del sito pubblicata in nuget.org <br>6. [Debug remoto](../debugger/remote-debugging.md) |
...
| Servizio di creazione di pacchetti | [account].visualstudio.com <br/> [account].\*.visualstudio.com <br/> \*.blob.core.windows.net <br/> registry.npmjs.org </br> nodejs.org <br/> dist.nuget.org <br/> nuget.org | https/443 | I file con estensione npmjs.org, nuget.org e nodejs.org sono necessari solo per determinati scenari di attività di compilazione (ad esempio, programma di installazione dello strumento NuGet, programma di installazione dello strumento Node) o se si prevede di usare l'upstream pubblico con i \* \* \* feed. Gli altri tre domini sono necessari per la funzionalità core del servizio Creazione pacchetto. |
...
| Bus di servizio di Azure | \*.servicebus.windows.net | ampq/5671 e 5672, </br> sbmp/9350-9354, </br> http/80, </br> https/443 | Consente di creare code, argomenti e sottoscrizioni. </br> Usato anche per inviare/ricevere messaggi da e verso code e argomenti del bus di servizio. |
| Azure Cosmos DB | \*File con estensione documents.azure.com | https/443 | Usato per chiamare le API del database di documenti principali. |
| Community degli sviluppatori | sendvsfeedback2.azurewebsites.net/api | https/443 | Usato per chiamare le DEVELOPER COMMUNITY Feedback Tool (problemi, ricerca, voto, commento, invio, caricamento, ripresa) |
| Intellicode | \*File con estensione intellicode.vsengsaas.visualstudio.com | https/443 | Usato per chiamare le API Intellicode |
| Live Share | \*File con estensione liveshare.vsengsaas.visualstudio.com| https/443 | Usato per chiamare Live Share API |
| GitHub Codespaces | \*File con estensione online.visualstudio.com | https/443 | Usato per chiamare le API Di GitHub Codespaces |
| Acquisizione automatica del tipo JavaScript | registry.npmjs.org | https/443 | Usato per installare definizioni di tipo TypeScript per fornire IntelliSense per le librerie JavaScript più diffusi |
| Sottoscrizioni di Visual Studio Licensing Service | app.vssps.visualstudio.com/apis/<br/>Licenze/ClientRights | https/443 | Licenze per l'attivazione online |
| Debugger | 1. <br>vsdebugger.blob.core.windows.net <br>vsdebugger.azureedge.net <br><br>2. <br>download.visualstudio.com/\*/<br/>onecore.msvsmon. \* . Zip<br><br> 3. referencesource.microsoft.com/symbols <br><br> 4. <br>symbols.nuget.org/download/symbols<br><br> 5. visualstudio.com<br><br>6. msdl.microsoft.com/download/symbols | https/443 | 1. <br>Usato per scaricare i bit del debugger per il debug di .NET Core in Unix/macOS su SSH <br><br>2. <br>Usato per scaricare i bit del debugger per il debug remoto dei contenitori Docker di Windows<br><br> 3. Usato per l'esecuzione di istruzioni nel codice sorgente di .NET Framework <br><br> 4. <br>(Se l'utente acconsente esplicitamente) Usato per il download dei simboli pubblicati nuget.org server dei simboli.<br><br> 5. (Se l'utente acconsente esplicitamente) Usato per scaricare i simboli e i file binari di MS, potrebbe essere necessario anche per il debug del codice gestito nei dump |
| GitHub Codespaces| \*File con estensione online.visualstudio.com | https/443 | Usato per chiamare le API Codespaces di GitHub |
| Pubblicazione di app Xamarin Android | \*File con estensione googleapis.com <br/> play.google.com <br/>accounts.google.com | https/443 | Usato per interagire con Google Play Store per pubblicare/caricare applicazioni Xamarin Android direttamente da Visual Studio. |
| Visual Studio di ricerca | data-ai.microsoft.com/search | https/443 | Usato per fornire il servizio Visual Studio di ricerca abilitato per l'intelligenza artificiale nella `Ctrl+Q` casella di ricerca |
