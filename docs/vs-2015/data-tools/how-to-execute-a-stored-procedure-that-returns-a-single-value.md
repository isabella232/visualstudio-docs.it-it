---
title: 'Procedura: eseguire una Stored Procedure che restituisce un singolo valore | Microsoft Docs'
ms.custom: ''
ms.date: 11/15/2016
...
ms.locfileid: "49281008"
---
# <a name="how-to-execute-a-stored-procedure-that-returns-a-single-value"></a>Procedura: eseguire una stored procedure che restituisce un valore singolo
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Per eseguire una stored procedure che restituisce un valore singolo, è possibile eseguire query TableAdapter che è configurato per eseguire una stored procedure (ad esempio, `CustomersTableAdapter.CustomerCount()`).  
 ...
 Negli esempi seguenti illustrano come oggetti comando o l'esecuzione di stored procedure che restituiscono valori a precisione singola da un database utilizzando TableAdapter. Per altre informazioni sull'esecuzione di query TableAdapter e comandi, vedere [compilare i set di dati usando oggetti TableAdapter](../data-tools/fill-datasets-by-using-tableadapters.md).  
  
 [!INCLUDE[note_settings_general](../includes/note-settings-general-md.md)]  
  
## <a name="executing-stored-procedures-that-return-single-values-using-a-tableadapter"></a>L'esecuzione di Stored procedure che restituiscono valori singoli mediante un TableAdapter  
...
#### <a name="to-execute-a-stored-procedure-that-returns-a-single-value-using-a-tableadapter"></a>Per eseguire una stored procedure che restituisce un valore singolo utilizzando un oggetto TableAdapter  
...
3.  Se si dispone già di una query su un determinato TableAdapter che utilizza una stored procedure per restituire un singolo valore, passare alla procedura successiva, "per"dichiarare un'istanza dell'oggetto TableAdapter ed eseguire la query. In caso contrario, continuare con il passaggio 4 per creare una nuova query che restituisce un valore singolo.  
...
7.  Selezionare l'opzione per restituire **un valore singolo**, quindi fare clic su **successivo**.  
  
8.  Specificare un nome per la query.  
  
9. Fare clic su **successivo** oppure **fine** per completare la procedura guidata; la query viene aggiunto all'oggetto TableAdapter.  
...
## <a name="executing-stored-procedures-that-return-single-values-using-a-command-object"></a>L'esecuzione di Stored procedure che restituiscono valori singoli usando un oggetto Command  
 Nell'esempio seguente viene illustrato come creare un comando ed eseguire una stored procedure che restituisce un valore singolo. Per informazioni sull'impostazione e recupero i valori dei parametri per un comando, vedere [procedura: impostare e ottenere i parametri per gli oggetti comando](http://msdn.microsoft.com/library/10110ecc-d2ed-4796-bb8f-74f2ecd40787).  
...
#### <a name="to-execute-a-stored-procedure-that-returns-a-single-value-using-a-datacommand"></a>Per eseguire una stored procedure che restituisce un valore utilizzando un DataCommand  
  
-   Aggiungere il codice seguente a un metodo che si desidera eseguire il codice da. Per restituire valori singoli, chiamare il `ExecuteScalar` metodo di un comando (ad esempio, <xref:System.Data.SqlClient.SqlCommand.ExecuteScalar%2A>). I dati vengono restituiti un `object`.  
