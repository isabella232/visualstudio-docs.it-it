...
2. Creare un nuovo set di dati basato sulla `Customers` tabella Northwind.

3. Creare un modulo con un oggetto <xref:System.Windows.Forms.DataGridView> per visualizzare i dati.

4. Compilare un set di dati con i dati della `Customers` tabella nel database Northwind.

5. Utilizzare [Visual Database Tools](https://msdn.microsoft.com/6b145922-2f00-47db-befc-bf351b4809a1) in Visual Studio per accedere direttamente alla `Customers` tabella dati e modificare un record.
...
4. Assegnare un nome al progetto `ConcurrencyWalkthrough` e quindi fare clic su **OK**.
...
1. Scegliere **Aggiungi nuova origine dati**dal menu **dati** .
     ...
     Verrà avviata la [Configurazione guidata origine dati](https://msdn.microsoft.com/library/c4df7de5-5da0-4064-940c-761dd6d9e28f) .
...
5. Espandere il nodo **tabelle** e selezionare la `Customers` tabella. Il nome predefinito per il set di dati deve essere `NorthwindDataSet` .
...
1. Selezionare **F5** per eseguire l'applicazione
     ...
     Il form viene visualizzato con un <xref:System.Windows.Forms.DataGridView> controllo che viene compilato con i dati della `Customers` tabella.
...
### <a name="addcode-to-handle-the-concurrency-exception"></a>AddCode per gestire l'eccezione di concorrenza
 ...
 In questa sezione viene aggiunto il codice che tenta di aggiornare il database. È anche possibile gestire gli eventuali <xref:System.Data.DBConcurrencyException> che potrebbero essere generati, nonché qualsiasi altra eccezione.

> [!NOTE]
> I `CreateMessage` `ProcessDialogResults` metodi e verranno aggiunti più avanti in questa procedura dettagliata.
...
- Creare il messaggio aggiungendo il codice seguente all'editor di **codice**. Immettere il codice sotto il `UpdateDatabase` metodo.
...
6. Nel primo record ( `ALFKI` ) modificare in `ContactName` `Maria Anders2` .
...
8. Nel primo record nel form ( `ALFKI` ) modificare in `ContactName` `Maria Anders1` .
