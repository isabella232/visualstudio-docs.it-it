 ...
 **Tema colori** Scegliere il tema colorato **blu**, **chiaro** o **scuro** per l'IDE.
 ...
 Per impostazione predefinita, applicare la distinzione tra maiuscole e minuscole nei menu della **barra dei menu** in Visual Studio 2015. Deselezionare questa opzione per impostarli su **TUTTO MAIUSCOLE**.

 **Regola automaticamente l'esperienza visiva in base alle prestazioni del client** Specifica se Visual Studio imposta automaticamente la regolazione dell'esperienza visiva o la regolazione viene impostata in modo esplicito. Questa regolazione può modificare la visualizzazione di colori da gradienti a colori uniformi o può limitare l'uso di animazioni nei menu o finestre a comparsa.

 **Abilita esperienza rich client** Consente l'esperienza visiva completa di Visual Studio, incluse le sfumature e le animazioni. Deselezionare questa opzione se si usano Connessioni desktop remoto o schede grafiche obsolete, perché le prestazioni di queste funzionalità potrebbero risultare compromesse. Questa opzione è disponibile solo se si deseleziona l'opzione **Regola automaticamente esperienza visiva in base alle prestazioni del client**.
 ...
 **Gestisci associazioni file** Consente di visualizzare la finestra di dialogo **Imposta associazioni programmi** , in cui è possibile visualizzare le estensioni di file per gli elementi in genere associati a Visual Studio e il programma predefinito corrente per l'apertura di ogni tipo di file. Per impostare Visual Studio come applicazione predefinita per i tipi di file non ancora associati a questa applicazione, selezionare l'estensione e quindi scegliere **Salva**.
