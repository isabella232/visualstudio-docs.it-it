...
In questo argomento vengono elencati gli errori comuni seguenti che possono verificarsi durante la distribuzione di un' [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] applicazione e vengono descritti i passaggi per risolvere ogni problema.  
...
#### <a name="when-you-try-to-locate-an-application-file-nothing-occurs-or-xml-renders-in-internet-explorer-or-you-receive-a-run-or-save-as-dialog-box"></a>Quando si tenta di individuare un file. Application, non viene eseguita alcuna operazione o viene eseguito il rendering XML in Internet Explorer oppure si riceve una finestra di dialogo Esegui o Salva con nome  
 ...
 Se il server è configurato correttamente, verificare che [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)] sia installato nel computer. Se [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)] è installato e si sta ancora riscontrando questo problema, provare a disinstallare e reinstallare [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)] per registrare nuovamente il tipo di contenuto nel client.  
...
#### <a name="error-message-says-unable-to-retrieve-application-files-missing-in-deployment-or-application-download-has-been-interrupted-check-for-network-errors-and-try-again-later"></a>Messaggio di errore: "Impossibile recuperare l'applicazione. File mancanti nella distribuzione "o" download dell'applicazione interrotto, verificare la presenza di errori di rete e riprovare più tardi "  
 Questo messaggio indica che non è possibile scaricare uno o più file a cui viene fatto riferimento dai [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] manifesti. Il modo più semplice per eseguire il debug di questo errore è provare a scaricare l'URL che indica che non è [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] possibile scaricarlo. Di seguito sono riportate alcune possibili cause:  
...
- Se il file config è bloccato dal server, vedere la sezione "errore di download quando si tenta di installare un' [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] applicazione con un file con estensione config" più avanti in questo argomento.  
  
- Determinare se questo errore si è verificato perché l' `deploymentProvider` URL nel manifesto di distribuzione punta a una posizione diversa da quella usata per l'attivazione.  
  
- Verificare che tutti i file siano presenti nel server. il [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] log deve indicare quale file non è stato trovato.  
...
- Provare a riavviare l'applicazione dal menu Start. [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] è possibile che l'aggiornamento sia stato rilevato in background, ma verrà richiesto di installare i bit alla successiva attivazione.  
...
#### <a name="error-cannot-start-the-application"></a>Errore: "Impossibile avviare l'applicazione"  
 Questo messaggio di errore indica in genere che si è verificato un problema durante l'installazione dell'applicazione nell' [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] archivio. Si è verificato un errore nell'applicazione oppure l'archivio è danneggiato. Il file di log potrebbe indicare dove si è verificato l'errore.  
...
|Il download dell'applicazione non è riuscito. Controllare la connessione di rete o contattare l'amministratore di sistema o il provider di servizi di rete.|[!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] Impossibile stabilire una connessione di rete al server. Esaminare la disponibilità del server e lo stato della rete.|  
...
|È in corso un tentativo di caricamento di un numero eccessivo di attivazioni distribuite in una sola volta.|[!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] limita il numero di applicazioni diverse che possono essere avviate allo stesso tempo. Questo è in gran parte utile per la protezione da tentativi dannosi di istigare attacchi Denial of Service contro il servizio locale. [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] gli utenti che tentano di avviare la stessa applicazione ripetutamente, in rapida successione, finiranno solo con una singola istanza dell'applicazione.|  
|Non è possibile attivare i collegamenti sulla rete.|I collegamenti a un' [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] applicazione possono essere avviati solo sul disco rigido locale. Non possono essere avviati aprendo un URL che punta a un file di collegamento in un server remoto.|  
