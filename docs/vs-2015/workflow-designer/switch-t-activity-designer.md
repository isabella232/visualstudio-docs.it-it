---
title: Commutatore&lt;T&gt; ActivityDesigner | Microsoft Docs
ms.date: 11/15/2016
...
L'attività <xref:System.Activities.Statements.Switch%601> valuta un'espressione specificata e viene eseguita da una raccolta di attività la cui chiave associata corrisponde al valore ottenuto dalla valutazione.  
 ...
 Il **commutatore\<T >** ActivityDesigner viene utilizzato per creare e configurare un <xref:System.Activities.Statements.Switch%601> attività nel [!INCLUDE[wfd1](../includes/wfd1-md.md)].  
...
## <a name="how-to-use-the-switcht-activity-designer"></a>Come usare l'opzione\<T > Activity Designer  
 Il **commutatore\<T >** ActivityDesigner è reperibile nella **flusso di controllo** categoria del **casella degli strumenti**, accessibile facendo il **Casella degli strumenti** scheda il [!INCLUDE[wfd2](../includes/wfd2-md.md)] (in alternativa, selezionare **sulla barra degli strumenti** dal **visualizzazione** menu oppure premere CTRL + ALT + X.) Dopo averlo rilasciato nel [!INCLUDE[wfd2](../includes/wfd2-md.md)], viene visualizzato il **Seleziona tipi** finestra di dialogo per consentire all'utente di specificare il tipo generico *T* usato in <xref:System.Activities.Statements.Switch%601> attività. Il valore predefinito è **Int32**. Una volta il tipo generico *T* è stata selezionata, un **commutatore\<T >** progettazione viene aggiunto nella finestra di progettazione del flusso di lavoro.  
  
 Di seguito sono le proprietà del **commutatore\<T >** finestra di progettazione. Tutte le proprietà possono essere modificate nella griglia delle proprietà. Alcune di esse possono anche essere modificate nell'area della finestra di progettazione.  
