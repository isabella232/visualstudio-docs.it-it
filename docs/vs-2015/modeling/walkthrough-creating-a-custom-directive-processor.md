...
I processori di direttiva * funzionano aggiungendo codice alla *classe Transformation generata*. Se si chiama una *direttiva* da un *modello di testo*, il resto del codice scritto nel modello di testo può basarsi sulla funzionalità fornita dalla direttiva.
...
## <a name="creating-a-custom-directive-processor"></a>Creazione di un processore di direttiva personalizzato
 ...
 Il processore di direttiva personalizzato aggiunge la variabile e la proprietà alla classe della trasformazione generata. La direttiva che si scrive utilizza le classi <xref:System.CodeDom> per creare il codice che il motore aggiunge alla classe della trasformazione generata. Le classi <xref:System.CodeDom> creano codice in Visual C# o [!INCLUDE[vbprvb](../includes/vbprvb-md.md)], a seconda del linguaggio specificato nel parametro `language` della direttiva `template`. Il linguaggio del processore di direttiva e il linguaggio del modello di testo che accede al processore di direttiva non devono essere uguali.
...
1. In Visual Studio, creare un progetto Libreria di classi in C# o in Visual Basic denominato CustomDP.
    ...
    > [!NOTE]
    > Se si desidera installare il processore di direttiva su più di un computer, è meglio utilizzare un progetto [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] Extension (VSIX) e includere un file .pkgdef nell'estensione. Per ulteriori informazioni, vedere la pagina relativa alla [distribuzione di un processore di direttiva personalizzato](../modeling/deploying-a-custom-directive-processor.md).
...
4. [!INCLUDE[vbprvb](../includes/vbprvb-md.md)]Solo per, aprire il menu **progetto** e fare clic su **Proprietà CustomDP**. Nella scheda **applicazione** , in **spazio dei nomi radice**, eliminare il valore predefinito `CustomDP` .
...
## <a name="registering-the-directive-processor"></a>Registrazione del processore di direttiva
 Prima di poter chiamare una direttiva da un modello di testo in [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] , è necessario aggiungere una chiave del registro di sistema per il processore di direttiva.

> [!NOTE]
> Se si desidera installare il processore di direttiva su più di un computer, è meglio definire un progetto [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] Extension (VSIX) che includa un file .pkgdef insieme all'assembly. Per ulteriori informazioni, vedere la pagina relativa alla [distribuzione di un processore di direttiva personalizzato](../modeling/deploying-a-custom-directive-processor.md).
...
## <a name="testing-the-directive-processor"></a>Test del processore di direttiva
 ...
 In questo esempio, il modello di testo chiama la direttiva e passa nel nome di un file XML che contiene documentazione per un file della classe. Per ulteriori informazioni, vedere [commenti relativi alla documentazione XML](https://msdn.microsoft.com/library/803b7f7b-7428-4725-b5db-9a6cff273199).
...
4. Modificare il contenuto di TestDP.tt nel testo seguente.
    ...
    > [!NOTE]
    > Assicurarsi di sostituire la stringa <`YOUR PATH>` con il percorso del file di DocFile.xml.
...
1. In **Esplora soluzioni**fare clic con il pulsante destro del mouse su TestDP.TT e quindi scegliere **Esegui strumento personalizzato**.
     ...
     Per [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] gli utenti, TestDP.txt potrebbe non essere visualizzato in **Esplora soluzioni** per impostazione predefinita. Per visualizzare tutti i file assegnati al progetto, aprire il menu **progetto** e fare clic su **Mostra tutti i file**.
...
1. Sostituire il codice in TestDP.tt con il seguente. Il codice HTML è evidenziato. Assicurarsi di sostituire la stringa `YOUR PATH` con il percorso del file di DocFile.xml.
