...
2. Aggiungere i riferimenti seguenti al progetto.
   ...
   |                                                                                                    Informazioni di riferimento                                                                                                    |                                                                                                  Operazioni consentite                                                                                                  |
   |-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   ...
   |                                                                                  Microsoft.VisualStudio.Modeling.Sdk.[versione]                                                                                  | Definire i gestori eventi del modello.<br /><br /> Incapsulare la serie di modifiche nel modello. Per altre informazioni, vedere [collegare aggiornamenti di modelli UML tramite transazioni](../modeling/link-uml-model-updates-by-using-transactions.md). |
...
### <a name="navigating-and-updating-the-model"></a>Esplorazione e aggiornamento del modello
 Gli elementi del modello UML sono tutti disponibili tramite l'API. Dalla selezione corrente o dalla radice del modello, è possibile accedere a tutti gli altri elementi. Per altre informazioni, vedere [esplorare il modello UML](../modeling/navigate-the-uml-model.md) e [programmare con l'API UML](../modeling/programming-with-the-uml-api.md).
 ...
 Per impostazione predefinita, ogni modifica apportata al metodo Execute verrà eseguita in una transazione separata. L'utente potrà annullare ogni singola modifica. Se si desidera raggruppare le modifiche in una singola transazione, utilizzare un <xref:Microsoft.VisualStudio.Modeling.ExtensionEnablement.ILinkedUndoTransaction> come descritto in [collegare gli aggiornamenti del modello UML utilizzando le transazioni](../modeling/link-uml-model-updates-by-using-transactions.md).
...
3. Aprire il menu di scelta rapida in qualsiasi punto del diagramma. Il comando dovrebbe essere visualizzato nel menu.
    ...
    - Il `QueryStatus` metodo non imposta `command` .`Enabled` o `Visible` su `false`.
...
## <a name="example"></a><a name="MenuExample"></a>Esempio
