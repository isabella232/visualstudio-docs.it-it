...
[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus fornisce un metodo per la creazione di collegamenti tra i modelli e da altri strumenti ai modelli. È ad esempio possibile collegare modelli di linguaggio specifico di dominio e modelli UML (Unified Modeling Language). È anche possibile creare un set integrato di DSL.
...
1. Scaricare e installare l'estensione ModelBus di Visual Studio, se non è già installata. Per altre informazioni, vedere [SDK di visualizzazione e modellazione](https://www.visualstudio.com/).
...
### <a name="making-sure-that-elements-can-be-referenced"></a>Verificare che sia possibile fare riferimento agli elementi
 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] Per impostazione predefinita, gli adapter ModelBus utilizzano il GUID di un elemento per identificarlo. Questi identificatori devono quindi essere persistenti nel file del modello.
...
2. Nel Finestra Proprietà, con la proprietà del dominio selezionata, impostare **tipo** su `ModelBusReference` .
...
3. Nel progetto di debug nell'istanza sperimentale di [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], aggiungere i file che sono istanze di ogni DSL.
    ...
    > [!NOTE]
    > [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus può risolvere solo i riferimenti a modelli che sono elementi nella stessa [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] soluzione. Ad esempio, non è possibile creare un riferimento a un file di modello in un'altra parte del file system.
