...
In Visual Studio è possibile adattare gli elementi di modello UML standard, come classi e componenti, in modo da personalizzarli per scopi specifici. A un elemento di modello è possibile applicare uno *stereotipo*, che può modificare l'elenco di proprietà dell'elemento. Gli stereotipi vengono definiti all'interno di raccolte chiamate *profili*.
