...
## <a name="creating-a-model-bus-adapter-for-access-from-text-templates"></a>Creazione di una scheda bus di modello per l'accesso da modelli di testo
...
1. Se la soluzione DSL di destinazione non dispone di un progetto **ModelBusAdapter** , crearne uno usando la procedura guidata di estensione ModelBus:
    ...
    1. Scaricare e installare l'estensione ModelBus di [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], se non è già stato fatto. Per altre informazioni, vedere [SDK di visualizzazione e modellazione](http://go.microsoft.com/fwlink/?LinkID=185579).
...
## <a name="writing-a-text-template-that-can-resolve-modelbus-references"></a>Scrittura di un modello di testo in grado di risolvere riferimenti ModelBus
...
```
 ...
 Quando viene eseguito questo modello di testo, la direttiva `SourceDsl` carica il file `Sample.source`. Il modello può accedere agli elementi del modello, a partire da `this.ModelRoot`. Il codice può usare le classi di dominio e le proprietà di tale DSL.
...
1. Costruire due DSLs. Un DSL, il *consumer*, ha una proprietà `ModelBusReference` che può fare riferimento all'altro DSL, il *provider*.
...
2. Nel diagramma di definizione DSL, fare clic con il pulsante destro del mouse su una parte vuota del diagramma che non si trova nella parte superiore, quindi fare clic su **Abilita ModelBus**.
   ...
   - Se non viene visualizzato **Abilita ModelBus**, è necessario scaricare e installare l'estensione ModelBus VMSDK. Trovarlo nel sito di VMSDK: [visualizzazione e modellazione dell'SDK](http://go.microsoft.com/fwlink/?LinkID=185579).
...
#### <a name="to-create-a-modelbus-adapter-that-is-configured-for-text-templates"></a>Per creare un adapter ModelBus configurato per i modelli di testo
...
2. Nel progetto DSL aggiungere un riferimento all'assembly DSL MBProvider. Fare clic con il pulsante destro del mouse su `MBConsumer\Dsl\References` e quindi scegliere **Aggiungi riferimento**. Nella scheda **Sfoglia** individuare `MBProvider\Dsl\bin\Debug\Company.MBProvider.Dsl.dll`
...
1. Nell'istanza sperimentale di [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] aprire un file di modello di testo di esempio. Impostarne il contenuto come segue.
    ...
    2. È possibile accedere al modello consumer nel modo consueto tramite il processore di direttiva generato in tale DSL.

    3. Le direttive assembly e Import devono essere in grado di accedere a ModelBus e ai tipi del DSL del provider.
