...
1. Se la soluzione DSL di destinazione non dispone di un progetto **ModelBusAdapter** , crearne uno usando la procedura guidata di estensione ModelBus:
    ...
    1. Scaricare e installare l' [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] estensione ModelBus, se non è già stato fatto. Per altre informazioni, vedere [SDK di visualizzazione e modellazione](https://www.visualstudio.com/).
...
2. Nel diagramma di definizione DSL, fare clic con il pulsante destro del mouse su una parte vuota del diagramma che non si trova nella parte superiore, quindi fare clic su **Abilita ModelBus**.
   ...
   - Se non viene visualizzato **Abilita ModelBus**, è necessario scaricare e installare l'estensione ModelBus VMSDK. Trovarlo nel sito di VMSDK: [visualizzazione e modellazione dell'SDK](https://www.visualstudio.com/).
...
1. Nella soluzione MBConsumer, premere CTRL + F5. [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]Nel progetto **MBConsumer\Debugging** verrà aperta un'istanza sperimentale di.
...
1. Nell'istanza sperimentale di [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] aprire un file di modello di testo di esempio. Impostarne il contenuto come segue.
    ...
    1. `hostSpecific` `inherits` È necessario impostare gli attributi e della `template` direttiva.
