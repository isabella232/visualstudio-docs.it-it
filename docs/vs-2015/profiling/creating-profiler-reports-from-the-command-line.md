...
Lo strumento da riga di comando **VSPerfReport** consente di creare report con estensione XML o con valori delimitati da virgole (CSV) dai file di dati di profilatura (con estensione vsp). I tipi di report VSPerfReport sono molto simili alle visualizzazioni basate su tabella dell'interfaccia di [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. È possibile filtrare il report per visualizzare solo il codice o per visualizzare solo un segmento del file di dati di profilatura. Per altre informazioni, vedere [VSPerfReport](../profiling/vsperfreport.md).  
...
|----------|---------------------|  
|**Creazione di un report di base.** Creare tutti i tipi di report VSPerfReport o solo un sottoinsieme.|-   [Creazione di report di base](../profiling/creating-basic-profiling-reports-from-the-command-line.md)|  
