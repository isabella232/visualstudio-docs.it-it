...
La versione più recente di questo argomento è reperibile in [procedura: collegare il Profiler a un'applicazione .NET Framework autonoma per raccogliere dati di memoria tramite la riga di comando](https://docs.microsoft.com/visualstudio/profiling/how-to-attach-the-profiler-to-a-dotnet-framework-stand-alone-application-to-collect-memory-data-by-using-the-command-line).  
  
Questo argomento descrive come usare gli strumenti da riga di comando disponibili negli strumenti di profilatura di [!INCLUDE[vsPreShort](../includes/vspreshort-md.md)] per connettere il profiler a un'applicazione NET Framework autonoma in esecuzione (client) e raccogliere dati di memoria.  
