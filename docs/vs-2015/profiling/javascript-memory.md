...
- Ridurre le informazioni non utilizzabili nei dati dell'heap JavaScript.  
   ...
   Gli oggetti che non vengono creati direttamente nel codice dell'app vengono esclusi automaticamente. È anche possibile filtrare i dati in base al nome dell'oggetto.  
  ...
  [Visualizza il riepilogo dell'utilizzo della memoria in tempo reale](#LiveMemory)   
  [Visualizzare un riepilogo degli snapshot](#SnapshotSummary)   
  [Visualizza i dettagli dello snapshot](#SnapshotDetails)   
  [Visualizzare le differenze di uno snapshot](#SnapshotDiff)   
  [Visualizza oggetti per dominatore](#FoldObjects)   
  [Filtrare i dati in base all'identificatore](#Filter)   
  [Trovare un oggetto nella struttura ad albero di oggetti](#ShowInRootsView)   
  [Visualizza riferimenti a oggetti condivisi](#References)   
  [Mostra oggetti incorporati](#BuiltInValues)   
  [Salvare i file della sessione di diagnostica](#Save)   
  [Associare il codice sorgente ai dati sull'utilizzo della memoria](#JSConsoleCommands)   
  [Suggerimenti per l'identificazione dei problemi di memoria](#Tips)  
  
## <a name="run-the-javascript-memory-analyzer"></a><a name="Run"></a> Eseguire JavaScript Memory Analyzer  
...
3. Nella barra dei menu scegliere **Debug** e quindi **Profiler prestazioni...**.  
     ...
     ![Modificare la destinazione di analisi](../profiling/media/js-tools-target.png "JS_Tools_Target")  
...
## <a name="check-memory-usage"></a><a name="Check"></a> Controllare l'utilizzo della memoria  
...
- [Visualizza i dettagli dello snapshot](#SnapshotDetails). Mostra in dettaglio i dati sull'utilizzo della memoria per un singolo snapshot.  
...
## <a name="view-live-memory-usage-summary"></a><a name="LiveMemory"></a> Visualizza il riepilogo dell'utilizzo della memoria in tempo reale  
...
## <a name="view-a-snapshot-summary"></a><a name="SnapshotSummary"></a> Visualizzare un riepilogo degli snapshot  
...
> JavaScript Memory Analyzer forza un'operazione di Garbage Collection prima di ogni snapshot, In questo modo otterrai risultati più coerenti tra le esecuzioni.  
 ...
 ![Riepilogo di snapshot](../profiling/media/js-mem-snapshot-summary.png "JS_Mem_Snapshot_Summary")  
...
## <a name="view-snapshot-details"></a><a name="SnapshotDetails"></a> Visualizzare i dettagli dello snapshot  
 ...
 ![Visualizzazione dei dettagli di snapshot con problemi potenziali](../profiling/media/js-mem-snapshot-details.png "JS_Mem_Snapshot_Details")  
...
## <a name="view-a-snapshot-diff"></a><a name="SnapshotDiff"></a> Visualizzare le differenze di uno snapshot  
...
- **Oggetti aggiunti tra lo snapshot # \<number> e \<number> #**. Questo filtro mostra tutti gli oggetti aggiunti all'heap rispetto allo snapshot precedente.  
  
- **Tutti gli oggetti nello snapshot \<number> n**.. Con questa impostazione di filtro non viene escluso nessuno oggetto sull'heap.  
  
  Per visualizzare i riferimenti a oggetti che non corrispondono al filtro **ambito** corrente, selezionare **Mostra riferimenti non corrispondenti** nell'elenco delle impostazioni elenco a ![discesa&#45;in Memory Analyzer](../profiling/media/js-mem-settings.png "JS_Mem_Settings") nell'angolo superiore destro del riquadro. Se abiliti questa impostazioni, i riferimenti non corrispondenti saranno visualizzati in testo grigio.  
...
## <a name="view-objects-by-dominator"></a><a name="FoldObjects"></a> Visualizzare gli oggetti in base al dominatore  
 Nelle visualizzazioni Tipi e Dominatori puoi scegliere se visualizzare gli oggetti ridotti nei rispettivi dominatori (visualizzazione predefinita della scheda Dominatori). Se è selezionata questa visualizzazione, solo i dominatori saranno mostrati nella visualizzazione di primo livello degli oggetti. Gli oggetti discendenti da oggetti non globali sono nascosti dalla visualizzazione di primo livello. Per alcune app, questo può chiarire quali oggetti causano una perdita di memoria riducendo il rumore nei dati.  
  
 Per attivare o disattivare la visualizzazione degli oggetti in base al dominatore, scegli il pulsante **Riduci gli oggetti in base a quello dominante** . ![Riduzione di oggetti nei relativi dominatori](../profiling/media/js-mem-fold-objects.png "JS_Mem_Fold_Objects")  
  
 Per altre informazioni sui dominatori, vedi [Visualizzare i dettagli dello snapshot](#SnapshotDetails).  
  
## <a name="filter-data-by-identifier"></a><a name="Filter"></a> Filtrare i dati in base all'identificatore  
...
## <a name="find-an-object-in-the-object-tree"></a><a name="ShowInRootsView"></a> Trovare un oggetto nella struttura ad albero di oggetti  
...
## <a name="view-shared-object-references"></a><a name="References"></a> Visualizza riferimenti a oggetti condivisi  
...
## <a name="show-built-in-objects"></a><a name="BuiltInValues"></a> Mostra oggetti incorporati  
 ...
 Per visualizzare questi oggetti, scegliere **Mostra incorporati** nell'elenco impostazioni ![&#45;elenco a discesa nell'analizzatore di memoria](../profiling/media/js-mem-settings.png "JS_Mem_Settings") nell'angolo superiore destro del riquadro.  
  
## <a name="save-diagnostic-session-files"></a><a name="Save"></a> Salvare file delle sessioni di diagnostica  
...
## <a name="associate-source-code-with-memory-usage-data"></a><a name="JSConsoleCommands"></a> Associare il codice sorgente ai dati sull'utilizzo della memoria  
...
> Usa `console.takeHeapSnapshot` per velocizzare l'analisi quando si ripetono gli scenari di utilizzo della memoria.  
 ...
 Questi comandi generano un'eccezione se li aggiungi all'app ed esegui quest'ultima all'esterno di JavaScript Memory Analyzer. Puoi tuttavia verificare se i comandi sono presenti prima di usarli. (I comandi non sono presenti all'inizio della fase di avvio della sessione.) Per verificare se è possibile chiamare in modo sicuro `takeHeapSnapshot` , usare il codice seguente:  
...
## <a name="tips-for-identifying-memory-issues"></a><a name="Tips"></a> Suggerimenti per l'identificazione dei problemi di memoria  
  
- Seguire il flusso di lavoro descritto in [isolare una perdita di memoria](#Isolate) e usare gli **oggetti rimasti da \<number> snapshot #** Filter in una visualizzazione differenze per identificare i probabili candidati per le perdite di memoria.  
...
- Leggere l'argomento [Gestione della memoria in app di Windows Store](https://msdn.microsoft.com/magazine/jj651575.aspx).  
