...
La versione più recente di questo argomento è reperibile in [procedura: instrumentare un'applicazione Web ASP.NET in modo dinamico compilato e raccogliere dati di memoria tramite la riga di comando del Profiler](https://docs.microsoft.com/visualstudio/profiling/how-to-instrument-a-dynamically-compiled-aspnet-web-application-and-collect-memory-data-by-using-the-profiler-command-line).  
  
Questo argomento illustra come usare gli strumenti della riga di comando disponibili negli strumenti di profilatura di [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] per raccogliere dati dettagliati dell'allocazione di memoria .NET della durata degli oggetti per un'applicazione Web [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] compilata in modo dinamico tramite il metodo di profilatura della strumentazione.  
...
1.  Avvia il profiler. Tipo:  
    ...
    |Opzione|Descrizione|  
    |------------|-----------------|  
    |[/user](../profiling/user-vsperfcmd.md) **:**[`Domain`**\\**]`UserName`|Specifica il dominio facoltativo e il nome utente dell'account proprietario del processo di lavoro [!INCLUDE[vstecasp](../includes/vstecasp-md.md)]. Questa opzione è obbligatoria se il processo è in esecuzione come utente diverso dall'utente connesso. Il nome è elencato nella colonna Nome utente nella scheda Processi di Gestione attività di Windows.|  
...
2.  Avviare l'applicazione Web [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] nel modo usuale.  
...
## <a name="ending-the-profiling-session"></a>Arresto della sessione di profilatura  
 Per terminare una sessione di profilatura, chiudere l'applicazione Web [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] di destinazione, arrestare Internet Information Services (IIS) per interrompere il processo profilato e arrestare il profiler. Riavviare quindi IIS.  
...
## <a name="restoring-the-application-and-computer-configuration"></a>Ripristino della configurazione dell'applicazione e del computer  
 Dopo aver completato l'attività di profilatura, sostituire il file web.config, cancellare le variabili di ambiente di profilatura e riavviare il computer per ripristinare gli stati originali del server e dell'applicazione [!INCLUDE[vstecasp](../includes/vstecasp-md.md)].  
