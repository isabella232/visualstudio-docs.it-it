...
### <a name="accessing-the-environment-font"></a>Accesso al tipo di carattere ambiente
 Nel codice nativo o WinForms, è possibile accedere al tipo di carattere dell'ambiente chiamando il metodo **IUIHostLocale:: GetDialogFont** dopo aver eseguito una query sull'interfaccia dal servizio SID_SUIHostLocale.

 Per Windows Presentation Foundation (WPF), derivare la classe della finestra di dialogo dalla classe **DialogWindow** della shell invece della classe della **finestra** di WPF.
...
```
 ...
 Per visualizzare la finestra di dialogo, chiamare "**ShowModal ()** " sulla classe su **ShowDialog ()** . **ShowModal ()** imposta lo stato modale corretto nella shell, assicura che la finestra di dialogo sia centrata nella finestra padre e così via.
 ...
 **ShowModal** restituisce un valore bool? (Nullable booleano) con **DialogResult**, che può essere usato se necessario. Il valore restituito è true se la finestra di dialogo è stata chiusa con **OK**.

 Se è necessario visualizzare un'interfaccia utente di WPF che non è una finestra di dialogo ed è ospitata nel relativo oggetto **HwndSource**, ad esempio una finestra popup o una finestra figlio WPF di una finestra della finestra padre Win32/WinForms, sarà necessario impostare **FontFamily** e **FontSize** sull'elemento radice dell'elemento WPF. La shell imposta le proprietà nella finestra principale, ma non verranno ereditate dopo un HWND. La shell fornisce le risorse a cui è possibile associare le proprietà, come indicato di seguito:
...
### <a name="BKMK_Formatting"></a>Guida di riferimento alla formattazione (ridimensionamento/grassetto)
...
> **Nota: prima di applicare la formattazione, assicurarsi di seguire le indicazioni disponibili in [stile testo](../../extensibility/ux-guidelines/fonts-and-formatting-for-visual-studio.md#BKMK_TextStyle).**
 ...
 Dove "incorporati vsui" è un riferimento allo spazio dei nomi Microsoft. VisualStudio. Shell:
 ...
 **Usare per:** (rare) interfaccia utente personalizzata, ad esempio nella pagina iniziale

 **Codice procedurale:** Dove "textBlock" è un TextBlock definito in precedenza e "label" è un'etichetta definita in precedenza.
 ...
 **Visualizzato come:** 28 PT Segoe UI chiaro

 **Usare per: titoli di finestre di firma di** grandi dimensioni, intestazione principale nei report
 ...
 **Usare per:** sottointestazioni, titoli nelle finestre di dialogo piccole e medie
 ...
 **Visualizzato come:** 14 PT Segoe UI

 **Usare per:** intestazioni di sezione nell'interfaccia utente dei documenti e nei report
 ...
 **Usare for:** sottointestazioni più piccole nelle finestre di dialogo della firma e interfaccia utente di Document well
 ...
 **Visualizzato come:** 11 PT Segoe UI

 **Usare per:** intestazioni di sezione nelle finestre di dialogo della firma, nodi principali nella visualizzazione albero, spostamento verticale con tabulazioni
 ...
 **Visualizzato come:** grassetto 9 PT Segoe UI

 **Usare per:** etichette e sottointestazioni nelle finestre di dialogo della firma, nei report e nell'interfaccia utente dei documenti
 ...
 **Impostazioni di tipi di carattere e colori nella finestra di dialogo Opzioni > Opzioni**
...
## <a name="BKMK_TextStyle"></a>Stile testo
...
|Articoli, a meno che non si tratti della prima parola nel titolo|un oggetto, un oggetto, il|
...
- Quando si usa il colore nelle intestazioni, è necessario rispettare le [linee guida](https://msdn.microsoft.com/library/dn742482.aspx)per i colori di Windows, inclusi il rapporto di contrasto e altre considerazioni sull'accessibilità.
