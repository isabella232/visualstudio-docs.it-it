...
Le sezioni seguenti forniscono elenchi di voci del registro di sistema per le varie opzioni di servizio di linguaggio disponibili in [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] .  
  
 Nel seguente elenco di voci del registro di sistema, *vs reg root* è uguale a HKEY_LOCAL_MACHINE \Software\Microsoft\VisualStudio \\ *x. y*, dove *x. y* è il [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] numero di versione.  
