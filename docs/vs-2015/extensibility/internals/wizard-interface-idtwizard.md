...
```  
 ...
 Le informazioni seguenti descrivono l' <xref:EnvDTE.IDTWizard> interfaccia che deve essere implementata dalle procedure guidate per lavorare nell' [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] IDE. L'IDE chiama il <xref:EnvDTE.IDTWizard.Execute%2A> metodo nella procedura guidata, passando il codice seguente:  
