...
2. Registrare lo spazio dei nomi del partner nella chiave del registro di sistema della Guida 2,1:
   ...
   - Per i sistemi operativi a 32 bit:
      ...
      <em> \\ Spazio dei nomi \> </em> HKEY_LOCAL_MACHINE \software\microsoft\help\v2.1\partner<

      "location" = "offline"

   - Per i sistemi operativi a 64 bit:

      <em> \\ Spazio dei nomi \> </em> HKEY_LOCAL_MACHINE \software\wow6432node\microsoft\help\v2.1\partner<
...
|\< meta name="Description" content="[topic description]" />|Fornisce un breve riepilogo del contenuto di questo argomento. Se questo tag viene usato in un argomento, deve essere usato una sola volta. A questa proprietà è possibile accedere direttamente dalla libreria di query; non è archiviato nel file di indice.|
 meta Name = "Microsoft. Help. TocParent" Content = "[parent_Id]"/>|Specifica l'argomento padre di questo argomento nel sommario. Questo tag è obbligatorio e deve essere usato una sola volta in un argomento. Il valore è Microsoft.Help.Id dell'elemento padre. Un argomento può avere una sola posizione nel sommario. "-1" è considerato l'ID argomento per la radice del sommario. In [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] Questa pagina è Home page Help Viewer. Questo è lo stesso motivo per cui si aggiunge in modo specifico TocParent =-1 ad alcuni argomenti per assicurarsi che vengano visualizzati al livello principale. Il Visualizzatore della guida home page è una pagina di sistema e pertanto non può essere sostituita. Se un VSP tenta di aggiungere una pagina con ID-1, è possibile che venga aggiunta al set di contenuti, ma Help Viewer utilizzerà sempre la pagina System-Help Viewer Home|
...
- Branding.xml: stringhe utilizzate in modo coerente nel contenuto del catalogo.  Nota: per gli elementi di testo di localizzazione di Visual Studio nella branding.xml, includere _locID = " \<unique value> "
...
- Printer. CSS: contiene gli elementi CSS per il rendering in cui SelfBranded = false
  ...
  Con la personalizzazione dei file CSS sono incluse le definizioni per la presentazione degli argomenti di Visual Studio. si noti che il file di personalizzazione. CSS contenuto nell'Branding_ \<locale> . mshc del servizio del pacchetto potrebbe cambiare.
...
2. [Visual Studio 2013 Redist della shell isolata](https://aka.ms/VS2013/IsoShell-LP/all)
   ...
   **Panoramica**
   ...
   La [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] Shell è una versione dell' [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] IDE in cui è possibile basare un'applicazione. Tali applicazioni contengono la shell isolata insieme alle estensioni create. Usare i modelli di progetto della shell isolata, inclusi nell' [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] SDK, per compilare le estensioni.

   I passaggi di base per la creazione di un'applicazione basata su Shell isolata e la relativa guida:

3. Ottenere la [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] Shell ridistribuibile ISO (download Microsoft).
...
- HKLM\SOFTWARE\Wow6432Node\Microsoft\Help\v2.1\Catalogs\VisualStudio12\en-US
   ...
   Chiave: CatalogName valore stringa: [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] documentazione
...
12. Creare un archivio di contenuti:
     ...
     Per la [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] Shell integrata, creare la cartella C:\ProgramData\Microsoft\HelpLibrary2\Catalogs\VisualStudio12

13. Creare CatalogType.xml e aggiungere all'archivio contenuto (passaggio precedente) contenente:
...
14. Aggiungere le chiavi del registro di sistema seguenti:
     ...
     Chiave: CatalogName valore stringa: [!INCLUDE[vs_dev12](../../includes/vs-dev12-md.md)] documentazione. Per la shell ISO, questo è il nome del catalogo.
