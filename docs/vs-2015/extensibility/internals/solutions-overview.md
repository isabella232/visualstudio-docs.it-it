...
ms.locfileid: "90840259"
---
# <a name="solutions-overview"></a>Panoramica delle soluzioni
[!INCLUDE[vs2017banner](../../includes/vs2017banner.md)]

Una soluzione è un raggruppamento di uno o più progetti che interagiscono tra loro per creare un'applicazione. Le informazioni sul progetto e sullo stato relative alla soluzione vengono archiviate in due diversi file di soluzione. Il file della soluzione (. sln) è basato su testo e può essere inserito nel controllo del codice sorgente e condiviso tra gli utenti. Il file delle opzioni utente della soluzione (. suo) è binario. Di conseguenza, il file con estensione suo non può essere inserito nel controllo del codice sorgente e contiene informazioni specifiche dell'utente.  
...
3. Se viene caricato un pacchetto VSPackage, l'ambiente chiama l' `QueryInterface` <xref:Microsoft.VisualStudio.Shell.Interop.IVsPackage> interfaccia per l'interfaccia richiesta dal pacchetto VSPackage.  
   ...
   1. Quando si esegue la lettura da un file con estensione sln, l'ambiente chiama `QueryInterface` `IVsPersistSolutionProps` .  
  
   2. Quando si esegue la lettura da un file con estensione suo, l'ambiente chiama `QueryInterface` `IVsPersistSolutionOpts` .  
