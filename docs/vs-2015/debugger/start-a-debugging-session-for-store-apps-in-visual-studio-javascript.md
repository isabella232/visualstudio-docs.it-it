---
title: Avviare una sessione di debug per le applicazioni dello Store (JavaScript) Documenti Microsoft
ms.date: 11/15/2016
...
> [!NOTE]
> Per le app scritte in XAML e in Visual C, Visual C' o Visual Basic, vedere Avviare una sessione di [debug (VB, C'è, C' e XAML)](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md)
...
- [Scegliere le opzioni di configurazione della build](#BKMK_Choose_the_build_configuration_options)
...
- [(Facoltativo) Disabilitare i loopback di rete](#BKMK__Optional__Disable_network_loopbacks)
  ...
  [Avviare la sessione di debugStart the debugging session](#BKMK_Start_the_debugging_session)

- [Avviare il debug (F5)](#BKMK_Start_debugging__F5_)

- [Avviare il debug (F5) ma ritardare l'avvio dell'app](#BKMK_Start_debugging__F5__but_delay_the_app_start)

  [Avviare un'app installata nel debuggerStart an installed app in the debugger](#BKMK_Start_an_installed_app_in_the_debugger)

  [Connettere il debugger a un'app in esecuzione](#BKMK_Attach_the_debugger_to_a_running_app_)
...
## <a name="configure-the-debugging-session"></a><a name="BKMK_Configure_the_debugging_session"></a>Configurare la sessione di debug
 Dato che lo script non è compilato, sono escluse le impostazioni relative a configurazione e piattaforma di compilazione. Se si sta eseguendo il debug di un componente gestito o di C, impostare **Configurazione** su **Debug** e scegliere la piattaforma di destinazione nella finestra di dialogo **Configurazione.**

### <a name="open-the-debugging-property-page-for-the-project"></a><a name="BKMK_Open_the_debugging_property_page_for_the_project"></a>Aprire la pagina delle proprietà di debug per il progetto
...
### <a name="choose-the-deployment-target"></a><a name="BKMK_Choose_the_deployment_target"></a>Scegliere la destinazione di distribuzione
 Puoi distribuire ed eseguire il debug di un'app nel computer Visual Studio, nel simulatore di Visual Studio sul computer locale o in un computer remoto. Scegliere la destinazione dal **Debugger per avviare** l'elenco delle proprietà **Debug** per il progetto.

 ![Si applica solo a Windows](../debugger/media/windows-only-content.png "windows_only_content")

 Per un'app di Windows Store, scegli una di queste opzioni dall'elenco Dispositivo di **destinazione:**
...
- Scegliere la freccia giù nella casella **Nome computer** e scegliere ** \<Individua...>**. Scegliere quindi il computer remoto dalla finestra di dialogo **Seleziona connessione debugger remoto.**
  ...
  Per un'app di Windows Store Phone, scegli **Dispositivo** o uno degli emulatori dall'elenco Dispositivi di **destinazione.**
...
### <a name="choose-the-debugger-to-use"></a><a name="BKMK_Choose_the_debugger_to_use"></a> Scegliere il debugger da utilizzare
 ...
 Scegliere uno dei debugger dall'elenco **Tipo debugger:**
...
### <a name="optional-delay-starting-the-app-in-the-debug-session"></a><a name="BKMK__Optional__Delay_starting_app_in_the_debug_session"></a>(Facoltativo) Ritardo dell'avvio dell'app nella sessione di debug
 ...
 Specificare se ritardare l'avvio dell'app nell'elenco **Avvia applicazione** della pagina delle proprietà **Debug** del progetto di app. Scegliere una delle opzioni seguenti:

- Scegliere **No** per ritardare l'avvio dell'app.

- Scegliere **Sì** per avviare immediatamente l'app.

### <a name="optional-disable-network-loopbacks"></a><a name="BKMK__Optional__Disable_network_loopbacks"></a>(Facoltativo) Disabilitare i loopback di rete
 ![Si applica solo a Windows](../debugger/media/windows-only-content.png "windows_only_content")
 ...
 Per rimuovere l'esenzione di loopback di rete, scegliere **No** dall'elenco **Consenti loopback di rete** nella pagina delle proprietà **Debug.**

## <a name="start-the-debugging-session"></a><a name="BKMK_Start_the_debugging_session"></a> Avviare la sessione di debug

### <a name="start-debugging-f5"></a><a name="BKMK_Start_debugging__F5_"></a>Avviare il debug (F5)
...
3. Avvia l'app dal menu Start, da un contratto di esecuzione o da un'altra routine.
   ...
   . Per altre informazioni sul debug di attività in background, vedere Attivare gli eventi di [sospensione, ripresa e in background per Windows Store.](../debugger/how-to-trigger-suspend-resume-and-background-events-for-windows-store-apps-in-visual-studio.md)
...
## <a name="start-an-installed-app-in-the-debugger"></a><a name="BKMK_Start_an_installed_app_in_the_debugger"></a> Avviare un'app installata nel debugger
 ...
 L'app può essere installata nel dispositivo locale oppure in un dispositivo remoto.  Puoi avviare l'app immediatamente oppure impostarla per l'esecuzione nel debugger quando viene avviata da un altro processo o metodo, ad esempio dal menu Start o da un contratto di attivazione. Puoi anche impostarne l'esecuzione in modalità debug per eseguire il debug di un processo in background senza avviare l'app. Per altre informazioni, vedere [Attivare gli eventi di sospensione, ripresa e in background per Windows Store).](../debugger/how-to-trigger-suspend-resume-and-background-events-for-windows-store-apps-in-visual-studio.md)
...
### <a name="set-the-app-to-run-in-debug-mode"></a><a name="BKMK_Set_the_app_to_run_in_debug_mode"></a>Impostare l'esecuzione dell'app in modalità di debugSet the app to run in debug mode

1. Installa Visual Studio Remote Tools sul dispositivo in cui è installata l'app. Consultate [Installazione degli strumenti remoti.](https://msdn.microsoft.com/library/windows/apps/hh441469.aspx#BKMK_Installing_the_Remote_Tools)
...
### <a name="attach-the-debugger"></a><a name="BKMK_Attach_the_debugger"></a>Collegare il debugger
...
2. Per connetterti a un'app in un dispositivo remoto, specifica il dispositivo remoto nella casella **Qualificatore** . È possibile:
   ...
   - Scegliere la freccia giù nella casella **Qualificatore** e scegliere il dispositivo da un elenco di dispositivi a cui è stato collegato in precedenza.

   - Scegliere **Trova** per scegliere il dispositivo da un elenco di dispositivi nella subnet locale.
...
4. Nell'elenco **Processi disponibili** scegliere il processo **wwahost.exe** appropriato. Usa la colonna **Titolo** per identificare la tua app.
...
## <a name="see-also"></a>Vedere anche
 Guida introduttiva al [controllo dell'esecuzione in una sessione di debug (JavaScript):](../debugger/control-execution-of-a-store-app-in-a-visual-studio-debug-session-for-windows-store-apps-javascript.md) eseguire il debug di eventi di sospensione, ripresa e background HTML e CSS per Windows Store) Eseguire il debug delle app in Visual StudioControl execution in a debug session [(JavaScript)](../debugger/debug-store-apps-in-visual-studio.md) [Quickstart: Debug HTML and CSS](../debugger/quickstart-debug-html-and-css.md) Trigger [suspend, resume, and background events for Windows Store)](../debugger/how-to-trigger-suspend-resume-and-background-events-for-windows-store-apps-in-visual-studio.md) Debug apps in Visual Studio
