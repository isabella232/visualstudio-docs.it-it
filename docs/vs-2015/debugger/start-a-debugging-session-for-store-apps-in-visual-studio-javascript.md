...
> [!NOTE]
> Per le app scritte in XAML e C#Visual, C++Visual o Visual Basic, vedere [avviare una sessione di debug (VB C#, C++ e XAML)](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md)
...
- [Opzionale Ritardare l'avvio dell'app nella sessione di debug](#BKMK__Optional__Delay_starting_app_in_the_debug_session)
...
2. Se la soluzione contiene progetti sia per le app di Windows Store sia per le app di Windows Store Phone, assicurati che il progetto di cui vuoi eseguire il debug sia il progetto di avvio. In Esplora soluzioni selezionare il progetto, quindi scegliere **Imposta come progetto di avvio** dal menu di scelta rapida.

3. Premere F5.

   ![Si applica solo a Windows Phone](../debugger/media/phone-only-content.png "phone_only_content")

   Visual Studio compila e avvia l'app con il debugger collegato. L'esecuzione continua fino a raggiungere un punto di interruzione, se sospendi manualmente l'esecuzione, si verifica un'eccezione non gestita o l'app termina. Per altre informazioni, vedere [Guida introduttiva: eseguire il debug di HTML e CSS](../debugger/quickstart-debug-html-and-css.md).

## <a name="BKMK_Configure_the_debugging_session"></a> Configurare la sessione di debug
 Dato che lo script non è compilato, sono escluse le impostazioni relative a configurazione e piattaforma di compilazione. Se si esegue il debug C++ di un componente o gestito, impostare la **configurazione** su **debug** e scegliere la piattaforma di destinazione dalla finestra di dialogo di **configurazione** .
...
- Fare clic sulla freccia in giù nella casella **nome computer** e scegliere **\<individuare... >** . Scegliere quindi la finestra di dialogo computer remoto dalla finestra di dialogo **Seleziona connessione debugger remoto** .
...
### <a name="BKMK__Optional__Delay_starting_app_in_the_debug_session"></a>Opzionale Ritardare l'avvio dell'app nella sessione di debug
...
### <a name="BKMK_Start_debugging__F5_"></a> Avviare i debug (F5)
 Quando si sceglie **Avvia debug** dal menu **debug** (tastiera: F5), Visual Studio avvia l'app con il debugger collegato. L'esecuzione continua fino a raggiungere un punto di interruzione, se sospendi manualmente l'esecuzione, si verifica un'eccezione non gestita o l'app termina.
...
4. Al prompt di PowerShell immettere `Enable-AppxDebug` *PackageFullName* dove *PackageFullName* è l'identificatore PackageFullName dell'app.
...
4. Nell'elenco **processi disponibili** scegliere il processo **WWAHost. exe** appropriato. Usare la colonna **title** per identificare l'app.
