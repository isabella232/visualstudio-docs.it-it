...
-   Se `removeEventListener` non riesce a rimuovere il listener specificato, potrebbe venire chiamato in un'istanza diversa dell'oggetto. È possibile usare la [metodo bind (Function)](~/E:/Repos/visualstudio-docs-pr/scripting-docs/javascript/reference/bind-method-function-javascript.md) metodo per risolvere questo problema quando si aggiunge il listener.  
  
-   Per rimuovere un listener di eventi che è stato aggiunto usando [metodo bind (Function)](~/E:/Repos/visualstudio-docs-pr/scripting-docs/javascript/reference/bind-method-function-javascript.md) o utilizzando una funzione anonima, archivia un'istanza della funzione quando si aggiunge il listener. Ecco un metodo per usare questo modello in modo sicuro:  
