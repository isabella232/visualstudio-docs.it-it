...
2. Dopo aver scaricato il programma di installazione in un percorso di file system, eseguire " \<executable name> /Layout".
...
3. Dopo aver eseguito questo comando verrà visualizzata una finestra di dialogo che consente di modificare la cartella in cui salvare il layout di installazione offline.   Fare quindi clic sul pulsante **download** .

     Quando il download del pacchetto viene completato correttamente, viene visualizzato un messaggio che indica che l' **installazione è riuscita. Tutti i componenti specificati sono stati acquisiti correttamente.**
...
- [Installa Visual Studio](install-visual-studio-2015.md)