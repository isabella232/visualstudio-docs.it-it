...
ms.locfileid: "47527650"
---
# <a name="msbuild-task"></a>Attività MSBuild
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

La versione più recente di questo argomento è reperibile in [attività MSBuild](https://docs.microsoft.com/visualstudio/msbuild/msbuild-task).  
...
|`RunEachTargetSeparately`|Parametro `Boolean` facoltativo.<br /><br /> Se `true`, l'attività [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] richiama le destinazioni nell'elenco passato a [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] una alla volta, invece che contemporaneamente. Impostare questo parametro su `true` garantisce che le destinazioni successive vengano richiamate anche se le destinazioni richiamate prima hanno avuto esito negativo. In caso contrario, un errore di compilazione arresterà la chiamata di tutte le destinazioni successive. Il valore predefinito è `false`.|  
...
|`ToolsVersion`|Parametro `String` facoltativo.<br /><br /> Specifica la `ToolsVersion` da usare quando si compilano i progetti passati a questa attività.<br /><br /> Consente a un'attività [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] di compilare un progetto destinato a una versione di [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] diversa da quella specificata nel progetto. I valori validi sono `2.0`, `3.0` e `3.5`. Il valore predefinito è `3.5`.|  
...
## <a name="remarks"></a>Note  
 ...
 Tutte le configurazioni richieste da [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] per consentire la compilazione simultanea dei progetti, anche se coinvolgono un'infrastruttura remota (ad esempio, porte, protocolli, timeout, tentativi e così via), devono essere rese configurabili usando un file di configurazione. Se possibile, gli elementi di configurazione devono essere specificati come parametri dell'attività nell'attività `MSBuild`.  
  
 A partire da [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] 3.5, i progetti di soluzione espongono TargetOutputs da tutti i sottoprogetti compilati.  
  
## <a name="passing-properties-to-projects"></a>Passaggio delle proprietà ai progetti  
 Nelle versioni di [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] precedenti a [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] 3.5, passare set di Proprietà diversi a progetti diversi elencati nell'elemento [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] è complicato. Se si usa l'attributo Properties dell'[attività MSBuild](../msbuild/msbuild-task.md), l'impostazione viene applicata a tutti i progetti da compilare a meno che non si divida in batch l'[attività MSBuild](../msbuild/msbuild-task.md) e non si forniscano in modo condizionale proprietà diverse per ogni progetto nell'elenco di elementi.  
...
## <a name="additionalproperties-metadata"></a>Metadati AdditionalProperties  
 Si consideri lo scenario seguente in cui si compilano due file di soluzione usando l'[attività MSBuild](../msbuild/msbuild-task.md), entrambi con la configurazione per la versione, uno dei quali usa l'architettura x86 e l'altro l'architettura ia64. In [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] 2.0 è necessario creare più istanze dell'[attività MSBuild](../msbuild/msbuild-task.md): una per compilare il progetto con la configurazione per la versione con l'architettura x86 e l'altro con la configurazione per la versione con l'architettura ia64. Il file di progetto sarà come il seguente:  
