...
- Tipi di cui non è possibile creare un'istanza e che definiscono `static` `Shared` solo i metodi (in Visual Basic).

  Se si applica <xref:System.Runtime.CompilerServices.InternalsVisibleToAttribute?displayProperty=fullName> all'assembly analizzato, questa regola non verrà eseguita su alcun costruttore contrassegnato come `internal` perché non è possibile stabilire se un campo è utilizzato da un altro `friend` assembly.

  Anche se non è possibile aggirare questa limitazione nell' [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] analisi del codice, l'FxCop autonomo esterno si verificherà sui costruttori interni se ogni `friend` assembly è presente nell'analisi.
...
- Un costruttore privato per i tipi destinati alle [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] versioni 1,0 e 1,1.

- Il `static` `Shared` modificatore (in Visual Basic) per i tipi destinati a [!INCLUDE[dnprdnlong](../includes/dnprdnlong-md.md)] .
