...
Questa regola richiede che le implementazioni di <xref:System.Collections.IEnumerator> forniscano anche una versione fortemente tipizzata della proprietà <xref:System.Collections.IEnumerator.Current%2A>, in modo che gli utenti non debbano eseguire il cast del valore restituito al tipo sicuro quando usano la funzionalità fornita dall'interfaccia. Questa regola presuppone che il tipo che implementa <xref:System.Collections.IEnumerator> contenga una raccolta di istanze di un tipo più forte rispetto a <xref:System.Object>.
...
Nell'esempio seguente viene illustrato il modo corretto per implementare un tipo di <xref:System.Collections.IEnumerator> fortemente tipizzato.
