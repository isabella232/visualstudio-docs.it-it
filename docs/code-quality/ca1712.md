...
|`dotnet_code_quality.CA1712.enum_values_prefix_trigger = Heuristic` | La regola viene attivata utilizzando l'euristica predefinita FxCop, ovvero quando almeno il 75% dei valori enum inizia con il nome del tipo enum.

È possibile configurare tutte queste opzioni solo per questa regola, per tutte le regole o per tutte le regole in questa categoria (denominazione). Per altre informazioni, vedere [configurare gli analizzatori FxCop](configure-fxcop-analyzers.md).
