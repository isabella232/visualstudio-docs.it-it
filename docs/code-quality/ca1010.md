...
Nell'esempio seguente viene illustrata una classe (tipo di riferimento) che deriva dalla classe `CollectionBase` non generica, che viola questa regola.
...
Nell'esempio seguente viene corretta la violazione modificando la classe di base della raccolta dalla classe `CollectionBase` non generica alla classe generica `Collection<T>` (`Collection(Of T)` in Visual Basic).
