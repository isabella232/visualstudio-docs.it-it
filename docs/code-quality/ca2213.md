...
Un tipo che implementa <xref:System.IDisposable?displayProperty=fullName> dichiara i campi che sono di tipi che implementano anche <xref:System.IDisposable>. Il metodo <xref:System.IDisposable.Dispose%2A> del campo non viene chiamato dal metodo <xref:System.IDisposable.Dispose%2A> del tipo dichiarante.

## <a name="rule-description"></a>Descrizione della regola

Un tipo è responsabile dell'eliminazione di tutte le relative risorse non gestite. Rule CA2213 verifica se un tipo Disposable, ovvero uno che implementa <xref:System.IDisposable>, `T` dichiara un campo `F` che è un'istanza di un tipo Disposable `FT`. Per ogni campo `F` a cui viene assegnato un oggetto creato localmente all'interno dei metodi o degli inizializzatori del tipo che lo contiene `T`, la regola tenta di individuare una chiamata a `FT.Dispose`. La regola cerca i metodi chiamati da `T.Dispose` e un livello inferiore, ovvero i metodi chiamati dai metodi chiamati da `T.Dispose`.
