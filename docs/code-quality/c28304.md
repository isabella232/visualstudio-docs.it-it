---
title: C28304
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
- C28304
helpviewer_keywords:
- C28304
ms.assetid: 5bc7f593-dfc3-4172-a403-97bdf7be4885
author: mikeblome
ms.author: mblome
manager: markl
ms.workload:
- multiple
ms.openlocfilehash: 410c7acd8a7731aac7ef5bf583933fe5c89b4668
ms.sourcegitcommit: 535ef05b1e553f0fc66082cd2e0998817eb2a56a
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 10/07/2019
ms.locfileid: "72014014"
---
# <a name="c28304"></a>C28304
avviso C28304: Per C++ il parametro di riferimento < parameter_name >, è stato trovato un operatore `_Notref_` imposto in modo errato, applicato a \<token >.

 L'operatore `_Notref_` deve essere usato solo in circostanze speciali che coinvolgono C++ parametri di riferimento e solo nelle macro fornite dal sistema. Deve essere seguito immediatamente da un operatore `_Deref_` o da un'annotazione funzionale.