...
2. Aggiungere le soglie desiderate al file di testo nel formato seguente:
   ...
   In questo esempio la regola [CA1502](ca1502-avoid-excessive-complexity.md) è configurata in modo da essere attivata quando la complessità ciclomatica di un metodo è maggiore di 10.
...
1. Dalla barra dei menu scegliere **analizza** > **Calcola metrica** > codice**per i progetti selezionati**.
...
È possibile eseguire l'override del nome del file `/p:MetricsOutputFile=<filename>`di output specificando. È anche possibile ottenere dati di metrica del codice di [tipo legacy](#previous-versions) specificando `/p:LEGACY_CODE_METRICS_MODE=true`. Ad esempio:
...
Altre metriche, ad esempio `CyclomaticComplexity` e `MaintainabilityIndex` , usano le stesse formule delle versioni precedenti di *metrics. exe*, ma il nuovo strumento conta il numero `IOperations` di istruzioni per le origini logiche anziché le istruzioni del linguaggio intermedio (il). I numeri saranno leggermente diversi rispetto a quelli generati dall'IDE di Visual Studio e dalle versioni precedenti di *metrics. exe*.
