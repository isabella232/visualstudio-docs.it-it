...
Prima di .NET Framework 2,0, per passare informazioni personalizzate al gestore eventi, era necessario dichiarare un nuovo delegato che specificava una classe derivata dalla classe <xref:System.EventArgs?displayProperty=fullName>. In .NET Framework 2,0 e versioni successive, il delegato di <xref:System.EventHandler%601?displayProperty=fullName> generico consente di usare tutte le classi derivate da <xref:System.EventArgs> insieme al gestore dell'evento.
