...
In fase di esecuzione, qualsiasi chiamata <xref:System.Security.CodeAccessPermission.Assert%2A> a da codice trasparente causerà <xref:System.InvalidOperationException> la generazione di un'eccezione. Questa situazione può verificarsi in assembly trasparenti 100% e anche in assembly Transparent/Critical misti in cui un metodo o un tipo è dichiarato trasparente, ma include un'asserzione dichiarativa o imperativa.
...
Questo codice avrà esito `SecurityTestClass` negativo se è trasparente, `Assert` quando il metodo <xref:System.InvalidOperationException>genera un'eccezione.
