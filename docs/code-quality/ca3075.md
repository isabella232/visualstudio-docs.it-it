...
- Il metodo <xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType> viene richiamato con un'istanza <xref:System.Xml.XmlReaderSettings> non protetta o senza alcuna istanza.
...
- Verificare che la proprietà <xref:System.Data.DataViewManager.DataViewSettingCollectionString%2A?displayProperty=nameWithType> venga assegnata da un'origine attendibile.
...
> [!NOTE]
> Sebbene <xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType> sia il modo consigliato per creare un'istanza di <xref:System.Xml.XmlReader>, esistono differenze di comportamento rispetto a <xref:System.Xml.XmlTextReader>. Un <xref:System.Xml.XmlReader> da <xref:System.Xml.XmlReader.Create%2A> normalizza `\r\n` per `\n` nei valori XML, mentre <xref:System.Xml.XmlTextReader> conserva la sequenza di `\r\n`.
