...
Il valore di un campo `const` viene calcolato in fase di compilazione e archiviato nei metadati, che aumenta le prestazioni in fase di esecuzione quando viene confrontato con un campo di `static readonly`.
...
> [!NOTE]
> L'uso di (modificatore[const](/dotnet/csharp/language-reference/keywords/const) non è consigliato per tutti gli scenari.
