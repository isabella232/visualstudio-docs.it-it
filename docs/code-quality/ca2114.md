...
Un metodo non deve avere una sicurezza dichiarativa a livello di metodo e a livello di tipo per la stessa azione. I due controlli non vengono combinati. viene applicata solo la richiesta a livello di metodo. Se, ad esempio, un tipo richiede l'autorizzazione `X`e uno dei relativi metodi richiede l'autorizzazione `Y`, il codice non deve avere `X` di autorizzazione per eseguire il metodo.
