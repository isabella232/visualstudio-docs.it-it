...
## <a name="example"></a>Esempio
 Nell'esempio seguente, lo strumento di analisi del codice determina che *s* è null nell'istruzione `if` e viene utilizzato il corpo della `if`. Il puntatore *s* viene quindi assegnato a *t* e quindi *t* viene usato in un modo in cui può verificarsi una dereferenziazione `NULL`.
