...
| [CA1404: Chiamare GetLastError immediatamente dopo P/Invoke](../code-quality/ca1404-call-getlasterror-immediately-after-p-invoke.md) | Viene eseguita una chiamata al metodo Marshal. GetLastWin32Error o alla funzione GetLastError [!INCLUDE[TLA2#tla_win32](../code-quality/includes/tla2sharptla_win32_md.md)] equivalente e la chiamata immediatamente precedente non è a un metodo di platform invoke. |
...
| [CA1415: Dichiarare correttamente P/Invoke](../code-quality/ca1415-declare-p-invokes-correctly.md) | Questa regola cerca platform invoke dichiarazioni di metodo che hanno come destinazione funzioni [!INCLUDE[TLA2#tla_win32](../code-quality/includes/tla2sharptla_win32_md.md)] con un puntatore a un parametro di struttura SOVRAPPOSTa e il parametro gestito corrispondente non è un puntatore a una struttura <xref:System.Threading.NativeOverlapped?displayProperty=fullName>. |
