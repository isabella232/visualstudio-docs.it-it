...
Un tipo visibile esternamente può essere assegnato all'interfaccia <xref:System.Runtime.Serialization.ISerializable?displayProperty=fullName> e si verifica una delle condizioni seguenti:
...
## <a name="example"></a>Esempio
Nell'esempio seguente vengono risolte le due violazioni precedenti fornendo un'implementazione sottoponibile a override di <xref:System.Runtime.Serialization.ISerializable.GetObjectData> sulla classe Book e fornendo un'implementazione di `GetObjectData` sulla classe della libreria.
