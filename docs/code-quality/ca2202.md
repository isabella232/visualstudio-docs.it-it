...
Per risolvere questo problema, utilizzare un `try`/blocco `finally` anziché l'istruzione `using` esterna. Nel blocco `finally` verificare che la risorsa `stream` non sia null.
...
> [!TIP]
> La sintassi del `?.` precedente è l' [operatore condizionale null](/dotnet/csharp/language-reference/operators/member-access-operators#null-conditional-operators--and-).
