...
Le proprietà che restituiscono il valore di un campo privato o interno sono ottimizzate per essere eseguite in modo analogo all'accesso a un campo. il miglioramento delle prestazioni derivante dall'utilizzo di campi visibili esternamente anziché di proprietà è minimo. *Visibile esternamente* si riferisce ai `public` `protected` livelli di accessibilità, e `protected internal` ( `Public` , `Protected` e `Protected Friend` in Visual Basic).
