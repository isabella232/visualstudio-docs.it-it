...
Le firme di uno e due costruttori di stringa di <xref:System.ArgumentException> e dei relativi tipi derivati non sono coerenti rispetto alla posizione `message` e `paramName` parametri. Assicurarsi che questi costruttori vengano chiamati con gli argomenti stringa corretti. Le firme sono le seguenti:
