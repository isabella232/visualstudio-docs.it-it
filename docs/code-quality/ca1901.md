...
In questo esempio, il parametro `nIconIndex` viene dichiarato come `IntPtr`, che è in larghezza 4 byte in una piattaforma a 32 bit e 8 byte in una piattaforma a 64 bit. Nella dichiarazione non gestita riportata di seguito, è possibile osservare che `nIconIndex` è un Unsigned Integer a 4 byte in tutte le piattaforme.
