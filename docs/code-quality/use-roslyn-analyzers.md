...
2. Aggiungere una voce per ogni regola che si desidera configurare con l'estensione di file corrispondente. Ad esempio, per impostare il livello di gravità per [CA1822](ca1822.md) su C# `error` per i file, la voce è simile alla seguente:
...
- Dal **Elenco errori**
  ...
  - Se si seleziona un **file di eliminazione**, viene visualizzata la finestra di dialogo **Anteprima modifiche** con un'anteprima dell'attributo <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute> aggiunto al file di eliminazione globale.
