...
Un'enumerazione senza un <xref:System.FlagsAttribute?displayProperty=fullName> applicato non definisce un membro con un valore pari a zero. In alternativa, un'enumerazione con un <xref:System.FlagsAttribute> applicato definisce un membro che ha un valore pari a zero, ma il cui nome non è' none '. In alternativa, l'enumerazione definisce più membri con valore zero.
...
Se un'enumerazione con la <xref:System.FlagsAttribute> applicata definisce un membro con valore zero, il nome deve essere ' none ' per indicare che non è stato impostato alcun valore nell'enumerazione. L'uso di un membro a valore zero per qualsiasi altro scopo è contrario all'uso del <xref:System.FlagsAttribute> in quanto gli operatori AND e OR bit per bit sono inutili con il membro. Questo implica che a un solo membro deve essere assegnato il valore zero. Se più membri con valore zero si verificano in un'enumerazione con attributi di flag, `Enum.ToString()` restituisce risultati non corretti per i membri che non sono zero.
