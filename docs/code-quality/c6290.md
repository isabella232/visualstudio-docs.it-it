...
L'operatore `!` restituisce un risultato booleano e ha precedenza più alta rispetto al `&`. L'operatore and bit per bit (&) accetta due argomenti aritmetici. Pertanto, è stato rilevato uno degli errori seguenti:

- L'espressione non è racchiusa tra parentesi:

   Poiché il risultato di `!` è booleano (zero o uno), un tentativo di verificare se due variabili hanno bits in comune comporterà solo il testing della presenza del bit più basso sul lato destro: `((!8) & 1) == 0`.

- L'operatore `!` non è corretto e deve essere un `~`:

   L'operatore `!` ha un risultato booleano, mentre l'operatore `~` ha un risultato aritmetico. Questi operatori non sono mai intercambiabili, anche quando operano su un valore booleano (zero o uno): `((!0x01) & 0x10) == 0x0`, mentre `((~0x01) & 0x10) == 0x10`.
