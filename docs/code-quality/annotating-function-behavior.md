...
|`_Must_inspect_result_`|Annota qualsiasi valore di output, inclusi il valore restituito, i parametri e le variabili globali.  L'analizzatore segnala un errore se il valore dell'oggetto annotato non è controllato in seguito. "L'ispezione" include se viene utilizzata in un'espressione condizionale, viene assegnata a un parametro di output o globale o viene passata come parametro.  Per i valori restituiti `_Must_inspect_result_` , `_Check_return_`implica.|
