---
title: "Ca2007: non eseguire direttamente l'attesa di un'attività"
ms.date: 03/08/2019
ms.topic: reference
f1_keywords:
- CA2007
...
ms.locfileid: "75584621"
---
# <a name="ca2007-do-not-directly-await-a-task"></a>Ca2007: non eseguire direttamente l'attesa di un'attività
...
Per correggere le violazioni, chiamare <xref:System.Threading.Tasks.Task.ConfigureAwait%2A> sul <xref:System.Threading.Tasks.Task>atteso. È possibile passare `true` o `false` per il parametro `continueOnCapturedContext`.

- La chiamata di `ConfigureAwait(true)` sull'attività ha lo stesso comportamento che non chiama in modo esplicito <xref:System.Threading.Tasks.Task.ConfigureAwait%2A>. Chiamando in modo esplicito questo metodo, si lascia che i lettori sappiano che si vuole intenzionalmente eseguire la continuazione nel contesto di sincronizzazione originale.

- Chiamare `ConfigureAwait(false)` sull'attività per pianificare le continuazioni nel pool di thread, evitando in tal modo un deadlock sul thread dell'interfaccia utente. Il passaggio di `false` è un'opzione efficace per le librerie indipendenti dall'app.
...
Per correggere la violazione, chiamare <xref:System.Threading.Tasks.Task.ConfigureAwait%2A> sulla <xref:System.Threading.Tasks.Task>attesa:
