...
Nelle versioni 1,0 e 1,1 del .NET Framework, un metodo pubblico o protetto contiene un `try` / `catch` / blocco `finally`. Il blocco `finally` sembra ripristinare lo stato di sicurezza e non è racchiuso in un blocco `finally`.
