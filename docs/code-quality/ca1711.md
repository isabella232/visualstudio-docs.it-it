...
Se si esegue questa regola da [analizzatori FxCop](install-fxcop-analyzers.md) (e non con analisi legacy), è possibile configurare un elenco di suffissi consentiti, con ogni suffisso separato dal carattere barra verticale ("|"). Ad esempio, per specificare che la regola non deve essere eseguita sui suffissi flag e Flags, aggiungere la coppia chiave-valore seguente a un file con estensione EditorConfig nel progetto:
