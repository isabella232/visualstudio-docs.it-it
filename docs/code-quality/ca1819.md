...
Nell'esempio seguente viene corretta la violazione modificando la proprietà in modo che restituisca <xref:System.Collections.ObjectModel.ReadOnlyCollection%601?displayProperty=fullName> :
...
Nell'esempio seguente viene corretta la violazione modificando la proprietà in modo che restituisca <xref:System.Collections.ObjectModel.Collection%601?displayProperty=fullName> :
