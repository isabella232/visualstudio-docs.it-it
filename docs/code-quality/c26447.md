...
C++Linee guida principali: [F. 6: Se non è possibile generare la funzione, dichiararla noexcept @ no__t-0.
...
- La regola si concentra solo sulle chiamate di funzione. Contrassegna le destinazioni che non sono **constExpr** e che potenzialmente possono generare eccezioni. in altre parole, non vengono contrassegnate in modo esplicito come non generate tramite **noexcept**, **_ _ declspec (nothrow)** , **throw ()** .
