...
## <a name="example"></a>Esempio
Nell'applicazione seguente viene utilizzata la libreria definita in precedenza. Si noti che il tipo dichiarato all'esterno di uno spazio dei nomi viene creato quando il nome `Test` non è qualificato da uno spazio dei nomi. Si noti inoltre che per accedere al tipo di `Test` in `Goodspace`, il nome dello spazio dei nomi è obbligatorio.
