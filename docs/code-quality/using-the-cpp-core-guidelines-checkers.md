---
title: Usando i controlli delle linee guida di base di C++
ms.date: 08/14/2018
ms.topic: conceptual
author: mikeblome
ms.author: mblome
...
## <a name="enable-the-c-core-check-guidelines-in-code-analysis"></a>Abilitare le linee guida C++ Core controllare nell'analisi del codice
 ...
 ![Pagina delle proprietà per le impostazioni generali di analisi codice](media/cppcorecheck_codeanalysis_general.png)

 Un sottoinsieme di regole di controllo di base di C++ è incluso nel set di regole consigliate Native Microsoft eseguita per impostazione predefinita quando è abilitata l'analisi del codice. Per abilitare le regole di base aggiuntive, fare clic sull'elenco a discesa e scegliere quali set di regole da includere:

 ![Elenco a discesa per set di regole di C++ Core controllare aggiuntive](media/cppcorecheck_codeanalysis_extensions.png)
...
Come vengono aggiunte nuove regole per il controllo di linee guida di base di C++, può aumentare il numero di avvisi generati per il codice preesistente. È possibile usare set di regole predefiniti per filtrare quali tipi di regole da abilitare.
...
Queste macro corrispondono ai set di regole ed espandono in un elenco delimitato da spazi di numeri di avviso. Usando i costrutti pragma appropriato, è possibile configurare il set di regole valido che è interessante per un progetto o una sezione di codice. Nell'esempio seguente, l'analisi del codice avvisa solo sui modificatori di costanti mancante:
...
7. Disabilitare l'uso dell'intestazione precompilata (**C / C++ \Precompiled intestazioni**). Ciò è necessario perché il motore delle estensioni può tentare di leggere le informazioni interne da intestazione precompilata (PCH); Se il file PCH compilato con le opzioni di progetto predefinite, non sarà compatibile.
