...
Anche se l'app demo è Visual Basic, la maggior parte delle funzionalità è C#applicabile C++a F#,,, Python, JavaScript e altri linguaggi supportati da Visual StudioF# (non supporta modifica e continuazione). F# e JavaScript non supportano la finestra **Auto**). Le schermate si trovano in Visual Basic.
...
1. Mentre viene sospesa sull'istruzione `name += letters[i]`, passare il puntatore del mouse sulla variabile `letters` e viene visualizzato il valore predefinito, ovvero il valore del primo elemento nella matrice `"f"c`.
