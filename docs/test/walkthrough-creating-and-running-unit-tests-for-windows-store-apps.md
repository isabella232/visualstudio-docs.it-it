...
Visual Studio include il supporto per gli unit test delle app della piattaforma UWP (Universal Windows Platform). Visual Studio fornisce modelli di progetto di unit test per C, Visual Basic e C.
...
1. Aprire Visual Studio. Nella finestra di avvio scegliere **Crea un nuovo progetto.**

2. Nella casella di ricerca della pagina **Crea un nuovo progetto** immettere unit **test**.

   Elenco di modelli che vengono filtrati in base a quelli per gli unit test.

3. Selezionare il modello di app di **unit test (Windows universale)** per C , o Visual Basic, quindi selezionare **Avanti**.

   ![Creare una nuova app di unit test UWP in Visual StudioCreate new UWP unit unit test app in Visual Studio](media/vs-2019/new-uwp-unit-test-app.png)

4. Facoltativamente, modificare il nome e il percorso del progetto o della soluzione, quindi selezionare **Crea**.

5. Facoltativamente, modificare le versioni di destinazione e di piattaforma minima, quindi scegliere **OK**.

Dopo aver completato questi passaggi, il progetto di unit test viene creato e visualizzato in Esplora soluzioni.
...
6. Lasciare selezionata l'opzione **Crea directory per soluzione** e scegliere il pulsante **OK** .
   ...
   **Esplora soluzioni** viene popolato con il progetto di unit test UWP e nell'editor di codice viene visualizzato lo unit test predefinito denominato UnitTest1.
...
Nell'editor di codice modificare lo unit test e aggiungere le asserzioni e la logica necessarie per il test.
...
2. Scegliere **Compila soluzione**dal menu **Compila** .
   ...
   Lo unit test viene ora visualizzato in Esplora test.
