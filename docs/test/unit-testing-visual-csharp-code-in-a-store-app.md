...
Questo articolo illustra *lo sviluppo basato su test.* In questo approccio si scrive prima di tutto un test che verifica un comportamento specifico nel sistema che si sta testando e quindi si scrive il codice che supera il test.
...
::: moniker-end
   ...
   Il progetto di test viene compilato ed eseguito. Sii paziente perché può richiedere un po' di tempo. Viene **visualizzata la finestra** Esplora test e il test è elencato in Test **superati**. Il **riquadro** Riepilogo nella parte inferiore della finestra fornisce dettagli aggiuntivi sul test selezionato.
...
3. Aggiungere il codice seguente al file *Rooter.cs della classe* **Rooter:**
   ...
   La **classe Rooter** dichiara un costruttore e il metodo di stima **SquareRoot.** Il **metodo SquareRoot** è solo un'implementazione minima, sufficiente per testare la struttura di base della configurazione di test.
...
4. Per evitare l'errore "Payload contains two or more files with the same  destination path" (Payload contiene due o più file con lo stesso percorso di destinazione), in **Esplora soluzioni** espandere il nodo Proprietà nel progetto **Maths** e quindi eliminare il file *Default.rd.xml.*
...
6. In **Esplora test** scegliere Esegui **tutto.**
   ...
   ![BasicTest superato in Esplora test](../test/media/ute_cpp_testexplorer_basictest.png)
...
In questa sezione si effettua il refactoring sia dell'app che del codice di test, quindi si rieseguino i test per assicurarsi che siano ancora superati.
...
2. Eseguire tutti i test per assicurarsi di non aver introdotto una regressione. Devono essere tutte superate.
...
Il **metodo RangeTest** imposta come hard coded il denominatore della variabile *tolerance* passata al <xref:Microsoft.VisualStudio.TestTools.UnitTesting.Assert> metodo . Se si prevede di aggiungere altri test che usano lo stesso calcolo della tolleranza, l'uso di un valore hard-coded in più posizioni rende più difficile la manutenzione del codice.
...
2. Eseguire **RangeTest** per assicurarsi che sia ancora superato.
