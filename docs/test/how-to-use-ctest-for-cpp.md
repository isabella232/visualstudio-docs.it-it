...
Il supporto di CMake in Visual Studio non riguarda il sistema di progetti di Visual Studio. Di conseguenza, i test di CTest vengono scritti e configurati come in qualsiasi ambiente CMake. Utilizzare `enable_testing()` il comando per abilitare il test e il `add_test()` comando o `gtest_discover_tests()` per aggiungere un nuovo test. Per ulteriori informazioni su CTest, vedere la documentazione di [CMake](https://gitlab.kitware.com/cmake/community/wikis/doc/ctest/Testing-With-CTest). 
