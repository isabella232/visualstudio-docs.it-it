...
Per impostazione predefinita, Visual Studio include i token seguenti: `HACK`, `TODO`, `UNDONE` e `NOTE`. Per i token non viene fatta distinzione tra maiuscole e minuscole.

È inoltre possibile creare token personalizzati. Per creare un token personalizzato:
