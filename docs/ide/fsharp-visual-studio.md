...
|---------------|----------------|-----|
|File di risorse|Yes||
|Impostazioni di compilazione, debug e riferimento|Yes||
|Multitargeting|Yes||
...
|Percorsi di riferimento|[Gestire i riferimenti in un progetto](managing-references-in-a-project.md)|Consente di specificare la posizione in cui cercare gli assembly da cui dipende il codice.|
...
|Segnalibri|Consente di salvare posizioni specifiche nell'editor.|Yes|
|Modifica del rientro|Consente di impostare o annullare un rientro per le righe selezionate.|Yes|
|Rientro automatico|Applica e annulla il rientro del cursore in base alle regole di ambito F#.|Yes|
|[Ricerca e sostituzione di testo](finding-and-replacing-text.md)|Consente di eseguire la ricerca in un file, in un progetto o in una soluzione, con la possibilità di modificare il testo.|Yes|
...
|Passaggio alla definizione per un'API definita dall'utente|Quando il cursore è posizionato su un'entità di programma definita dall'utente, sposta il cursore nella posizione all'interno del codice in cui l'entità è definita.|Yes|
|Vai alla riga|Consente di passare a una riga specifica in un file, in base al numero di riga.|Yes|
|Barre di spostamento nella parte superiore del file|Consente di passare a posizioni specifiche all'interno del codice, ad esempio a un nome di funzione.|Yes|
|Linee guida per strutture a blocchi|Mostra linee guida che indicano ambiti F# che visualizzano un'anteprima se si sposta il puntatore del mouse su di essi.|Yes|
|[Struttura](outlining.md)|Consente di comprimere le sezioni del codice per creare una visualizzazione più compatta.|Yes|
|Inserimento di tabulazioni|Converte gli spazi in tabulazioni.|Yes|
|Colorazione dei tipi|Visualizza i nomi dei tipi definiti con un colore particolare.|Yes|
|Ricerca veloce. Vedere Ricerca veloce, finestra Trova e sostituisci.|Consente di eseguire ricerche all'interno di un file o di un progetto.|Yes|
|**Ctrl**+**clic** per passare a una definizione|Consente di richiamare Vai a definizione tenendo premuto **Ctrl** e facendo clic su un simbolo F#.|Yes|
|Vai a definizione da Informazioni rapide|Simboli selezionabili all'interno delle descrizioni comando che richiamano Vai a definizione.|Yes|
|Vai a tutti|Consente lo spostamento per corrispondenze fuzzy globale per tutti i costrutti F# tramite **Ctrl**+**T**.|Yes|
|Ridenominazione inline|Consente di rinominare tutte le occorrenze di un simbolo inline.|Yes|
|Trova tutti i riferimenti|Trova tutte le occorrenze di un simbolo in una codebase.|Yes|
|Correzione del codice Semplifica nome|Rimuove i qualificatori non necessari per i simboli F#.|Yes|
|Correzione del codice Rimuovi istruzioni `open` non usate|Rimuove tutte le istruzioni `open` non necessarie presenti in un documento.|Yes|
|Correzione del codice valore non usato|Suggerisce di rinominare un identificatore non usato in un carattere di sottolineatura.|Yes|
...
|Funzionalità|Descrizione|Supporto in F#|
|-------|-----------|----------------|
|Implementazione automatica di interfacce|Genera stub di codice per metodi di interfaccia.|Yes|
|Frammenti di codice|Inserisce codice da una libreria di costrutti di codifica comuni in argomenti.|No|
|Completa parola|Salva il testo digitato completando le parole e i nomi durante la digitazione.|Yes|
|Completamento automatico|Se abilitata, questa funzionalità fa in modo che il completamento delle parole selezioni la prima corrispondenza durante la digitazione, anziché attendere la selezione di una parola o la pressione di **Ctrl**+**Spazio** da parte dell'utente.|Yes|
|Offerta di completamento dei simboli all'interno di spazi dei nomi non aperti|Con il completamento automatico, viene suggerito un simbolo corrispondente presente in uno spazio dei nomi non aperto, consentendo il completamento con l'istruzione `open` corrispondente, se selezionato.|Yes|
|Generazione di elementi di codice|Consente di generare codice di stub per un'ampia gamma di costrutti.|No|
|Elenca membri|Quando si digita l'operatore di accesso ai membri (.), visualizza i membri di un tipo.|Yes|
...
|Informazioni sul parametro|Visualizza informazioni utili sui parametri durante la digitazione di una chiamata di funzione.|Yes|
|Informazioni rapide|Visualizza la dichiarazione completa per ogni identificatore incluso nel codice.|Yes|
|Completamento parentesi graffa automatico|Completa automaticamente i costrutti di sintassi con parentesi graffe di F# in modo transazionale.|Yes|
...
|Punti di interruzione|Consente di sospendere l'esecuzione del codice in corrispondenza di punti specifici durante il debug.|Yes|
|Punti di interruzione condizionali|Consente l'uso di punti di interruzione che eseguono il test di una condizione che determina se l'esecuzione deve essere sospesa.|Yes|
...
|Debug cronologico|Consente di eseguire un'istruzione di codice eseguito in precedenza.|Yes|
|Finestra Variabili locali|Visualizza valori e variabili definiti localmente.|Yes|
|Esecuzione fino al cursore|Consente di eseguire il codice fino al raggiungimento della riga che contiene il cursore.|Yes|
|Esegui istruzione|Consente di anticipare l'esecuzione passando a una qualsiasi chiamata di funzione.|Yes|
|Esegui istruzione/routine|Consente di anticipare l'esecuzione nello stack frame corrente superando qualsiasi chiamata di funzione.|Yes|
...
|[Finestra Elenco errori](reference/error-list-window.md)|Visualizza l'elenco degli errori nel codice.|Yes|
|[F# Interactive](/dotnet/fsharp/tutorials/fsharp-interactive/)|Consente di digitare (o copiare e incollare) codice F# ed eseguirlo immediatamente, indipendentemente dalla compilazione del progetto. La finestra F# Interactive è una finestra REPL (Read-Eval-Print).|Yes|
...
|[Finestra Output](reference/output-window.md)|Visualizza l'output della compilazione.|Yes|
|Analisi delle prestazioni|Rende disponibili strumenti per la misurazione delle prestazioni del codice.|Yes|
|Finestra Proprietà|Visualizza le proprietà dell'oggetto con stato attivo nell'ambiente di sviluppo e consente la modifica di tali proprietà.|Yes|
|Esplora server|Offre alcuni metodi per l'interazione con un'ampia gamma di risorse del server.|Yes|
...
|Casella degli strumenti|Visualizza schede che contengono oggetti trascinabili quali controlli e sezioni di testo o codice.|Yes|
