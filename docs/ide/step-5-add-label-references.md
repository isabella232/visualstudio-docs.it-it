...
description: Informazioni su come aggiungere riferimenti di etichetta al modulo.
...
4. Scegliere il controllo etichetta successivo. Non accadrà nulla. Il programma tiene già traccia della prima etichetta scelta dal giocatore, quindi non è uguale a in C# o `firstClicked` `null` in `Nothing` Visual Basic. Quando l'istruzione `if` controlla `firstClicked` per stabilire se sia uguale a `null` o `Nothing`, rileva che non lo è e non esegue le istruzioni contenute nell'istruzione `if`. Quindi, solo la prima icona scelta diventa nera e le altre icone sono invisibili, come illustrato nell'immagine seguente.
