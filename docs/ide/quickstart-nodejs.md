...
In questa introduzione di 5-10 minuti all'ambiente Visual Studio di sviluppo integrato (IDE) viene creata una semplice Node.js web.
...
### <a name="install-visual-studio-and-the-nodejs-workload"></a>Installare Visual Studio e il carico di Node.js di lavoro

Se non è ancora stato installato Visual Studio:

1. Passare alla pagina [Visual Studio download per](https://visualstudio.microsoft.com/downloads) installare Visual Studio 2022 gratuitamente.
...
Anche Node.js è compilato per architetture a 32 e 64 bit, il programma di Node.js supporta solo una versione alla volta.
...
Visual Studio in genere rileva il runtime installato, ma in caso contrario, è possibile configurare il progetto per fare riferimento al runtime installato:
   ...
   1. Selezionare **Proprietà** e impostare il percorso **Node.exe.** È possibile usare un'Node.js globale o specificare il percorso di un interprete locale per qualsiasi Node.js progetto.
...
Visual Studio possibile configurare l'ambiente, inclusa l'installazione di strumenti comuni con Node.js sviluppo.
...
1. Installare la versione LTS del [runtimeNode.js.](https://nodejs.org/en/download/) È consigliabile usare la versione LTS per garantire la migliore compatibilità con framework e librerie esterni.
    ...
    Anche Node.js è compilato per architetture a 32 e 64 bit, il programma di installazione Node.js supporta solo una versione installata alla volta.
...
Se il modello di progetto **Applicazione Web** Node.js vuoto non è visualizzato, è necessario aggiungere il carico di lavoroNode.js **sviluppo.** Per istruzioni, vedere [Prerequisiti](#prerequisites).
...
1. Nel riquadro destro esaminare l'Esplora soluzioni **.**
   
   - Al livello superiore è disponibile *una soluzione*, che per impostazione predefinita ha lo stesso nome del progetto. Una soluzione, rappresentata da un file *sln* su disco, è un contenitore per uno o più progetti correlati.
   ...
   ![Screenshot che mostra il Esplora soluzioni di lavoro.](../ide/media/vs-2022/quickstart-nodejs-solution-explorer.png)

1. Per installare pacchetti npm o Node.js comandi da un prompt dei comandi, fare clic con il pulsante destro del mouse sul nodo del progetto e scegliere **Apri prompt dei comandi qui**.
...
1. Per testare la navigazione nel codice sorgente, nel file *server.js* aperto selezionare e premere F12 oppure fare clic con il pulsante destro del mouse e scegliere Vai a definizione `createServer` dal menu di scelta  `createServer` rapida.  Questo comando consente di accedere alla definizione della `createServer` funzione in *http.d.ts*.
...
1. Tornare a *server.js*, individuare questa riga di codice: `res.end('Hello World\n');` e modificarla in:
   ...
   Quando si digita `connection.` , IntelliSense fornisce opzioni per completare automaticamente la voce di codice.
...
1. Nel riquadro destro esaminare l'Esplora soluzioni **.**
   ...
   ![Screenshot che mostra il Esplora soluzioni di lavoro.](../ide/media/quickstart-nodejs-solution-explorer.png)

1. Per installare pacchetti npm o Node.js comandi da un prompt dei comandi, fare clic con il pulsante destro del mouse sul nodo del progetto e scegliere **Apri prompt** dei comandi qui dal menu di scelta rapida.
...
1. Per testare la navigazione nel codice sorgente, nel file *server.js* aperto selezionare **http.createServer** e premere **F12** o scegliere **Vai** a definizione dal menu di scelta rapida. Questo comando consente di accedere alla definizione della `createServer` funzione in *http.d.ts*.
...
1. Premere **CTRL** + **F5 o** selezionare Avvia **debug** senza  >  **eseguire debug** per eseguire l'app.
...
Congratulazioni! È stata creata un'app Node.js semplice con Visual Studio. Per approfondire il contenuto, passare alla **sezione Esercitazioni** del sommario.
