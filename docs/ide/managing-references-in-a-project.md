...
2.  Andare al sito del fornitore proprietario dell'SDK di estensione non supportato e installare la versione dell'SDK di estensione con dipendenze compatibili con la versione della piattaforma di destinazione del progetto.  
    ...
    > [!NOTE]
    >  Un modo per verificare se un SDK di estensione ha dipendenze da altri SDK di estensione è riavviare Visual Studio, creare un nuovo progetto UWP in C#, fare clic con il pulsante destro del mouse sul progetto e scegliere **Aggiungi riferimento**, passare alla scheda **Windows**, accedere alla sottoscheda **Estensioni**, selezionare l'SDK di estensione e osservare il riquadro destro di **Gestione riferimenti**. Se dispone di dipendenze, risulteranno elencate.  
