...
In questo passaggio si aggiunge un controllo <xref:System.Windows.Forms.TableLayoutPanel> al modulo. TableLayoutPanel consente di allineare in modo corretto i controlli nel modulo che si aggiungerà successivamente.  
...
## <a name="to-lay-out-your-form-with-a-tablelayoutpanel-control"></a>Per creare il layout del modulo con un controllo TableLayoutPanel  
...
3.  È possibile aggiungere al modulo controlli quali pulsanti, caselle di controllo ed etichette. Fare doppio clic sul controllo TableLayoutPanel nella **casella degli strumenti**. In alternativa, è possibile trascinare il controllo dalla casella degli strumenti al modulo. Quando si esegue questa operazione, l'IDE aggiunge un controllo TableLayoutPanel al modulo, come mostrato nell'immagine seguente.  
  
     ![Controllo TableLayoutPanel](../ide/media/express_formtablelayout.png "Express_FormTableLayout")  
Controllo **TableLayoutPanel**  
  
    > [!NOTE]
    >  Dopo aver aggiunto il controllo TableLayoutPanel, se nel modulo viene visualizzata una finestra con il titolo **Attività di TableLayoutPanel**, fare clic in qualsiasi punto all'interno del modulo per chiuderla. Verranno fornite ulteriori informazioni su questa finestra più avanti nell'esercitazione.  
...
6.  Il selettore dei controlli è un elenco a discesa che si trova nella parte superiore della finestra **Proprietà**. In questo esempio è selezionato un controllo denominato `tableLayoutPanel1`. È possibile selezionare i controlli scegliendo un'area in **Progettazione Windows Form** o selezionandoli dal selettore dei controlli. Dopo aver selezionato TableLayoutPanel, individuare la relativa proprietà **Dock** e scegliere **Dock**, il cui valore dovrebbe essere impostato su **Nessuno**. Si noti che viene visualizzata una freccia a discesa accanto al valore. Fare clic sulla freccia, quindi selezionare il pulsante **Riempimento** (il pulsante grande al centro), come mostrato nell'immagine seguente.  
     ...
     ![Finestra Proprietà con Riempi selezionato](../ide/media/express_docktable.png "Express_DockTable")  
Finestra **Proprietà** con valore **Riempi** selezionato  
...
7.  Dopo aver impostato la proprietà **Dock** di TableLayoutPanel su **Riempi**, il pannello riempie l'intero modulo. Se si ridimensiona nuovamente il modulo, TableLayoutPanel resta ancorato e viene ridimensionato correttamente.  
...
**Form1** con controllo **TableLayoutPanel** ridimensionato  
