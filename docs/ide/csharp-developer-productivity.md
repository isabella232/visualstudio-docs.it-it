...
| **F12** (anche **Ctrl**+**Click**) | Vai a definizione | Consente di passare al punto in cui si trova la definizione di un simbolo |
...
| **ALT**+**Home (Pagina iniziale)** | Vai a base | Spostarsi verso l'alto nella catena di ereditarietà |
...
| **Ctrl**+**Ctrl**+**E** **E** +**W** **Ctrl**, Ctrl E (Profilo predefinito) o Ctrl W , E (Profilo C**\\** | Visualizzazione dell'elenco errori | Consente di visualizzare tutti gli errori nel documento, nel progetto o nella soluzione |
| **Alt** + **PgSu/PgDn** | Passa a problema successivo/precedente | Passa all'errore, avviso, suggerimento precedente/successivo nel documento (disponibile in **Visual Studio 2017 versione 15.8** e versioni successive) |
...
| **Ctrl**+**SHIFT**+**/** | Attiva/Disattiva commento per il blocco/Rimuovi commento | Questo comando aggiunge o rimuove commenti per il blocco a seconda dell'elemento selezionato |

> [!NOTE]
> Alcune estensioni separano i tasti di scelta rapida predefiniti di Visual Studio. Per utilizzare i comandi precedenti, ripristinare le associazioni di tasti alle impostazioni predefinite di Visual Studio scegliendo **Strumenti** > **Importa ed esporta impostazioni** > **Ripristina tutte le impostazioni** o Opzioni **strumenti** > **Ripristino** > **tastiera** > **Reset**.
...
- Aggiungere un file predefinito o . Stile NET EditorConfig al progetto scegliendo **Aggiungi** > **nuovo elemento**. Nella finestra di dialogo **Aggiungi nuovo elemento** cercare "editorconfig". Selezionare uno dei due modelli di elemento **File editorconfig** e quindi scegliere **Aggiungi**.
...
Visual Studio offre la formattazione on-demand del file di codice, incluse le preferenze di stile per il codice, attraverso la funzionalità **Pulizia del codice**. Per eseguire Code Cleanup, fare clic sull'icona della scopa nella parte inferiore dell'editor o premere **Ctrl**+**K**, **Ctrl**+**E**.
...
Visual Studio include numerosi refactoring, azioni di generazione del codice e correzioni del codice. Le sottolineature rosse rappresentano errori, quelle verdi rappresentano gli avvisi e tre punti grigi rappresentano suggerimenti per il codice. È possibile accedere alle correzioni del codice facendo clic sull'icona della lampadina o del cacciavite oppure premendo **CTRL**+**.** o **Alt**+**Invio**. Ogni correzione è dotata di una finestra di anteprima che mostra come funziona la correzione in un diff di codice live.
...
Per ulteriori informazioni, consultate [Funzionalità di generazione del codice.](code-generation-in-visual-studio.md)
...
| Spostamento agli assembly decompilati | **F12** o **CTRL**+**fare clic su** | Passare all'origine esterna (decompilata con ILSpy) abilitando la funzionalità: **Opzioni degli strumenti** > **Editor** > **Text Editor** > di testo**C ,** > **Avanzate** > di abilitazione per la navigazione in origini**decompilate**. |
...
- [Gli unit test](../test/live-unit-testing.md) in tempo reale esecano continuamente i test interessati dalle modifiche al codice e aggiornano le icone dell'editor inline per infognire lo stato dei test. Includere o escludere progetti di test o test specifici dal set di test attivi. (Solo per Visual Studio Enterprise Edition.)
...
|-|-|-|
| Individuare il file in Esplora soluzioni | Evidenzia il file attivo in **Esplora soluzioni** | **Strumenti** > **Opzioni** > **Progetti e soluzioni** > Traccia elemento attivo in Esplora**soluzioni** |
| Aggiungere le direttive using per i tipi in assembly di riferimento e pacchetti NuGet | Visualizza una lampadina di errore con una correzione del codice per installare un pacchetto NuGet per un tipo senza riferimenti | **Tools** > **Opzioni** > **C#** >  > **Suggest usings for types in reference assemblies** **Text Editor** **Suggest usings for types in NuGet packages** **Advanced**Editor di testo C ' Suggerimento avanzato per i tipi negli assembly di riferimento e Suggerire utilizzi per i tipi nei pacchetti NuGet >  |
| Abilita analisi della soluzione completa | Visualizzare tutti gli errori nella soluzione **nell'Elenco errori** | **Strumenti** > **Opzioni** > **Editor** > di testo**C -** > **Avanzate** > **Abilita l'analisi completa della soluzione** |
| Abilita spostamento a origini decompilate | Consente l'uso di Vai a definizione per tipi o membri di origini esterne e del decompilatore ILSpy per visualizzare i corpi dei metodi | **Strumenti** > **Opzioni** > **Editor** > di testo**C -5** > **Avanzate** > **Abilita l'esplorazione in origini decompilate** |
...
| [CodeLens](../ide/find-code-changes-and-other-history-with-codelens.md) | Visualizza le informazioni di riferimento del codice e la cronologia modifiche nell'editor (gli indicatori CodeLens del controllo del codice sorgente non sono disponibili nell'edizione Visual Studio Community). | **Opzioni** > **Opzioni** > **Editor** > di testo**Tutti i linguaggi** > **CodeLens** |
| [Frammenti di codice](../ide/visual-csharp-code-snippets.md) | Utili per di generare codice boilerplate comune | Digitare il nome di un frammento e premere **TAB** due volte. |
