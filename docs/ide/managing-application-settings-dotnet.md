...
Le impostazioni dell'applicazione consentono di archiviare le informazioni sull'applicazione in modo dinamico. Le impostazioni consentono di archiviare nel computer client informazioni che non devono essere incluse nel codice dell'applicazione, ad esempio una stringa di connessione, le preferenze utente e altre informazioni necessarie in fase di esecuzione.
...
Quando si crea un'impostazione con ambito di applicazione, ad esempio una [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] stringa di connessione al `<applicationSettings>` database o un riferimento alle risorse del server, la salva in *app.config* con il tag. (Le stringhe di connessione vengono salvate nel tag `<connectionStrings>` .)

Quando si crea un'impostazione con ambito di utente (ad esempio, tipo di carattere predefinito, home page o dimensioni della finestra), [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] la salva in *app.config* con il `<userSettings>` tag .

> [!IMPORTANT]
> Quando si archiviano le stringhe di connessione in *app.config*, è necessario prendere precauzioni per evitare di rivelare informazioni riservate, ad esempio password o percorsi del server, nella stringa di connessione.
>
...
**In Progettazione impostazioni** viene innanzitutto eseguita la ricerca del file *Settings.settings* creato dal sistema del progetto. questo file è il file predefinito visualizzato da *Settings.settings* **Progettazione progetti** nella *My Project* scheda [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] **Impostazioni.** *Properties* [!INCLUDE[csprcs](../data-tools/includes/csprcs_md.md)] **Progettazione progetti** cerca quindi altri file di impostazioni nella cartella radice del progetto. Pertanto, è necessario inserirvi il file di impostazioni personalizzato. Se si aggiunge un file *.settings* in un altro punto del progetto, **Progettazione progetti** non sarà in grado di individuarlo.
...
Nei progetti Visual Basic è possibile accedere alle impostazioni dell'applicazione in fase di esecuzione usando l'oggetto `My.Settings`. Nella pagina **Impostazioni** fare clic sul pulsante **Visualizzare codice** per visualizzare il file *Settings.vb*. *Settings.vb* definisce `Settings` la classe , che consente di gestire <xref:System.Configuration.ApplicationSettingsBase.SettingChanging> <xref:System.Configuration.ApplicationSettingsBase.PropertyChanged>questi <xref:System.Configuration.ApplicationSettingsBase.SettingsLoaded>eventi <xref:System.Configuration.ApplicationSettingsBase.SettingsSaving>nella classe delle impostazioni: , , e . La classe `Settings` nel file *Settings.vb* è una classe parziale in cui viene visualizzato solo il codice di proprietà dell'utente, non l'intera classe generata. Per altre informazioni sull'accesso alle impostazioni dell'applicazione usando l'oggetto `My.Settings`, vedere [Accedere alle impostazioni dell'applicazione (.NET Framework)](/dotnet/visual-basic/developing-apps/programming/app-settings/accessing-application-settings).

I valori di tutte le impostazioni con ambito di utente modificate dall'utente in fase di esecuzione, ad esempio la posizione di un modulo, vengono archiviati in un file *user.config.* Si noti che i valori predefiniti vengono comunque salvati in *app.config*.
...
Si consiglia di utilizzare `My.Settings` l'oggetto e il file *.settings* predefinito per accedere alle impostazioni. Ciò è dovuto al fatto che è possibile utilizzare **Progettazione impostazioni** per assegnare proprietà alle impostazioni e, inoltre, le impostazioni utente vengono salvate automaticamente prima dell'arresto dell'applicazione. L'applicazione Visual Basic può comunque accedere direttamente alle impostazioni. In tal caso è `MySettings` necessario accedere alla classe e utilizzare un file *.settings* personalizzato nella radice del progetto. È necessario salvare le impostazioni utente prima di terminare l'applicazione, come per le applicazioni C#, come illustrato nella sezione seguente.
