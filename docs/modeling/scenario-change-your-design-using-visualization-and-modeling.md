...
Utilizzare strumenti quali mappe codice, diagrammi di dipendenza e diagrammi classi per:Use tools such as code maps, dependency diagrams, and class diagrams to:
...
|Diagramma di dipendenza, convalida del livelloDependency diagram, layer validation|||Sì|Sì|Sì|
|Mappa codice|||Sì|Sì|Sì|
|Progettazione classi (basata su codice)||||Sì||

Per disegnare diagrammi di dipendenza, è necessario creare un progetto di modellazione come parte di una soluzione esistente o di una nuova soluzione. Questi diagrammi devono essere creati nel progetto di modellazione.
Gli elementi nei diagrammi di dipendenza si trovano nel progetto di modellazione, ma non vengono archiviati nel modello comune. Mappe codice e diagrammi classi .NET creati dal codice esistono al di fuori del progetto di modellazione.
...
Entrambi i team utilizzano anche la convalida delle dipendenze per assicurarsi che il codice in fase di sviluppo rimanga coerente con la progettazione. Vedere:

- [Mantenere il codice coerente con la progettazione](#ValidatingCode)

- [Descrivere l'architettura logica: diagrammi di dipendenzaDescribe the Logical Architecture: Dependency Diagrams](#DescribeLayers)

- [Convalidare il codice con i diagrammi delle dipendenze](../modeling/validate-code-with-layer-diagrams.md)

> [!NOTE]
> Alcune versioni di Visual Studio supportano la convalida delle dipendenze e le versioni di sola lettura delle mappe codice per la visualizzazione e la modellazione. Per sapere quali edizioni di Visual Studio supportano questa funzionalità, vedere [Supporto dell'edizione per gli strumenti](../modeling/what-s-new-for-design-in-visual-studio.md#VersionSupport)di architettura e modellazione .
...
Dinner Now e Lucerna comunicano tra loro e con le parti interessate del progetto utilizzando i diagrammi come linguaggio comune. Ad esempio, Dinner Now usa i diagrammi per eseguire queste attività:
...
Prima che i team eseguano il controllo delle modifiche, convalidano il codice rispetto ai test e alla progettazione eseguendo compilazioni che includono la convalida delle dipendenze e test automatizzati. Questo garantisce che il codice aggiornato non sia in conflitto con la progettazione e non interferisca con funzionalità precedentemente funzionanti.

### <a name="identify-changes-to-the-existing-system"></a>Identificazione delle modifiche al sistema esistente
...
### <a name="keep-code-consistent-with-the-design"></a><a name="ValidatingCode"></a>Mantenere il codice coerente con la progettazione
...
|-|-|
|*Diagramma delle dipendenze*<br /><br /> Vedere:<br /><br /> - [Creare diagrammi di dipendenza dal codiceCreate dependency diagrams from your code](../modeling/create-layer-diagrams-from-your-code.md)<br />- [Diagrammi di dipendenza: riferimentoDependency Diagrams: Reference](../modeling/layer-diagrams-reference.md)<br />- [Diagrammi di dipendenza: linee guidaDependency Diagrams: Guidelines](../modeling/layer-diagrams-guidelines.md)<br />- [Convalidare il codice con i diagrammi di dipendenzaValidate code with dependency diagrams](../modeling/validate-code-with-layer-diagrams.md)|L'architettura logica del codice.<br /><br /> Un diagramma di dipendenza organizza ed esegue il mapping degli elementi in una soluzione di Visual Studio a gruppi astratti *denominati layer*. Questi livelli identificano i ruoli, le attività o le funzioni che questi elementi eseguono nel sistema.<br /><br /> I diagrammi di dipendenza sono utili per descrivere la progettazione prevista del sistema e convalidare il codice in evoluzione rispetto a tale progettazione.<br /><br /> Per creare livelli, trascinare elementi da Esplora soluzioni, mappe codice, Visualizzazione classi e Visualizzatore oggetti. Per disegnare nuovi livelli, usare la casella degli strumenti o fare clic con il pulsante destro del mouse sulla superficie del diagramma.<br /><br /> Per visualizzare le dipendenze esistenti, fare clic con il pulsante destro del mouse sulla superficie del diagramma di dipendenza e quindi **scegliere Genera dipendenze**. Per specificare le dipendenze desiderate, disegnare nuove dipendenze.|

Ad esempio, il diagramma di dipendenza seguente descrive le dipendenze tra i livelli e il numero di elementi associati a ogni livello:For example, the following dependency diagram describes dependencies between layers and the number of artifacts that are associated with each layer:

![Diagramma di dipendenza del sistema di pagamento integratoDependency diagram of integrated payment system](../modeling/media/layer_integrated_dnlucerne.png)

 **Diagramma di dipendenza**

Per assicurarsi che i conflitti con la progettazione non si verifichino durante lo sviluppo del codice, i team usano la convalida delle dipendenze nelle compilazioni eseguite in Azure DevOps.To make sure that conflicts with the design do not occur during code development, the teams uses dependency validation on builds that are run on Azure DevOps. Creano anche un'attività MSBuild personalizzata per richiedere la convalida delle dipendenze nelle operazioni di archiviazione. Per raccogliere gli errori di convalida, usano report di compilazione.

Vedere:

- [Usare la finestra di progettazione visivaUse the visual designer](/azure/devops/pipelines/get-started-designer)

- [Check-in gated TFVC](/azure/devops/pipelines/build/triggers)
...
- Rapporti di *masterizzazione* giornaliera che mostrano se completeranno il lavoro pianificato nel tempo previsto. Altri rapporti analoghi vengono generati da Team Foundation Server per tenere traccia dello stato di avanzamento dei bug.
...
- [Informazioni sugli strumenti Agile e sulla gestione dei progetti Agile](/azure/devops/boards/backlogs/backlogs-overview?view=vsts)

- [Grafici, dashboard e widget (servizi DevOps di Azure)Charts, dashboards, and widgets (Azure DevOps Services)](/azure/devops/report/dashboards/overview?view=vsts)
...
Quando i team completano ogni attività, archiviano il codice nel controllo del codice sorgente e ricevono promemoria da Team Foundation Server, se dimenticano. Prima che Team Foundation Server accetti le archiviazioni, i team eseguono unit test e convalida delle dipendenze per verificare il codice rispetto ai test case e alla progettazione. Utilizzano Team Foundation Server per eseguire regolarmente compilazioni, unit test automatizzati e convalida delle dipendenze. Questo permette di verificare che il codice soddisfi i criteri seguenti:
...
Per assicurarsi che il codice sia conforme alla progettazione, i team configurano le compilazioni in Azure DevOps in modo da includere la convalida delle dipendenze. In caso di conflitti, viene generato un report con i dettagli.
...
## <a name="update-the-system-using-visualization-and-modeling"></a>Aggiornamento del sistema tramite visualizzazione e modellazione
...
- Trova aree di complessità, dipendenze naturali o modelli o altre aree che potrebbero trarre vantaggio dal miglioramento.
...
|**Diagramma**|**Oggetto della descrizione**|
|-|-|
|Diagramma delle dipendenze|L'architettura logica del sistema. Utilizzare la convalida delle dipendenze per assicurarsi che il codice rimanga coerente con la progettazione.<br /><br /> Per identificare le dipendenze esistenti o quelle previste, creare una mappa del codice e raggruppare gli elementi correlati. Per creare un diagramma di dipendenza, vedere:To create a dependency diagram, see:<br /><br /> - [Creare diagrammi di dipendenza dal codiceCreate dependency diagrams from your code](../modeling/create-layer-diagrams-from-your-code.md)<br />- [Diagrammi di dipendenza: linee guidaDependency Diagrams: Guidelines](../modeling/layer-diagrams-guidelines.md)|
...
### <a name="describe-the-logical-architecture-dependency-diagrams"></a><a name="DescribeLayers"></a>Descrivere l'architettura logica: diagrammi di dipendenzaDescribe the Logical Architecture: dependency Diagrams
 ...
 Per mantenere il codice coerente con la progettazione, Dinner Now e Lucerne utilizzano il diagramma di dipendenza seguente per convalidare il codice man mano che si evolve:

 ![Diagramma di dipendenza del sistema di pagamento integratoDependency diagram of integrated payment system](../modeling/media/layer_integrated_dnlucerne.png)

 **Diagramma delle dipendenze per Dinner Now integrato con LucernaDependency diagram for Dinner Now integrated with Lucerne**
...
#### <a name="drawing-a-dependency-diagram"></a>Disegno di un diagramma di dipendenzaDrawing a Dependency Diagram

Un diagramma di dipendenza presenta le funzionalità principali seguenti:A dependency diagram has the following major features:
...
- *Collegamento* : un'associazione tra un livello e un elemento.
  ...
  - Se un livello contiene altri livelli collegati a elementi, anche il livello contenitore sarà collegato a tali elementi nonostante il numero raffigurato sul livello contenitore non includa quegli elementi.
    ...
    Per visualizzare gli elementi collegati a un livello, fare clic con il pulsante destro del mouse sulla dipendenza, quindi **scegliere Visualizza collegamenti** per aprire Esplora **livelli.**
...
- *Dipendenza* : indica che un livello può usare le funzionalità di un altro livello, ma non viceversa. *Dipendenza bidirezionale* : indica che un livello può usare le funzionalità di un altro livello e viceversa.
     ...
     Per visualizzare le dipendenze esistenti nel diagramma di dipendenza, fare clic con il pulsante destro del mouse sulla superficie del diagramma e quindi **scegliere Genera dipendenze**. Per descrivere le dipendenze previste, disegnarne altre.
...
#### <a name="summary-strengths-of-dependency-diagrams"></a>Riepilogo: punti di forza dei diagrammi di dipendenzaSummary: Strengths of Dependency Diagrams

I diagrammi di dipendenza consentono di:Dependency diagrams help you:
