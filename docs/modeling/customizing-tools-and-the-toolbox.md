 ...
 **Nella finestra Dettagli DSL direttiva di connessione di collegamento**
 ...
 ![Nella finestra Dettagli DSL direttiva di connessione di collegamento](../modeling/media/connectionbuilder_4b.png)
...
```
 ...
 È possibile usare un codice simile, ad esempio, per impedire agli utenti di creare cicli con collegamenti padre-figlio. Queste restrizioni sono considerate vincoli 'rigidi' in quanto gli utenti non possono violare in qualsiasi momento. È anche possibile creare controlli di convalida 'flessibili' che gli utenti possono ignorare temporaneamente tramite la creazione di configurazioni non valide che non possono essere salvate.
...
### <a name="good-practice-in-defining-connection-builders"></a>Procedure consigliate per la definizione dei generatori di connessioni
 ...
 Si usa codice personalizzato da applicare vincoli 'rigidi', ma è necessario considerare se gli utenti devono essere in grado di eseguire temporaneamente connessioni non valide. Se si vuole concedere questa possibilità, è possibile modificare i vincoli in modo che le connessioni non vengano convalidate fino a quando gli utenti non tentano di salvare le modifiche.
