---
title: 'Diagrammi di dipendenza: linee guida'
ms.date: 11/04/2016
ms.topic: conceptual
helpviewer_keywords:
...
1.  [Creare un diagramma di dipendenze](#Create) per l'intera applicazione o per un livello nell'applicazione.
...
4.  [Modificare i livelli e dipendenze](#EditArchitecture) per mostrare aggiornato progettazione che si desidera che il codice in modo da riflettere.

5.  [Progettare nuove aree dell'applicazione](#NewAreas) creando livelli per rappresentare i blocchi di architettura principali o i componenti e la definizione di dipendenze per mostrare in modo ogni livello Usa gli altri.
...
9. [Includere la convalida dei livelli nel processo di compilazione](#BuildValidation) per garantire che il codice continua a essere conforme alla progettazione.
...
-   **Non è necessario collegare la maggior parte delle classi e altri artefatti ai livelli** poiché si trovano in elementi più grandi quali spazi dei nomi che sono già stati collegati ai livelli.
