...
Attendere un codice scritto a mano inoltre al codice generato. È insolito per uno schema di generazione di codice per essere in grado di tenere conto per tutte le possibili varianti che potrebbero verificarsi. Pertanto, si deve prevedere di aggiungere o modificare parte del codice generato. In cui il materiale generato è in un linguaggio .NET, ad esempio C# o Visual Basic, due strategie sono particolarmente utili:
