...
|Campo di testo visualizza un valore derivato da altri valori.|Eseguire il mapping dell'elemento Decorator di testo a una proprietà di Archiviazione calcolata. Per altre informazioni, vedere [Calculated and Custom Archiviazione Properties](../modeling/calculated-and-custom-storage-properties.md).|
...
|Elimina, ricollega o ricollega elementi correlati quando viene eliminato un elemento.|Impostare il **valore Propagates Delete** di un ruolo relazione. Per effetti più complessi, eseguire `ShouldVisitRelationship` l'override `ShouldVisitRolePlayer` dei metodi e nella classe , definiti in `MyDslDeleteClosure` **DomainModel.cs.**|
