...
description: Si noti che un Visual Studio di testo T4 inizia in genere con una direttiva di modello, che specifica come deve essere elaborato il modello.
...
ms.locfileid: "126637347"
---
# <a name="t4-template-directive"></a>Direttiva template T4

Un Visual Studio di testo T4 inizia in genere con una direttiva , che specifica `template` come deve essere elaborato il modello. Un modello di testo e qualsiasi file in esso incluso non devono contenere più di una direttiva template.
...
Poiché il tipo di questa proprietà dipende dal tipo di host, è utile solo se si scrive un modello di testo che funziona solo con un host specifico. È applicabile ai modelli [in fase di progettazione,](../modeling/design-time-code-generation-by-using-t4-text-templates.md)ma non [ai modelli di run-time.](../modeling/run-time-text-generation-with-t4-text-templates.md)

Quando è e si usa Visual Studio, è possibile eseguire il cast a IServiceProvider per accedere alle Visual Studio `hostspecific` `true` `this.Host` funzionalità. È inoltre possibile utilizzare `Host.ResolvePath(filename)` per ottenere il percorso assoluto di un file nel progetto. Ad esempio:
...
È possibile utilizzare l'ereditarietà tra i modelli di testo della fase di esecuzione per creare un modello di base che disponga di molte varianti derivate. I modelli di run-time sono quelli la cui proprietà **Strumento** personalizzato è impostata su **TextTemplatingFilePreprocessor.** Un modello della fase di esecuzione genera codice che è possibile chiamare nell'applicazione per creare il testo definito nel modello. Per altre informazioni, vedere [Generazione di testo di run-time con modelli di testo T4.](../modeling/run-time-text-generation-with-t4-text-templates.md)
...
Per altre informazioni, vedere ["Inheritance in Text Templates" (Ereditarietà nei modelli di testo) nel blog di Jones.](/archive/blogs/garethj/vs2010-sp1-t4-template-inheritance-part-i-sample-metadata)
...
Questo attributo può essere utile anche se i nomi file assoluti nei pragma causano unioni di distrazione nel controllo del codice sorgente.
