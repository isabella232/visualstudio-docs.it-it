---
title: 'Riga di comando del profiler: instrumentare ASP.NET app statica, ottenere dati di memoriaProfiler command line: Instrument static ASP.NET app, get memory data'
ms.date: 11/04/2016
ms.topic: conceptual
...
## <a name="end-the-profiling-session"></a>Terminare la sessione di profilatura
 Per terminare una sessione di [!INCLUDE[vstecasp](../code-quality/includes/vstecasp_md.md)] profilatura, chiudere l'applicazione Web e quindi utilizzare il [!INCLUDE[vstecasp](../code-quality/includes/vstecasp_md.md)] comando **IISReset** di Internet Information Services (IIS) per chiudere il processo di lavoro. Chiamare l'opzione **VSPerfCmd** [/shutdown](../profiling/shutdown.md) per disattivare il profiler e chiudere il file di dati di profilatura. Il comando **VSPerfClrEnv /globaloff** cancella le variabili di ambiente di profilatura. È necessario riavviare il computer per applicare le nuove impostazioni di ambiente.
