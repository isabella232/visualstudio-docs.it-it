...
| **Raccogliere dati di concorrenza:** Usare il metodo di concorrenza per raccogliere dati sui conflitti di risorse e sull'attività dei thread che mostrano l'utilizzo della CPU, la conflitti tra thread, la migrazione dei thread, i ritardi di sincronizzazione, le aree di I/O sovrapposte e altri eventi di sistema. | -   [Raccogliere dati di concorrenza](../profiling/collecting-concurrency-data-for-a-service-by-using-the-profiler-command-line.md) |
