...
2. Inizializzare le variabili di ambiente di profilatura. Digitare:
   ...
   - Le opzioni **/globalsamplegc** e **/globalsamplegclife** specificano il tipo di dati di memoria da raccogliere.
       ...
       |Opzione|Descrizione|
       |------------|-----------------|
       |**/globalsamplegc (esempiogc)**|Abilita la raccolta dei dati di allocazione della memoria.|
       |**/globalsamplegclife (esempioglobalegclife)**|Abilita la raccolta dei dati di allocazione della memoria e dei dati di durata degli oggetti.|
...
5. Avvia il profiler. Digitare:
    ...
    **VSPerfCmd**  [/start](../profiling/start.md) **:esempio**  [/output](../profiling/output.md) **:** `OutputFile` [`Options`]
...
7. Connettere il profiler al processo di lavoro [!INCLUDE[vstecasp](../code-quality/includes/vstecasp_md.md)]. Digitare:
   ...
   - **/targetclr:** `Version` specifica la versione di Common Language Runtime (CLR) da profilare quando più versioni del runtime vengono caricate in un'applicazione.
