...
description: Informazioni su come la visualizzazione Righe dei dati sui problemi elenca i dati sulle prestazioni per le istruzioni in esecuzione durante la raccolta degli esempi nell'esecuzione della profilatura.
ms.date: 11/04/2016
ms.topic: conceptual
helpviewer_keywords:
- Lines view
...
|**Nome riga**|Identificatore generato dal profiler della riga. L'identificatore usa la sintassi `SourceFile` **seguente: ;[** `LineNumberStart` **,**`CharacterStart` **]->; [**`LineNumberEnd`**,**`CharacterEnd`**]**|
