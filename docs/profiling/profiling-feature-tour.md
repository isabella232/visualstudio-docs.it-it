...
Gli strumenti nel Profiler prestazioni sono destinati a fornire l'analisi per **le build di** versione. Nell'Profiler prestazioni è possibile raccogliere informazioni di diagnostica mentre l'app è in esecuzione e quindi esaminare le informazioni raccolte dopo l'arresto dell'app (un'analisi post-mortem).
...
In alcuni scenari, la finestra consente di selezionare [più strumenti di profilatura](../profiling/use-multiple-profiler-tools-simultaneously.md). Gli strumenti come Utilizzo CPU possono visualizzare dati complementari che agevolano l'analisi. È anche possibile usare il [profiler della riga di comando](../profiling/profile-apps-from-command-line.md) per abilitare scenari che coinvolgono più strumenti di profilatura.
...
Quando si usa lo strumento Utilizzo CPU integrato nel debugger, aprire la finestra Strumento di diagnostica (se è chiusa, scegliere **Debug/Finestre/Mostra** Strumenti di diagnostica). Durante il debug, aprire la  **visualizzazione Riepilogo** e selezionare **Registra profilo CPU**.
...
![.NET Async stato arrestato](../profiling/media/async-tool-opened.png ".NET Async arresto dello strumento")
...
Nelle app UWP è possibile abilitare **l'analisi dell'interfaccia utente** **Strumenti di diagnostica** finestra. Lo strumento cerca i comuni problemi di prestazioni o accessibilità e li indica nella visualizzazione **Eventi** durante il debug. Le descrizioni degli eventi contengono informazioni che possono essere utili per risolvere i problemi.
...
::: moniker range=">=vs-2019"
Nel Visual Studio 2019, il Esplora prestazioni legacy e gli strumenti di profilatura correlati, ad esempio la Creazione guidata sessione di prestazioni, sono stati ripiegati nel Profiler prestazioni, che è possibile aprire usando Debug  >  **Profiler prestazioni**. Nel Profiler prestazioni, gli strumenti di diagnostica disponibili dipendono dalla destinazione scelta e dal progetto di avvio aperto corrente. Lo strumento Utilizzo CPU offre la funzionalità di campionamento precedentemente supportata nella Creazione guidata prestazioni. Lo strumento Strumentazione fornisce la funzionalità di profilatura instrumentata (per conteggi e durate precise delle chiamate) presente nella Creazione guidata sessione di prestazioni. Gli strumenti aggiuntivi per la memoria vengono visualizzati anche nel Profiler prestazioni.
::: moniker-end
...
|[Contatori .NET](../profiling/dotnet-counters-tool.md)|sì (solo .NET Core)|no|sì (ASP.NET Core)|
|[Database](../profiling/analyze-database.md)|sì (solo .NET Core)|no|sì (ASP.NET Core)|
