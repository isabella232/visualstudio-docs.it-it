...
description: Informazioni su come aggiungere controlli a un documento di Microsoft Word e associare i controlli ai dati in fase di esecuzione.
...
2. Se il progetto è destinato a [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] o , aggiungere un riferimento [!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)] *all'Microsoft.Office.Tools.Word.v4.0.Utilities.dll* assembly. Tale riferimento è necessario per aggiungere controlli Windows Form a livello di codice al documento più avanti in questa procedura dettagliata.
...
7. Fare clic su **Fine**.
   ...
   - Oggetto TableAdapter denominato `CustomerTableAdapter` . Questo TableAdapter può essere usato per leggere e scrivere dati in `AdventureWorksLTDataSet` . Per altre informazioni, vedere [Panoramica di TableAdapter.](../data-tools/fill-datasets-by-using-tableadapters.md#tableadapter-overview)
...
3. Aggiungere il codice seguente al metodo `ThisAddIn_Startup` . Viene generato un elemento host che estende il documento. Per altre informazioni, vedere Estendere documenti di Word e cartelle di lavoro di Excel nei componenti aggiuntivi [VSTO in fase di esecuzione.](../vsto/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md)
