...
|Le classi di elementi host, ad esempio `ThisWorkbook`, `Sheet`*n*, `ThisDocument`e `ThisAddIn`, non forniscono più un metodo di `Dispose` che è possibile sostituire.|È necessario trasferire qualsiasi codice nell'override del metodo `Dispose` nel gestore eventi `Shutdown` nella classe degli elementi host, ad esempio `ThisAddIn_Shutdown`, e rimuovere l'override del metodo `Dispose` dalla classe degli elementi host.|
