...
description: Informazioni su come gli strumenti di sviluppo di Office in Visual Studio interfacce di estendibilità che consentono di personalizzare le funzionalità dell'interfaccia utente.
...
## <a name="overview-of-extensibility-interfaces"></a>Panoramica delle interfacce di estendibilità
 ...
 Per altre informazioni sulle classi e le finestre di progettazione Visual Studio per queste [](../vsto/ribbon-designer.md)funzionalità, vedere Riquadri attività [personalizzati,](../vsto/custom-task-panes.md)finestra di progettazione della barra multifunzione e Creare aree del modulo [di Outlook.](../vsto/creating-outlook-form-regions.md)

## <a name="extensibility-interfaces-you-can-implement-in-a-vsto-add-in"></a>Interfacce di estendibilità che è possibile implementare in un componente aggiuntivo VSTO
...
|---------------|-----------------|------------------|
|<xref:Microsoft.Office.Core.IRibbonExtensibility>|Implementare questa interfaccia per personalizzare l'interfaccia utente della barra multifunzione. **Nota:**  È possibile aggiungere un **elemento barra multifunzione (XML)** a un progetto per generare un'implementazione predefinita nel componente aggiuntivo <xref:Microsoft.Office.Core.IRibbonExtensibility> VSTO. Per altre informazioni, vedere [Ribbon XML](../vsto/ribbon-xml.md).|Excel<br /><br /> [!INCLUDE[InfoPath_15_short](../vsto/includes/infopath-15-short-md.md)]<br /><br /> InfoPath 2010<br /><br /> Outlook<br /><br /> PowerPoint<br /><br /> Project<br /><br /> Visio<br /><br /> Word|
...
## <a name="use-extensibility-interfaces"></a>Usare le interfacce di estendibilità
 ...
 Per un'applicazione di esempio che illustra come implementare le interfacce , e in un componente aggiuntivo VSTO per Outlook, vedere l'esempio di gestione dell'interfaccia utente negli esempi di sviluppo <xref:Microsoft.Office.Core.IRibbonExtensibility> <xref:Microsoft.Office.Core.ICustomTaskPaneConsumer> per <xref:Microsoft.Office.Interop.Outlook.FormRegionStartup> [Office.](../vsto/office-development-samples.md)
...
- La classe `TaskPaneUI` fornisce l'interfaccia utente del riquadro attività. Gli attributi della classe `TaskPaneUI` rendono la classe visibile a COM, permettendo alle applicazioni di Microsoft Office di individuare la classe. In questo esempio, <xref:System.Windows.Forms.UserControl>è un'interfaccia utente vuota, ma è possibile aggiungere i controlli modificando il codice.
  ...
  Per altre informazioni sull'implementazione di , vedere Creare riquadri attività <xref:Microsoft.Office.Core.ICustomTaskPaneConsumer> personalizzati in Office System [2007](/previous-versions/office/developer/office-2007/aa338197(v=office.12)) nella documentazione Microsoft Office.
