...
## <a name="overview-of-extensibility-interfaces"></a>Panoramica delle interfacce di estendibilità
 ...
 Per ulteriori informazioni sulle classi e le finestre di progettazione fornite da Visual Studio per queste funzionalità, vedere [riquadri attività personalizzati](../vsto/custom-task-panes.md), [finestra di progettazione della barra multifunzione](../vsto/ribbon-designer.md)e [creazione di aree del modulo di Outlook](../vsto/creating-outlook-form-regions.md).
...
## <a name="use-extensibility-interfaces"></a>Usare le interfacce di estendibilità
 ...
 Per un'applicazione di esempio che illustra come implementare le <xref:Microsoft.Office.Core.IRibbonExtensibility> <xref:Microsoft.Office.Core.ICustomTaskPaneConsumer> interfacce, e <xref:Microsoft.Office.Interop.Outlook.FormRegionStartup> in un componente aggiuntivo VSTO per Outlook, vedere l'esempio di gestione dell'interfaccia utente in [esempi di sviluppo per Office](../vsto/office-development-samples.md).
...
- La classe `TaskPaneUI` fornisce l'interfaccia utente del riquadro attività. Gli attributi della classe `TaskPaneUI` rendono la classe visibile a COM, permettendo alle applicazioni di Microsoft Office di individuare la classe. In questo esempio, <xref:System.Windows.Forms.UserControl>è un'interfaccia utente vuota, ma è possibile aggiungere i controlli modificando il codice.
  ...
  Per ulteriori informazioni sull'implementazione di <xref:Microsoft.Office.Core.ICustomTaskPaneConsumer> , vedere la pagina relativa alla [creazione di riquadri attività personalizzati in Office System 2007](/previous-versions/office/developer/office-2007/aa338197(v=office.12)) nella documentazione di Microsoft Office.

### <a name="example-of-overriding-the-requestservice-method"></a>Esempio di override del metodo del RequestService
