...
description: Informazioni sulle nozioni di data binding in un progetto a livello di documento e sul fatto che un singolo campo dati in un database di SQL Server sia associato a un intervallo denominato in Microsoft Excel.
...
1. Creare un Excel cartella di lavoro con il nome **My Simple Data Binding** usando Visual Basic o C#. Assicurarsi che **l'opzione Crea un nuovo documento** sia selezionata. Per altre informazioni, [vedere Procedura: Creare Office progetti in Visual Studio](../vsto/how-to-create-office-projects-in-visual-studio.md).
...
1. Aggiungere un gestore eventi per l'evento del pulsante e aggiungere il codice seguente per spostare indietro <xref:System.Windows.Forms.Control.Click> `Button2` di uno la posizione:
...
1. Aggiungere un gestore eventi per <xref:System.Windows.Forms.Control.Click> l'evento del pulsante e aggiungere il `Button3` codice seguente per far avanzare di uno la posizione:
...
## <a name="test-the-application"></a>Testare l'applicazione
 È ora possibile testare la cartella di lavoro per assicurarsi che sia possibile esplorare i record nel database.
...
## <a name="next-steps"></a>Passaggi successivi
 Questa procedura dettagliata illustra le nozioni di base dell'associazione di un intervallo denominato a un campo in un database. Ecco alcune possibili attività successive:
...
- Associare celle a più colonne in una tabella anziché a un solo campo. Per altre informazioni, vedere [Walkthrough: Complex data binding in a document-level project](../vsto/walkthrough-complex-data-binding-in-a-document-level-project.md).

- Usare un <xref:System.Windows.Forms.BindingNavigator> controllo per scorrere i record. Per altre informazioni, vedere [Procedura: Esplorare i dati con il controllo BindingNavigator Windows Forms.](/dotnet/framework/winforms/controls/bindingnavigator-control-overview-windows-forms)
