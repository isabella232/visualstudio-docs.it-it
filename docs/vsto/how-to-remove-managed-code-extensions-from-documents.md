...
1. Nel codice di personalizzazione chiamare il metodo <xref:Microsoft.Office.Tools.Word.Document.RemoveCustomization%2A> (per Word) o il metodo <xref:Microsoft.Office.Tools.Excel.Workbook.RemoveCustomization%2A> (per Excel). Questo metodo deve essere chiamato solo dopo che la personalizzazione non è più necessaria.
