...
## <a name="troubleshoot-office-solutions-by-using-the-event-viewer"></a>Risolvere i problemi di soluzioni Office utilizzando il Visualizzatore eventi  
...
## <a name="change-the-assembly-name-causes-conflicts"></a>Modifica il nome dell'assembly causa conflitti  
 Se si modifica il **nome dell'Assembly** valore il **applicazione** pagina del **creazione progetti** dopo aver già distribuito una soluzione, gli strumenti di pubblicazione modificheranno il Pacchetto di installazione abbia una *Setup.exe* file e due manifesti di distribuzione. Se si distribuiscono due file manifesto, potrebbero verificarsi le condizioni seguenti:  
...
## <a name="check-for-updates-takes-a-long-time"></a>Verificare la disponibilità per aggiornamenti richiede molto tempo  
...
## <a name="cant-update-or-publish-to-a-network-file-share"></a>Non è possibile aggiornare o pubblicare in una condivisione di file di rete  
 Soluzioni Office che si trovano in una condivisione di file di rete potrebbe essere visualizzato un messaggio fuorviante durante gli aggiornamenti se la soluzione *Setup.exe* file è bloccato in un processo durante l'aggiornamento viene pubblicata. Il messaggio può essere analogo al seguente: "Impossibile aggiungere 'setup.exe' al sito Web. Il file 'setup.exe' esiste già nel sito Web".  
...
## <a name="publish-using-localhost-can-cause-installation-problems"></a>Esegue la pubblicazione mediante 'Localhost' può causare problemi di installazione  
...
## <a name="prompt-to-manually-uninstall-appears-when-you-publish-and-install-a-solution-on-the-development-computer"></a>Verrà visualizzata la richiesta di disinstallazione manuale quando si pubblica e si installa una soluzione nel computer di sviluppo  
...
## <a name="uncaught-exception-or-method-not-found-error-when-you-install-a-solution"></a>Eccezione non rilevata o un metodo non ha trovato errore quando si installa una soluzione  
...
## <a name="manifest-registry-keys-for-add-ins-change-after-an-installshield-limited-edition-project-is-built"></a>Manifesto le chiavi del Registro di sistema per la modifica di componenti aggiuntivi dopo la compilazione di un progetto InstallShield Limited Edition  
 La chiave del Registro di sistema del manifesto che fa parte di un'installazione del componente aggiuntivo VSTO programma talvolta cambia da *VSTO* alla *. manifest* quando si compila un progetto InstallShield Limited Edition.  
...
## <a name="the-clickonce-installer-for-your-office-solution-doesnt-install-the-primary-interop-assemblies"></a>Il programma di installazione ClickOnce per la soluzione Office non installa l'assembly di interoperabilità primari  
...
## <a name="reinstall-office-solutions-causes-an-argument-out-of-range-exception"></a>Reinstallare un argomento fuori eccezione compreso nell'intervallo vengono illustrate le cause di soluzioni Office  
...
## <a name="cant-install-a-clickonce-solution-by-opening-the-deployment-manifest-from-the-web"></a>Non è possibile installare una soluzione ClickOnce aprendo il manifesto di distribuzione dal web  
 Gli utenti possono installare soluzioni Office aprendo il manifesto della distribuzione dal Web. Tuttavia, alcune installazioni dei blocchi di Internet Information Services (IIS) di *VSTO* estensione di file. È necessario definire il tipo MIME in IIS prima di utilizzarlo per distribuire una soluzione Office.  
...
## <a name="see-also"></a>Vedere anche  
 [Risolvere i problemi delle distribuzioni ClickOnce](/visualstudio/deployment/troubleshooting-clickonce-deployments)   
