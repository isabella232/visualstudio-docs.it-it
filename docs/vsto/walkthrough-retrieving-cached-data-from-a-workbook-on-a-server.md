...
description: Informazioni su come recuperare dati da un set di dati memorizzato nella cache di una cartella di lavoro di Microsoft Excel senza avviare Excel usando la classe ServerDocument.
...
# <a name="walkthrough-retrieve-cached-data-from-a-workbook-on-a-server"></a>Procedura dettagliata: recuperare i dati memorizzati nella cache da una cartella di lavoro in un server
  Questa procedura dettagliata illustra come recuperare dati da un set di dati memorizzato nella cache di una cartella di lavoro di Excel Microsoft Office senza avviare Excel usando la <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> classe.
...
5. Fare clic su **OK**.
     ...
     [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] aggiunge il progetto **DataReader** a **Esplora soluzioni** e apre il file di codice *Program.cs* o *Module1. vb* .
...
## <a name="retrieve-data-from-the-cached-dataset-by-using-the-console-application"></a>Recuperare dati dal set di dati memorizzato nella cache tramite l'applicazione console
 Utilizzare la <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> classe nell'applicazione console per leggere i dati in un oggetto locale `AdventureWorksLTDataSet` . Per confermare che il set di dati locale è stato inizializzato con i dati del set di dati memorizzato nella cache, l'applicazione Visualizza il numero di righe nel set di dati locale.
