...
description: Informazioni su come recuperare dati da un set di dati memorizzato nella cache in una cartella di lavoro di Microsoft Excel senza avviare Excel usando la classe ServerDocument.
...
# <a name="walkthrough-retrieve-cached-data-from-a-workbook-on-a-server"></a>Procedura dettagliata: Recuperare i dati memorizzati nella cache da una cartella di lavoro in un server
  Questa procedura dettagliata illustra come recuperare i dati da un set di dati memorizzato nella cache in una cartella di lavoro Microsoft Office Excel senza avviare Excel usando la <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> classe .
...
5. Fare clic su **OK**.
     ...
     [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]aggiunge il **progetto DataReader** a **Esplora soluzioni** apre il file di codice *Program.cs* o *Module1.vb.*
...
8. Aggiungere il codice seguente al metodo `Main` . Questo codice dichiara gli oggetti seguenti:
   ...
   - Oggetto <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> da utilizzare per accedere alla cache dei dati nella cartella di lavoro.
     ...
     > [!NOTE]
     > Il codice seguente presuppone che la cartella di lavoro sia salvata usando *l'estensione xlsx.* Se la cartella di lavoro nel progetto ha un'estensione diversa, modificare il percorso in base alle esigenze.
...
1. In **Esplora soluzioni** fare clic con il pulsante destro del mouse **sul progetto DataReader,** scegliere **Debug** e quindi fare clic su Avvia **nuova istanza.**
     ...
     Verificare che l'applicazione segnala che il set di dati locale contiene 295 righe.
