...
# <a name="walkthrough-change-cached-data-in-a-workbook-on-a-server"></a>Procedura dettagliata: modificare i dati memorizzati nella cache in una cartella di lavoro in un server
  Questa procedura dettagliata illustra come modificare un set di dati memorizzato nella cache di una cartella di lavoro di Excel Microsoft Office senza avviare Excel usando la classe <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument>.
...
## <a name="change-data-in-the-cached-dataset-by-using-the-console-application"></a>Modificare i dati nel set di dati memorizzato nella cache utilizzando l'applicazione console
 Utilizzare la classe <xref:Microsoft.VisualStudio.Tools.Applications.ServerDocument> nell'applicazione console per leggere i dati in un oggetto `AdventureWorksLTDataSet` locale, modificare questi dati e quindi salvarli nuovamente nel set di dati memorizzato nella cache.
...
9. Aggiungere il codice seguente al metodo `Main`, dopo il codice aggiunto nel passaggio precedente. Mediante il codice vengono effettuate le seguenti attività:
   ...
   - Modifica il valore `ListPrice` di ogni prodotto nella tabella Product del set di dati.
...
## <a name="test-the-workbook"></a>Testare la cartella di lavoro
 Quando si apre la cartella di lavoro, il <xref:Microsoft.Office.Tools.Excel.ListObject> ora Visualizza le modifiche apportate alla colonna `ListPrice` dei dati nel set di dati memorizzato nella cache.
...
3. Verificare che il valore nella colonna **ListPrice** per la prima riga del <xref:Microsoft.Office.Tools.Excel.ListObject> ora sia 1574,65.
