...
-   [Utilizzare Progettazione aree di form](#UsingFormRegionDesigner)  
  
-   [Utilizzare un'area del modulo progettata in Outlook](#UsingFormRegionDesignedOutlook)  
...
###  <a name="CreatingFormRegion"></a> Creare una nuova area del modulo utilizzando la procedura guidata  
...
-   Una risorsa di archiviazione di modulo di Outlook (*OFS*) file.  
...
###  <a name="AddingExistingFormRegion"></a> Aggiungere un'area del modulo esistente al progetto  
 ...
 L'area del modulo esistente deve avere un file di codice (*vb* oppure *cs*); non è possibile aggiungere Outlook Form Storage (*OFS*) i file utilizzando il **Aggiungi elemento esistente** finestra di dialogo. Tuttavia, è possibile creare un'area del modulo nuova importando un file ofs (Outlook From Storage). Per altre informazioni, vedere [procedura: aggiungere un'area del modulo a un progetto di componente aggiuntivo di Outlook](../vsto/how-to-add-a-form-region-to-an-outlook-add-in-project.md).  
  
##  <a name="UsingFormRegionDesigner"></a> Utilizzare Progettazione aree di form  
...
##  <a name="UsingFormRegionDesignedOutlook"></a> Importare un'area del modulo progettata in Outlook  
 ...
 È possibile importare aree del modulo in un progetto di componente aggiuntivo VSTO di Outlook mediante il **nuova area del modulo di Outlook** procedura guidata. Nel **selezionare come si desidera creare l'area del modulo** selezionare **importare un file (OFS) di Outlook Form Storage**. È quindi possibile passare al percorso di un file OFS (*OFS*) file. (Outlook Salva le aree del modulo come *OFS* file.)  
  
 Il **nuova area del modulo di Outlook** guidata copia il *OFS* file alla directory del progetto e aggiunge i riferimenti ai controlli per il file di progettazione aree di modulo. È possibile quindi gestire gli eventi di controllo nel file di codice dell'area del modulo.  
 ...
 Per gestire gli eventi in un progetto C#, sottoscrivere gli eventi di controllo nel metodo <xref:Microsoft.Office.Tools.Outlook.FormRegionControl.FormRegionShowing>. Per altre informazioni, vedere [procedura: sottoscrivere e annullare la sottoscrizione di eventi &#40;C&#35; Guida alla programmazione&#41;](/dotnet/csharp/programming-guide/events/how-to-subscribe-to-and-unsubscribe-from-events).  
...
##  <a name="AddingCustomCode"></a> Aggiungere codice personalizzato a un'area del modulo  
 ...
 È possibile usare classi, metodi e proprietà nello spazio dei nomi `Microsoft.Office.Interop.Outlook` per eseguire la maggior parte delle attività di programmazione di Outlook. Per ulteriori informazioni sul modello a oggetti di Outlook, vedere [Cenni preliminari sul modello a oggetti di Outlook](../vsto/outlook-object-model-overview.md). Per esempi di attività tipiche che si avvalgono del modello a oggetti di Outlook, vedere [soluzioni Outlook](../vsto/outlook-solutions.md).  
...
|Titolo|Descrizione|  
|-----------|-----------------|  
|[Linee guida per creano aree del modulo di Outlook](../vsto/guidelines-for-creating-outlook-form-regions.md)|Fornisce informazioni che consentono di ottimizzare le aree del modulo ed evitare eventuali problemi.|  
