...
1. Creare un nuovo progetto nel computer che esegue Visual Studio.
    ...
    Scegliere il modello corretto. In questo esempio scegliere **ASP.NET App Web (.NET Framework)** o (solo per C#) **ASP.NET Core Web Application** e quindi selezionare **OK.**

    Se i modelli di progetto specificati non sono visualizzati, passare al collegamento **Apri** Programma di installazione di Visual Studio nel riquadro sinistro della finestra **di** dialogo Nuovo progetto. Verrà avviato il Programma di installazione di Visual Studio. Installare il carico **di lavoro ASP.NET sviluppo Web e** web.
