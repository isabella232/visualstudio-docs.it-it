...
description: Eseguire il debug delle estensioni SharePoint strumenti in Visual Studio. Eseguire SharePoint le estensioni degli strumenti nell'istanza sperimentale o nell'istanza normale di Visual Studio.
...
## <a name="debug-extensions-in-the-experimental-instance-of-visual-studio"></a>Eseguire il debug delle estensioni nell'istanza sperimentale di Visual Studio
 Per proteggere l'ambiente di sviluppo Visual Studio dal danneggiamento accidentale da estensioni non testate, Visual Studio SDK fornisce un'istanza di Visual Studio alternativa, denominata istanza *sperimentale*, che è possibile usare per installare e testare le estensioni. Si sviluppano nuove estensioni usando l'istanza regolare di Visual Studio, ma si esegue il debug e l'esecuzione nell'istanza sperimentale. Per altre informazioni, vedere [Istanza sperimentale.](../extensibility/the-experimental-instance.md)
...
- [Procedura dettagliata: Chiamare nel modello a oggetti SharePoint client in un'Esplora server personalizzata](../sharepoint/walkthrough-calling-into-the-sharepoint-client-object-model-in-a-server-explorer-extension.md)
...
5. Nell'istanza Visual Studio di cui è in corso il debug, eseguire i passaggi necessari per eseguire il comando.
