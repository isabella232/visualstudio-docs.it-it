...
description: Individuare i problemi o gli avvisi che possono verificarsi quando si esegue il debug di soluzioni SharePoint tramite il debugger di Visual Studio.
...
```
 ...
 Come illustrato nell'esempio seguente, un riferimento a un campo in un tipo di contenuto deve essere definito usando il formato dell'elemento vuoto ( \<FieldRef /> ), non usando gli elementi Start/End ( \<FieldRef> \</FieldRef> ):
...
- \<*model name*> contiene gli errori di attivazione del tipo di contenuto esterno seguenti...
...
## <a name="sharepoint-debugging-session-terminated-by-iis"></a>Sessione di debug di SharePoint terminata da IIS
 Questo problema si verifica se si imposta un punto di interruzione in una [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] soluzione SharePoint, si preme il tasto **F5** per eseguirlo, quindi si rimane in corrispondenza di un punto di interruzione superiore a 90 secondi.
