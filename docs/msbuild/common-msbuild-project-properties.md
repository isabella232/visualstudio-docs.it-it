...
| BuildProjectReferences | Valore booleano che indica se i riferimenti al progetto sono compilati da MSBuild. Impostato automaticamente su `false` se si compila il progetto in Visual Studio Integrated Development Environment (IDE), `true` in caso contrario. È possibile specificare `-p:BuildProjectReferences=false` nella riga di comando per evitare di controllare che i progetti di riferimento siano aggiornati. |
...
| CscToolPath | Il percorso di *csc. exe*, il C# compilatore. |
...
| GenerateFullPaths | (C#) Generare percorsi completi per i nomi di file nell'output utilizzando l'opzione del compilatore [-fullpaths](/dotnet/csharp/language-reference/compiler-options/fullpaths-compiler-option) . |
