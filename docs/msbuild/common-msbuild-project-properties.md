...
| BaseOutputPath | Tutti | Specifica il percorso di base del file di output. Se è impostata, MSBuild verrà utilizzato `OutputPath = $(BaseOutputPath)\$(Configuration)\` . Esempio di sintassi: `<BaseOutputPath>c:\xyz\bin\</BaseOutputPath>` |
...
| DisableFastUpToDateCheck | Tutti | Valore booleano che si applica solo Visual Studio. Il Visual Studio compilazione usa un processo denominato FastUpToDateCheck per determinare se un progetto deve essere ricompilato per essere aggiornato. Questo processo è più veloce rispetto all'MSBuild per determinare questo problema. L'impostazione della proprietà DisableFastUpToDateCheck su consente di ignorare lo strumento di gestione compilazione Visual Studio e forzarlo a usare MSBuild per determinare se il progetto è `true` aggiornato. |
...
| SkipInvalidConfigurations | Tutti | Quando , genera un avviso in combinazioni di piattaforma e configurazione non valide, ma non genera errori di compilazione. Quando o non è definito `true` `false` (impostazione predefinita), genera un errore. |
