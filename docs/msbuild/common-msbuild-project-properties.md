...
| BuildProjectReferences | Tutti | Valore booleano che indica se i riferimenti al progetto vengono compilati MSBuild. Impostare automaticamente su se si compila il progetto nell'ambiente Visual Studio di sviluppo `false` integrato (IDE), in caso `true` contrario. È possibile specificare `-p:BuildProjectReferences=false` nella riga di comando per evitare di controllare che i progetti di riferimento siano aggiornati. |
