...
| Gli | Riservata o nota | Descrizione |
|----------------------------------|------------------------| - |
...
| `MSBuildRuntimeType` | Riservata | Il tipo di runtime che è attualmente in esecuzione. Introdotto in MSBuild 15. Il valore può essere non definito (prima di MSBuild 15), `Full` indicante che MSBuild è in esecuzione sul desktop .NET Framework, `Core` che indica che MSBuild è in esecuzione in .NET Core (ad esempio, in `dotnet build`) o `Mono` che indica che MSBuild è in esecuzione su mono. |
