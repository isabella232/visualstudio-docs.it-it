...
ms.locfileid: "77631237"
---
# <a name="vbc-task"></a>Vbc (attività)

Esegue il wrapping di *vbc.exe*, che produce file eseguibili (*.exe*), librerie a collegamento dinamico (*dll*) o moduli di codice (*.netmodule*). Per ulteriori informazioni su *vbc.exe*, vedere [Compilatore da riga di comando](/dotnet/visual-basic/reference/command-line-compiler/index)di Visual Basic .
...
| `NoConfig` | Parametro `Boolean` facoltativo.<br /><br /> Specifica che il compilatore non deve utilizzare il file *vbc.rsp.* Questo parametro corrisponde al parametro [-noconfig](/dotnet/visual-basic/reference/command-line-compiler/noconfig) del compilatore *vbc.exe.* |
...
| `TargetCompactFramework` | Parametro `Boolean` facoltativo.<br /><br /> Se `true`, l'attività è destinata a .NET Compact Framework. Questa opzione corrisponde all'opzione [-netcf](/dotnet/visual-basic/reference/command-line-compiler/netcf) del compilatore *vbc.exe.* |
...
| `ToolPath` | Parametro `String` facoltativo.<br /><br /> Specifica il percorso da cui l'attività caricherà il file eseguibile sottostante (*vbc.exe*). Se questo parametro non viene specificato, l'attività utilizza il percorso di installazione SDK corrispondente alla versione del framework che esegue MSBuild. |
...
| `Win32Icon` | Parametro `String` facoltativo.<br /><br /> Inserisce un file *ICO* nell'assieme, che conferisce al file di output l'aspetto desiderato in **Esplora file**. Questo parametro corrisponde all'opzione [-win32icon](/dotnet/visual-basic/reference/command-line-compiler/win32icon) del compilatore *vbc.exe.* |
...
- [Compilatore da riga di comando di Visual Basic](/dotnet/visual-basic/reference/command-line-compiler/index)
