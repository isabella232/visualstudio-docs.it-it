 ...
 *MSBuild.exe* riconosce qualsiasi estensione di file di progetto che corrisponda al modello *.\*proj*. Tuttavia, in Visual Studio viene riconosciuto solo un subset di queste estensioni di file di progetto, che determinano il sistema di progetto specifico del linguaggio che caricherà il progetto. Visual Studio non dispone di un sistema di progetto basato su MSBuild indipendente dal linguaggio.

 Il C# sistema di progetto carica, ad esempio, i file con *estensione csproj* , ma Visual Studio non è in grado di caricare un file con *estensione xxproj* . Un file di progetto per i file di origine in un linguaggio arbitrario deve usare la stessa estensione C# dei file di progetto o di Visual Basic da caricare in Visual Studio.
...
## <a name="configurations-and-platforms"></a>Configurazioni e piattaforme
 ...
 Le configurazioni sono rappresentate nei progetti MSBuild da proprietà raggruppate in un elemento `PropertyGroup` che contiene un attributo `Condition`. In Visual Studio vengono esaminate queste condizioni per creare un elenco di configurazioni di progetto e piattaforme da visualizzare. Per estrarre correttamente tale elenco, le condizioni devono avere un formato simile a quello riportato di seguito:
...
```
 ...
 Visual Studio esamina le condizioni sugli elementi `PropertyGroup`, `ItemGroup`, `Import`, Property e Item a questo scopo.
...
## <a name="build-solutions"></a>Creare soluzioni
 ...
 Quando si compila in Visual Studio, la proprietà `$(BuildingInsideVisualStudio)` è impostata su `true`. Questa impostazione può essere usata nel progetto o nei file con estensione *targets* per modificare il comportamento della compilazione.
...
## <a name="debugging"></a>Debug
 ...
 Per trovare e avviare l'assembly di output e allinearlo, Visual Studio richiede che le proprietà `OutputPath`, `AssemblyName`e `OutputType` definite correttamente. La connessione del debugger non riesce se con il processo di compilazione il compilatore non ha generato un file con estensione *pdb*.
...
## <a name="intellisense-and-validation"></a>IntelliSense e convalida
 ...
 Quando si usa l'editor XML per modificare i file di progetto, IntelliSense e la convalida sono basati sui file dello schema MSBuild. Tali file vengono installati nella cache dello schema, che si trova in *\<Directory di installazione di Visual Studio>Xml\Schemas\1040\MSBuild*.
