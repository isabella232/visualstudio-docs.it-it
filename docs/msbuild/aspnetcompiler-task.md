...
|`Updateable`|Parametro `Boolean` facoltativo.<br /><br /> Se questo parametro è `true`, l'applicazione precompilata sarà aggiornabile.  Il valore predefinito è `false`. Questo parametro corrisponde all'opzione **-u** su *aspnet_compiler. exe*.|
...
Nell'esempio di codice seguente viene `AspNetCompiler` usata l'attività per precompilare un'applicazione ASP.NET.
