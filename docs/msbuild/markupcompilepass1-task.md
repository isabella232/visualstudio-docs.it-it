...
ms.locfileid: "77633512"
---
# <a name="markupcompilepass1-task"></a>Attività MarkupCompilePass1

L'attività <xref:Microsoft.Build.Tasks.Windows.MarkupCompilePass1> converte i file di progetto XAML non localizzabili in formato binario compilato.
...
| `ApplicationMarkup` | Parametro **Facoltativo ITaskItem[].**<br /><br /> Specifica il nome del file XAML di definizione dell'applicazione. |
...
| `AssemblyName` | Parametro **di stringa** obbligatorio.<br /><br /> Specifica il nome breve dell'assembly generato per un progetto. Ad esempio, se un progetto genera un eseguibile di Windows il cui nome è *WinExeAssembly.exe*, il parametro **AssemblyName** ha il valore **WinExeAssembly**. |
...
| `ContentFiles` | Parametro **Facoltativo ITaskItem[].**<br /><br /> Specifica l'elenco dei file di contenuto separati. |
| `DefineConstants` | Parametro **String** facoltativo.<br /><br /> Specifica che viene mantenuto il valore corrente di **DefineConstants**, che influisce sulla generazione dell'assieme di destinazione; Se questo parametro viene modificato, l'API pubblica nell'assembly di destinazione può essere modificata e la compilazione di file XAML che fanno riferimento a tipi locali può essere interessata. |
| `ExtraBuildControlFiles` | Parametro **Facoltativo ITaskItem[].**<br /><br /> Specifica un elenco di file che controllano l'attivazione di una ricompilazione durante la riesecuzione dell'attività <xref:Microsoft.Build.Tasks.Windows.MarkupCompilePass1>. La ricompilazione viene attivata in caso di modifica di uno di questi file. |
...
| `KnownReferencePaths` | Parametro **String[]** facoltativo.<br /><br /> Specifica i riferimenti ad assembly che non vengono modificati durante il processo di compilazione. Include gli assembly che si trovano nella Global Assembly Cache (GAC), in una directory di installazione .NET e così via. |
...
| `LanguageSourceExtension` | Parametro **String** facoltativo.<br /><br /> Specifica l'estensione aggiunta all'estensione del file di codice gestito generato:<br /><br /> `<Filename>.g<LanguageSourceExtension>`<br /><br /> Se il parametro **LanguageSourceExtension** non è impostato con un valore specifico, viene utilizzata l'estensione del nome file di origine predefinita per un linguaggio: *.vb* per Visual Basic, *.csharp* per C. |
...
| `PageMarkup` | Parametro **Facoltativo ITaskItem[].**<br /><br /> Specifica un elenco di file XAML da elaborare. |
| `References` | Parametro **Facoltativo ITaskItem[].**<br /><br /> Specifica l'elenco di riferimenti dai file agli assembly che contengono i tipi utilizzati nei file XAML. |
...
| `SourceCodeFiles` | Parametro **Facoltativo ITaskItem[].**<br /><br /> Specifica l'elenco dei file di codice per il progetto corrente. L'elenco non include file di codice gestito generati specifici del linguaggio. |
| `UICulture` | Parametro **String** facoltativo.<br /><br /> Specifica l'assembly satellite per le impostazioni cultura dell'interfaccia utente in cui sono incorporati i file in formato binario XAML generati. Se **UICulture** non è impostato, i file di formato binario XAML generati vengono incorporati nell'assembly principale. |
...
L'attività <xref:Microsoft.Build.Tasks.Windows.MarkupCompilePass1> in genere compila XAML in formato binario e genera file di codice. Se un file XAML contiene riferimenti a tipi definiti nello stesso progetto, la compilazione in formato binario viene posticipata da **MarkupCompilePass1** a un secondo passaggio di compilazione del markup (**MarkupCompilePass2**). La compilazione di tali file deve essere rinviata poiché è necessario attendere la compilazione dei tipi definiti localmente a cui si fa riferimento. Tuttavia, se un `x:Class` file <xref:Microsoft.Build.Tasks.Windows.MarkupCompilePass1> XAML ha un attributo, genera il file di codice specifico del linguaggio per esso.

Un file XAML è localizzabile se `x:Uid` contiene elementi che usano l'attributo:A XAML file is localizable if it contains elements that use the attribute:
...
Un file XAML fa riferimento a un tipo definito localmente quando dichiara uno spazio dei nomi XML che usa il valore per fare riferimento a uno spazio dei nomi nel progetto corrente:A XAML file references a locally-defined type when it declares an XML namespace that uses the `clr-namespace` value to refer to a namespace in the current project:
