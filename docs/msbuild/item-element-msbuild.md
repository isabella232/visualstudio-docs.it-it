...
Contiene un elemento definito dall'utente e i relativi metadati. Ogni elemento usato in un progetto MSBuild deve essere specificato come figlio di un elemento `ItemGroup`.
...
Il motore MSBuild può valutare caratteri jolly, ad esempio `*` e `?` e caratteri jolly ricorsivi, ad esempio */\*\*/. cs*.\* Per altre informazioni, vedere [Items](../msbuild/msbuild-items.md) (Elementi).
