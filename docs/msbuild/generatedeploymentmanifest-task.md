...
Genera un manifesto di distribuzione ClickOnce. Un manifesto di distribuzione ClickOnce descrive la distribuzione di un'applicazione definendo un'identità univoca per la distribuzione, identificando le caratteristiche di distribuzione, ad esempio l'installazione o la modalità online, specificando le impostazioni di aggiornamento e i percorsi di aggiornamento dell'applicazione e che indica il manifesto dell'applicazione ClickOnce corrispondente.
...
| `MapFileExtensions` | Parametro `Boolean` facoltativo.<br /><br /> Specifica se viene usato il mapping dell'estensione di file *deploy*. Se il parametro è impostato su `true`, tutti i file di programma vengono pubblicati con l'estensione di file *deploy*. Questa opzione è utile per la sicurezza del server Web per limitare il numero di estensioni di file che devono essere sbloccate per consentire la distribuzione di applicazioni ClickOnce. Il valore predefinito di questo parametro è `false`. |
