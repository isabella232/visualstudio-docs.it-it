...
L NuGet Gestione pacchetti'interfaccia utente in Visual Studio per Mac consente di installare, disinstallare e aggiornare facilmente i NuGet in progetti e soluzioni. È possibile cercare e aggiungere pacchetti ai progetti .NET Core, ASP.NET Core e Xamarin.
...
Per un'introduzione all'NuGet in Visual Studio per Mac, vedere Guida introduttiva: Installare e [usare un pacchetto in Visual Studio per Mac](/nuget/quickstart/install-and-use-a-package-in-visual-studio-mac)
...
2. Verrà visualizzata la **finestra Gestisci NuGet distribuzione.** Assicurarsi che l'elenco a discesa Origine nell'angolo superiore sinistro della finestra di dialogo sia impostato su , in modo da eseguire la ricerca nel repository dei pacchetti NuGet `nuget.org` centrale.
...
Per eseguire l'aggiornamento a una versione non definitiva più  recente di un pacchetto, è possibile fare clic con il pulsante destro del mouse su Dipendenze per aprire il menu di **scelta** rapida e scegliere il menu NuGet pacchetti.
...
Verrà visualizzata anche una notifica accanto al nome del pacchetto quando è disponibile una nuova versione di un pacchetto, in modo da poter decidere se è possibile aggiornarlo.
