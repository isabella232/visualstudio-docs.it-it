...
Perché un pacchetto di estensione possa essere compilato da Visual Studio per Mac, deve includere estensioni compilate da punti di estensione preesistenti all'interno dell'IDE di Visual Studio per Mac. Quando un pacchetto di estensione si basa su un punto di estensione definito in un host di componenti aggiuntivi, si dice che presenta una _dipendenza_   da tale pacchetto di estensione.
...
I pacchetti di estensione archiviano i metadati su nome, versione, dipendenze e altre informazioni in attributi C#. Addin Maker crea due file, `AddinInfo.cs` e `AssemblyInfo.cs`, per archiviare e organizzare queste informazioni. I pacchetti di estensione devono avere un ID univoco e uno spazio dei nomi specificati nell' * `Addin` attributo*:
...
Con queste risorse a disposizione, i concetti principali che è necessario conoscere sono [`ITextBuffer`](/dotnet/api/microsoft.visualstudio.text.itextbuffer) [`ITextView`](/dotnet/api/microsoft.visualstudio.text.editor.itextview) :
...
Per un oggetto specificato [`MonoDevelop.Ide.Gui.Document`](http://source.monodevelop.com/#MonoDevelop.Ide/MonoDevelop.Ide.Gui/Document.cs,4e960d4735f089b5) , è possibile recuperare rispettivamente le sottostanti associate `ITextBuffer` e `ITextView` tramite `Document.GetContent<ITextBuffer>()` e `Document.GetContent<ITextView>()` .
