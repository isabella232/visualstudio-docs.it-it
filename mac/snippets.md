...
La finestra **Modifica modello** è illustrata di seguito e rappresenta la modifica del frammento predefinito `prop`. Il frammento di codice contiene due parole chiave &ndash; `$type$` e `$name$` &ndash; per le quali possono essere impostate altre proprietà (ad esempio un valore predefinito e la descrizione comando) sul lato destro della finestra:
...
- `$end$` &ndash; Quando l'utente termina la modifica delle parole chiave in un frammento di codice, il cursore verrà posizionato in corrispondenza della posizione della parola chiave `$end$`.
